...
Fix:      0309_marketplace_parties_are_not_editable.sql. The obvious repair —
          ...
          "you may not change who the parties are". 0309 makes the four identity
...
Status:   FIXED — inert until an operator applies 0309
          (docs/PENDING_PROD_MIGRATIONS.md, which also gained the 0307 and 0308
          rows it was missing).
```
...
Fix:      0310_a_review_belongs_to_whoever_wrote_it.sql. Scoped to the owner
          ...
          0309's table-branching trigger function is replaced by
          columns_are_immutable(), which takes its column list from the trigger
          definition; 0309's two triggers are re-pointed at it. That generality
...
Problem:  0310 stopped a member REWRITING another member's review. The DELETE
          ...
          one query.
Evidence: Replayed schema, before 0311:
...
Fix:      0311_deleting_a_review_is_rewriting_it.sql. Offers scoped to the two
...
Fix:      0312_a_social_restriction_is_not_self_service.sql — DELETE carries the
