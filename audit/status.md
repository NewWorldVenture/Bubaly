...
NEXT: classify and convert what is left — app/ 44 sites in 25 files, lib/ 79 in 52. COMPONENTS/ IS AT ZERO. A meaningful share of the remainder is CORRECT rather than outstanding: the AI prompt builders (the reader is the model), crons and CSV exports (no reader whose language is known), Super Admin pages (the platform's own currency), and lib/i18n's locale codes, which are data. Classify those explicitly first so the ceiling can stop at the honest floor rather than at zero. Then re-read claude-2/3/4.md for anything still OPEN and sweep for gaps none of the four workers reached.
