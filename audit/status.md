...
NEXT: the 31 remaining mixed component files, ONE FILE AT A TIME — a sweep was attempted and reverted (Pass AE). Four independent per-file facts break a mechanical edit: a chained helper (documents-module's relative-time helper falls through to fmtDate), a pre-existing `locale` declared further in than any head window, declaration order when that existing one comes after the insertion point, and async SERVER components mixed in (ai-home-dashboard, like the two dashboards in Pass AD). Plus ten files declare helpers as `const fmtDate = ...` rather than `function`. Per file: check those four, convert, tsc + eslint, commit, repeat. Then lib/ (66), app pages (18), api routes (18) — ~39 of the 180 are CORRECT as en-US (AI prompts, Super Admin, lib/i18n's locale-as-data).
