...
NEXT: the 54 lib/ shared helpers reached by a family surface. COMPONENTS/ IS AT ZERO and APP/ IS AT ITS FLOOR OF 25 (17 api/ai prompt sites, 7 Super Admin, 1 cron — all exempt, nothing family-facing left there). Ratchet at 104, floor 41, so 63 real defects remain: 54 in lib/, 8 BLOCKED on I18N-001 (a family's language lives only in a cookie, so nothing a cron sends can be localised — owner-owned, needs a migration), and lib/activity/feed.ts. Each lib/ helper must be converted WITH ITS CALLERS so the ratchet only falls for output that actually changed.
