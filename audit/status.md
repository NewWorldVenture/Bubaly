...
NEXT: CONVERT the remaining 180 (ratchet ceiling), of which ~39 are CORRECT as en-US (13 AI-prompt sites where the reader is the model, 8 Super Admin, and lib/i18n's locale-as-data). The real remainder: the 34 MIXED component files (57 sites) where a module-scope helper must be threaded together with its call sites — the file is the unit, because converting only the component-level uses leaves one date localised and another not in the same view; then lib/ (66), app pages (18), api routes (18).
