...
NEXT: S-05 (163 CASCADE FKs with no index — measured 5,715 buffers -> 4), U-05 (74 icon-only buttons with no accessible name; plus the bare <div onClick> calendar/note/photo rows), then the 245 hardcoded 'en-US' formatters and the USD-only lib/utils/format.ts on an eleven-locale product. Rebuild finalaudit.md from all four files as each lands.
