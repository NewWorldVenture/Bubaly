...
NEXT: S-04 (audit_logs lets any member forge actor_id, including the family_id-NULL rows the admin Security page renders with the service client), S-05 (163 CASCADE FKs with no index), then U-05 (74 icon-only buttons with no accessible name). Rebuild finalaudit.md from all four files as each lands.
