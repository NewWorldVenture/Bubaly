...
NEXT: CONVERT the remaining 240 (ratchet ceiling). lib/marketplace/fees.ts (6) and listings.ts (2) each define their OWN formatCents shadowing the ledger's, inside narrative strings — a separate piece. Then app/(app)/marketplace/{orders,insights,item} (8 server), components/modules/chores-module.tsx (1), and the six other lib/ money functions. lib/wallet/ledger.ts formatCents is the big one — 137 calls across 22 files outside lib/ — then lib/finance/{hub,splits,timeline}.ts, lib/home/utilities.ts, lib/purchases/answer.ts, lib/services/finances/index.ts, lib/wallet/hub.ts. Each must be converted WITH ITS CALLERS: adding an optional locale param that nobody passes lowers the ratchet without changing anything a family sees, which would make the ratchet lie. Then the 70 client and 74 server date surfaces. Then re-sweep the four worker files for anything still OPEN.
