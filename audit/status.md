...
NEXT: convert the 74 unblocked defects. THE CLASSIFICATION IS DONE (Pass AG) — of the 123 remaining, 41 are CORRECT as en-US and must be left: 17 app/api/ai prompt sites (read by the MODEL — verified in chat/route.ts:110), 13 lib/ai builders, 9 Super Admin pages, and 2 in lib/onboarding/ics-time.ts which are NOT a display formatter at all but a timezone-resolution mechanism pinning calendar/numberingSystem/hourCycle for ICS part extraction — a sweep to zero would have BROKEN that parser. THE HONEST FLOOR IS 41. Of the 82 defects, 8 are BLOCKED on the owner: a family's language lives only in a cookie with no column on any member table, so nothing a cron sends can be localised (see Pass AG). The 74 to do: lib/ shared helpers reached by a family surface (54), family-facing pages (18), lib/activity/feed.ts, the feedback board.
