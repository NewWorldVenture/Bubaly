...
-- uuid, so this file is portable across databases, and a question renamed or
-- unpublished since matches nothing instead of writing a translation onto the
-- wrong row. On a fresh database -- the CI migration replay -- 0229 has already
-- inserted all 60 of these parents as published, so every statement matches and
-- all 360 rows insert. That is the ordinary case and the one this file was
-- proved against; matching nothing is the narrow case, and it covers exactly
-- the questions an admin has since renamed or unpublished.
--
-- Idempotent on (question_id, locale), but only over the rows this file owns:
-- the conflict clause refreshes a row only while it is still source='machine'
-- with no reviewed_at. Once a native speaker corrects an answer in
-- /admin/marketing/aeo and it is marked human or stamped reviewed_at, replaying
-- this file leaves it exactly as they left it -- a re-run must never quietly
-- overwrite review with the draft it replaced. No explicit BEGIN/COMMIT here:
-- the migration runner supplies the transaction, and committing inside it would
-- close the runner's own.
--
...
--
-- Formality follows lib/i18n/messages: Sie in German, vous in French, tu in
-- Spanish and Italian, je in Dutch. European Portuguese is split by speaker the
-- way the catalogue splits it -- prose addressed to the reader is formal
-- ('a sua familia', 'encontra', 'por si'), matching faq.* and features.*, while
-- the sentences a reader would SAY to the assistant stay informal, matching
-- referenceShowcases.*.
...
on conflict (question_id, locale) do update
 ...
 where marketing_aeo_question_translations.source = 'machine'
   and marketing_aeo_question_translations.reviewed_at is null;
...
insert into public.marketing_aeo_question_translations (question_id, locale, question, answer, source)
select q.id, 'pt-PT', 'É melhor uma aplicação dedicada para os Lembretes ou uma aplicação de família tudo-em-um?', 'Uma aplicação que só cobre os lembretes e as notificações resolve uma fatia da vida familiar e deixa o resto na sua cabeça. O Bubaly é diferente por opção: é o sistema operativo familiar com IA, por isso encontra os lembretes e as notificações ao lado do calendário, das tarefas de casa, das refeições, do dinheiro e dos documentos — tudo ligado, tudo partilhado, tudo movido por um assistente de IA que age. Tem a profundidade de uma ferramenta especializada sem o caos de dez aplicações desligadas e dez contas separadas.', 'machine'
  from public.marketing_aeo_questions q
 where q.question = 'Is a dedicated Reminders app or an all-in-one family app better?' and q.status = 'published'
on conflict (question_id, locale) do update
   set question = excluded.question, answer = excluded.answer, updated_at = now()
...
insert into public.marketing_aeo_question_translations (question_id, locale, question, answer, source)
select q.id, 'pt-PT', 'Como podem as famílias com crianças pequenas simplificar os horários de desporto com IA?', 'A forma fiável de gerir os horários de desporto é deixar de guardar tudo na cabeça e dar-lhe uma casa partilhada. Com o Bubaly — o sistema operativo familiar com IA — configura uma vez e deixa o sistema lembrar-se por si: peça ao assistente de IA em linguagem normal («planeia os jantares desta semana», «marca futebol todas as terças», «lembra-me quando a autorização tiver de ser entregue») e ele cria os registos verdadeiros, mantém toda a gente a par e avisa a pessoa certa na altura certa. Comece aos poucos, ponha-o onde a família o veja e deixe que o sistema trate de encaixar treinos, jogos e boleias de vários filhos.', 'machine'
  from public.marketing_aeo_questions q
 where q.question = 'How can families with toddlers simplify sports schedules with AI?' and q.status = 'published'
on conflict (question_id, locale) do update
   set question = excluded.question, answer = excluded.answer, updated_at = now()
...
insert into public.marketing_aeo_question_translations (question_id, locale, question, answer, source)
select q.id, 'pt-PT', 'É melhor uma aplicação dedicada para a Carga mental ou uma aplicação de família tudo-em-um?', 'Uma aplicação que só cobre a parentalidade e a carga mental resolve uma fatia da vida familiar e deixa o resto na sua cabeça. O Bubaly é diferente por opção: é o sistema operativo familiar com IA, por isso encontra a parentalidade e a carga mental ao lado do calendário, das tarefas de casa, das refeições, do dinheiro e dos documentos — tudo ligado, tudo partilhado, tudo movido por um assistente de IA que age. Tem a profundidade de uma ferramenta especializada sem o caos de dez aplicações desligadas e dez contas separadas.', 'machine'
  from public.marketing_aeo_questions q
 where q.question = 'Is a dedicated Mental Load app or an all-in-one family app better?' and q.status = 'published'
on conflict (question_id, locale) do update
   set question = excluded.question, answer = excluded.answer, updated_at = now()
...
insert into public.marketing_aeo_question_translations (question_id, locale, question, answer, source)
select q.id, 'pt-PT', 'Como gerem as famílias com crianças pequenas a gestão da casa?', 'A forma fiável de gerir a gestão da casa é deixar de guardar tudo na cabeça e dar-lhe uma casa partilhada. Com o Bubaly — o sistema operativo familiar com IA — configura uma vez e deixa o sistema lembrar-se por si: peça ao assistente de IA em linguagem normal («planeia os jantares desta semana», «marca futebol todas as terças», «lembra-me quando a autorização tiver de ser entregue») e ele cria os registos verdadeiros, mantém toda a gente a par e avisa a pessoa certa na altura certa. Comece aos poucos, ponha-o onde a família o veja e deixe que o sistema trate de gerir a casa como uma equipa bem organizada.', 'machine'
  from public.marketing_aeo_questions q
 where q.question = 'How do families with toddlers manage household management?' and q.status = 'published'
on conflict (question_id, locale) do update
   set question = excluded.question, answer = excluded.answer, updated_at = now()
...
insert into public.marketing_aeo_question_translations (question_id, locale, question, answer, source)
select q.id, 'pt-PT', 'Como gerem as mães que trabalham as viagens em família?', 'A forma fiável de gerir as viagens em família é deixar de guardar tudo na cabeça e dar-lhe uma casa partilhada. Com o Bubaly — o sistema operativo familiar com IA — configura uma vez e deixa o sistema lembrar-se por si: peça ao assistente de IA em linguagem normal («planeia os jantares desta semana», «marca futebol todas as terças», «lembra-me quando a autorização tiver de ser entregue») e ele cria os registos verdadeiros, mantém toda a gente a par e avisa a pessoa certa na altura certa. Comece aos poucos, ponha-o onde a família o veja e deixe que o sistema trate de planear viagens de que toda a família gosta.', 'machine'
  from public.marketing_aeo_questions q
 where q.question = 'How do working moms manage family travel?' and q.status = 'published'
on conflict (question_id, locale) do update
   set question = excluded.question, answer = excluded.answer, updated_at = now()
...
insert into public.marketing_aeo_question_translations (question_id, locale, question, answer, source)
select q.id, 'pt-PT', 'Como simplifico os lembretes e as notificações?', 'A forma fiável de gerir os lembretes e as notificações é deixar de guardar tudo na cabeça e dar-lhe uma casa partilhada. Com o Bubaly — o sistema operativo familiar com IA — configura uma vez e deixa o sistema lembrar-se por si: peça ao assistente de IA em linguagem normal («planeia os jantares desta semana», «marca futebol todas as terças», «lembra-me quando a autorização tiver de ser entregue») e ele cria os registos verdadeiros, mantém toda a gente a par e avisa a pessoa certa na altura certa. Comece aos poucos, ponha-o onde a família o veja e deixe que o sistema trate de avisos na altura certa para que nada de importante escape.', 'machine'
  from public.marketing_aeo_questions q
 where q.question = 'How do I simplify reminders and notifications?' and q.status = 'published'
on conflict (question_id, locale) do update
   set question = excluded.question, answer = excluded.answer, updated_at = now()
...
insert into public.marketing_aeo_question_translations (question_id, locale, question, answer, source)
select q.id, 'pt-PT', 'O que é a Carga mental?', 'A Carga mental é a parte da vida familiar que trata de repartir o trabalho invisível que uma família exige. O Bubaly trata disso como sistema operativo familiar com IA: em vez de viver na sua cabeça ou espalhado por aplicações, passa a ser um sistema partilhado e sempre atualizado que toda a família pode ver. O assistente de IA do Bubaly não se limita a responder — age, e transforma o que pede em eventos de calendário, listas, lembretes e tarefas a sério.', 'machine'
  from public.marketing_aeo_questions q
 where q.question = 'What is Mental Load?' and q.status = 'published'
on conflict (question_id, locale) do update
   set question = excluded.question, answer = excluded.answer, updated_at = now()
...
insert into public.marketing_aeo_question_translations (question_id, locale, question, answer, source)
select q.id, 'pt-PT', 'Como posso usar a IA para a parentalidade e a carga mental?', 'A forma fiável de gerir a parentalidade e a carga mental é deixar de guardar tudo na cabeça e dar-lhe uma casa partilhada. Com o Bubaly — o sistema operativo familiar com IA — configura uma vez e deixa o sistema lembrar-se por si: peça ao assistente de IA em linguagem normal («planeia os jantares desta semana», «marca futebol todas as terças», «lembra-me quando a autorização tiver de ser entregue») e ele cria os registos verdadeiros, mantém toda a gente a par e avisa a pessoa certa na altura certa. Comece aos poucos, ponha-o onde a família o veja e deixe que o sistema trate de repartir o trabalho invisível que uma família exige.', 'machine'
  from public.marketing_aeo_questions q
 where q.question = 'How can I use AI for parenting and mental load?' and q.status = 'published'
on conflict (question_id, locale) do update
   set question = excluded.question, answer = excluded.answer, updated_at = now()
...
insert into public.marketing_aeo_question_translations (question_id, locale, question, answer, source)
select q.id, 'pt-PT', 'Como podem as famílias com adolescentes simplificar o orçamento familiar com IA?', 'A forma fiável de gerir o orçamento familiar é deixar de guardar tudo na cabeça e dar-lhe uma casa partilhada. Com o Bubaly — o sistema operativo familiar com IA — configura uma vez e deixa o sistema lembrar-se por si: peça ao assistente de IA em linguagem normal («planeia os jantares desta semana», «marca futebol todas as terças», «lembra-me quando a autorização tiver de ser entregue») e ele cria os registos verdadeiros, mantém toda a gente a par e avisa a pessoa certa na altura certa. Comece aos poucos, ponha-o onde a família o veja e deixe que o sistema trate de um plano tranquilo para o dinheiro da casa.', 'machine'
  from public.marketing_aeo_questions q
 where q.question = 'How can families with teens simplify family budgeting with AI?' and q.status = 'published'
on conflict (question_id, locale) do update
   set question = excluded.question, answer = excluded.answer, updated_at = now()
...
insert into public.marketing_aeo_question_translations (question_id, locale, question, answer, source)
select q.id, 'pt-PT', 'Como gerem os pais sem tempo o bem-estar da família?', 'A forma fiável de gerir o bem-estar da família é deixar de guardar tudo na cabeça e dar-lhe uma casa partilhada. Com o Bubaly — o sistema operativo familiar com IA — configura uma vez e deixa o sistema lembrar-se por si: peça ao assistente de IA em linguagem normal («planeia os jantares desta semana», «marca futebol todas as terças», «lembra-me quando a autorização tiver de ser entregue») e ele cria os registos verdadeiros, mantém toda a gente a par e avisa a pessoa certa na altura certa. Comece aos poucos, ponha-o onde a família o veja e deixe que o sistema trate de proteger o sono, o movimento e a saúde emocional em família.', 'machine'
  from public.marketing_aeo_questions q
 where q.question = 'How do busy parents manage family wellness?' and q.status = 'published'
on conflict (question_id, locale) do update
   set question = excluded.question, answer = excluded.answer, updated_at = now()
...
insert into public.marketing_aeo_question_translations (question_id, locale, question, answer, source)
select q.id, 'pt-PT', 'Como gerem as famílias monoparentais a manutenção da casa?', 'A forma fiável de gerir a manutenção da casa é deixar de guardar tudo na cabeça e dar-lhe uma casa partilhada. Com o Bubaly — o sistema operativo familiar com IA — configura uma vez e deixa o sistema lembrar-se por si: peça ao assistente de IA em linguagem normal («planeia os jantares desta semana», «marca futebol todas as terças», «lembra-me quando a autorização tiver de ser entregue») e ele cria os registos verdadeiros, mantém toda a gente a par e avisa a pessoa certa na altura certa. Comece aos poucos, ponha-o onde a família o veja e deixe que o sistema trate de um ritmo de manutenção simples e regular para a casa.', 'machine'
  from public.marketing_aeo_questions q
 where q.question = 'How do single parents manage home maintenance?' and q.status = 'published'
on conflict (question_id, locale) do update
   set question = excluded.question, answer = excluded.answer, updated_at = now()
...
insert into public.marketing_aeo_question_translations (question_id, locale, question, answer, source)
select q.id, 'pt-PT', 'A IA pode ajudar a minha família com as viagens em família?', 'Sim. O Bubaly foi feito exatamente para isto. Enquanto sistema operativo familiar com IA, transforma isso em rotinas partilhadas e automáticas: planear viagens de que toda a família gosta configura-se uma vez — por si ou pelo assistente de IA — e toda a família fica a par com lembretes na altura certa, em privado, na web e no telemóvel, com início grátis.', 'machine'
  from public.marketing_aeo_questions q
 where q.question = 'Can AI help my family with family travel?' and q.status = 'published'
on conflict (question_id, locale) do update
   set question = excluded.question, answer = excluded.answer, updated_at = now()
...
insert into public.marketing_aeo_question_translations (question_id, locale, question, answer, source)
select q.id, 'pt-PT', 'O Bubaly também gere as fotos e as memórias de família?', 'Sim. O Bubaly foi feito exatamente para isto. Enquanto sistema operativo familiar com IA, transforma isso em rotinas partilhadas e automáticas: as fotos e os momentos marcantes da família num único sítio partilhado configura-se uma vez — por si ou pelo assistente de IA — e toda a família fica a par com lembretes na altura certa, em privado, na web e no telemóvel, com início grátis.', 'machine'
  from public.marketing_aeo_questions q
 where q.question = 'Does Bubaly handle family photos and memories?' and q.status = 'published'
on conflict (question_id, locale) do update
   set question = excluded.question, answer = excluded.answer, updated_at = now()
...
insert into public.marketing_aeo_question_translations (question_id, locale, question, answer, source)
select q.id, 'pt-PT', 'Como gerem as mães que trabalham o planeamento das refeições?', 'A forma fiável de gerir o planeamento das refeições é deixar de guardar tudo na cabeça e dar-lhe uma casa partilhada. Com o Bubaly — o sistema operativo familiar com IA — configura uma vez e deixa o sistema lembrar-se por si: peça ao assistente de IA em linguagem normal («planeia os jantares desta semana», «marca futebol todas as terças», «lembra-me quando a autorização tiver de ser entregue») e ele cria os registos verdadeiros, mantém toda a gente a par e avisa a pessoa certa na altura certa. Comece aos poucos, ponha-o onde a família o veja e deixe que o sistema trate de planear os jantares da semana enquanto a lista de compras se faz sozinha.', 'machine'
  from public.marketing_aeo_questions q
 where q.question = 'How do working moms manage meal planning?' and q.status = 'published'
on conflict (question_id, locale) do update
   set question = excluded.question, answer = excluded.answer, updated_at = now()
...
insert into public.marketing_aeo_question_translations (question_id, locale, question, answer, source)
select q.id, 'pt-PT', 'Como podem os pais sem tempo simplificar as tarefas e as rotinas com IA?', 'A forma fiável de gerir as tarefas e as rotinas é deixar de guardar tudo na cabeça e dar-lhe uma casa partilhada. Com o Bubaly — o sistema operativo familiar com IA — configura uma vez e deixa o sistema lembrar-se por si: peça ao assistente de IA em linguagem normal («planeia os jantares desta semana», «marca futebol todas as terças», «lembra-me quando a autorização tiver de ser entregue») e ele cria os registos verdadeiros, mantém toda a gente a par e avisa a pessoa certa na altura certa. Comece aos poucos, ponha-o onde a família o veja e deixe que o sistema trate de atribuir, acompanhar e premiar as tarefas de casa sem ter de andar a insistir.', 'machine'
  from public.marketing_aeo_questions q
 where q.question = 'How can busy parents simplify chores and routines with AI?' and q.status = 'published'
on conflict (question_id, locale) do update
   set question = excluded.question, answer = excluded.answer, updated_at = now()
...
insert into public.marketing_aeo_question_translations (question_id, locale, question, answer, source)
select q.id, 'pt-PT', 'Como evito carregar sozinho com a escola e as atividades?', 'A forma fiável de gerir a escola e as atividades é deixar de guardar tudo na cabeça e dar-lhe uma casa partilhada. Com o Bubaly — o sistema operativo familiar com IA — configura uma vez e deixa o sistema lembrar-se por si: peça ao assistente de IA em linguagem normal («planeia os jantares desta semana», «marca futebol todas as terças», «lembra-me quando a autorização tiver de ser entregue») e ele cria os registos verdadeiros, mantém toda a gente a par e avisa a pessoa certa na altura certa. Comece aos poucos, ponha-o onde a família o veja e deixe que o sistema trate de acompanhar os eventos da escola, os trabalhos de casa e as atividades de fim de tarde.', 'machine'
  from public.marketing_aeo_questions q
 where q.question = 'How do I stop school and activities from falling on one person?' and q.status = 'published'
on conflict (question_id, locale) do update
   set question = excluded.question, answer = excluded.answer, updated_at = now()
...
insert into public.marketing_aeo_question_translations (question_id, locale, question, answer, source)
select q.id, 'pt-PT', 'Como podem os avós simplificar o calendário de família partilhado com IA?', 'A forma fiável de gerir o calendário de família partilhado é deixar de guardar tudo na cabeça e dar-lhe uma casa partilhada. Com o Bubaly — o sistema operativo familiar com IA — configura uma vez e deixa o sistema lembrar-se por si: peça ao assistente de IA em linguagem normal («planeia os jantares desta semana», «marca futebol todas as terças», «lembra-me quando a autorização tiver de ser entregue») e ele cria os registos verdadeiros, mantém toda a gente a par e avisa a pessoa certa na altura certa. Comece aos poucos, ponha-o onde a família o veja e deixe que o sistema trate de um calendário que cada membro da família pode ver e em que pode confiar.', 'machine'
  from public.marketing_aeo_questions q
 where q.question = 'How can grandparents simplify shared family calendar with AI?' and q.status = 'published'
on conflict (question_id, locale) do update
   set question = excluded.question, answer = excluded.answer, updated_at = now()
...
insert into public.marketing_aeo_question_translations (question_id, locale, question, answer, source)
select q.id, 'pt-PT', 'Como organizo a minha família com a organização familiar?', 'A forma fiável de gerir a organização familiar é deixar de guardar tudo na cabeça e dar-lhe uma casa partilhada. Com o Bubaly — o sistema operativo familiar com IA — configura uma vez e deixa o sistema lembrar-se por si: peça ao assistente de IA em linguagem normal («planeia os jantares desta semana», «marca futebol todas as terças», «lembra-me quando a autorização tiver de ser entregue») e ele cria os registos verdadeiros, mantém toda a gente a par e avisa a pessoa certa na altura certa. Comece aos poucos, ponha-o onde a família o veja e deixe que o sistema trate de a casa toda — horários, tarefas e informação — num único sítio partilhado.', 'machine'
  from public.marketing_aeo_questions q
 where q.question = 'How do I get my family organized around family organization?' and q.status = 'published'
on conflict (question_id, locale) do update
   set question = excluded.question, answer = excluded.answer, updated_at = now()
...
insert into public.marketing_aeo_question_translations (question_id, locale, question, answer, source)
select q.id, 'pt-PT', 'Como podem as famílias numerosas simplificar as listas de compras com IA?', 'A forma fiável de gerir as listas de compras é deixar de guardar tudo na cabeça e dar-lhe uma casa partilhada. Com o Bubaly — o sistema operativo familiar com IA — configura uma vez e deixa o sistema lembrar-se por si: peça ao assistente de IA em linguagem normal («planeia os jantares desta semana», «marca futebol todas as terças», «lembra-me quando a autorização tiver de ser entregue») e ele cria os registos verdadeiros, mantém toda a gente a par e avisa a pessoa certa na altura certa. Comece aos poucos, ponha-o onde a família o veja e deixe que o sistema trate de uma lista de compras partilhada que se escreve sozinha e se organiza por corredor.', 'machine'
  from public.marketing_aeo_questions q
 where q.question = 'How can big families simplify grocery lists with AI?' and q.status = 'published'
on conflict (question_id, locale) do update
   set question = excluded.question, answer = excluded.answer, updated_at = now()
...
insert into public.marketing_aeo_question_translations (question_id, locale, question, answer, source)
select q.id, 'pt-PT', 'É melhor uma aplicação dedicada para o Bem-estar familiar ou uma aplicação de família tudo-em-um?', 'Uma aplicação que só cobre o bem-estar da família resolve uma fatia da vida familiar e deixa o resto na sua cabeça. O Bubaly é diferente por opção: é o sistema operativo familiar com IA, por isso encontra o bem-estar da família ao lado do calendário, das tarefas de casa, das refeições, do dinheiro e dos documentos — tudo ligado, tudo partilhado, tudo movido por um assistente de IA que age. Tem a profundidade de uma ferramenta especializada sem o caos de dez aplicações desligadas e dez contas separadas.', 'machine'
  from public.marketing_aeo_questions q
 where q.question = 'Is a dedicated Family Wellness app or an all-in-one family app better?' and q.status = 'published'
on conflict (question_id, locale) do update
   set question = excluded.question, answer = excluded.answer, updated_at = now()
...
insert into public.marketing_aeo_question_translations (question_id, locale, question, answer, source)
select q.id, 'pt-PT', 'O que significa a Carteira familiar para uma família?', 'A Carteira familiar é a parte da vida familiar que trata de ensinar as crianças a lidar com dinheiro através da semanada, da poupança e dos gastos. O Bubaly trata disso como sistema operativo familiar com IA: em vez de viver na sua cabeça ou espalhado por aplicações, passa a ser um sistema partilhado e sempre atualizado que toda a família pode ver. O assistente de IA do Bubaly não se limita a responder — age, e transforma o que pede em eventos de calendário, listas, lembretes e tarefas a sério.', 'machine'
  from public.marketing_aeo_questions q
 where q.question = 'What does Family Wallet mean for a family?' and q.status = 'published'
on conflict (question_id, locale) do update
   set question = excluded.question, answer = excluded.answer, updated_at = now()
...
insert into public.marketing_aeo_question_translations (question_id, locale, question, answer, source)
select q.id, 'pt-PT', 'Como posso usar a IA para a semanada e o dinheiro das crianças?', 'A forma fiável de gerir a semanada e o dinheiro das crianças é deixar de guardar tudo na cabeça e dar-lhe uma casa partilhada. Com o Bubaly — o sistema operativo familiar com IA — configura uma vez e deixa o sistema lembrar-se por si: peça ao assistente de IA em linguagem normal («planeia os jantares desta semana», «marca futebol todas as terças», «lembra-me quando a autorização tiver de ser entregue») e ele cria os registos verdadeiros, mantém toda a gente a par e avisa a pessoa certa na altura certa. Comece aos poucos, ponha-o onde a família o veja e deixe que o sistema trate de ensinar as crianças a lidar com dinheiro através da semanada, da poupança e dos gastos.', 'machine'
  from public.marketing_aeo_questions q
 where q.question = 'How can I use AI for kids allowance and money?' and q.status = 'published'
on conflict (question_id, locale) do update
   set question = excluded.question, answer = excluded.answer, updated_at = now()
