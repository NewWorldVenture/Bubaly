...
--
-- IDEMPOTENT + POOLER-SAFE: no temp tables or BEGIN/COMMIT (so it works in
-- the Supabase SQL Editor over a pooled connection). It first clears prior
-- seed rows for the 5 demo family ids, then re-inserts, so it is safe to run
-- repeatedly and only ever touches those 5 demo families.
--
