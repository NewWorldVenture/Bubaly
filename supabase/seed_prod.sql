-- ============================================================
-- FamilyOS :: seed_prod.sql  — ONE paste-and-run script
--
-- Self-contained: (1) creates the migration-0014 Family OS tables if they
-- are missing (idempotent), then (2) ensures the 5 demo families + 25
-- members exist, then (3) seeds >=500 rows into every family-scoped data
-- table (~23k rows total). Safe to re-run; scoped strictly to the 5 demo
-- family ids, so real households are never touched.
--
-- ⚠️  PRODUCTION NOTE: this loads demo data. To remove everything later:
...
-- ─────────────────────────────────────────────────────────
-- PART 2 + 3: demo families/members + high-volume seed
-- ─────────────────────────────────────────────────────────
BEGIN;
...
-- Demo members: reset to exactly 5 per demo family (cascades any prior demo child rows)
DELETE FROM public.family_members WHERE family_id IN (SELECT id FROM _seed_fams);
