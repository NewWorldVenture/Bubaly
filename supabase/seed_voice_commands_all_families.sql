-- ============================================================================
-- seed_voice_commands_all_families.sql — seeds the Voice Control command history
-- (public.voice_commands, migration 0121) for EVERY family / profile.
-- ----------------------------------------------------------------------------
-- Companion to supabase/seed_voice_one_family.sql (which fills ONE showcase
-- family with 500 rows). This one seeds a realistic history for ALL families so
-- every profile's /dashboard/voice "recent commands" rail is populated.
--
-- WHAT IT DOES
--   1. Re-asserts family-scoped RLS on voice_commands (drift-safe, idempotent).
--   2. For EVERY family in public.families, inserts 60 spoken-command log rows
--      with a realistic spread of:
--        • transcript    — natural phrasings ("Remind me to…", "Add … to the
--                           shopping list", "Note that…", "Schedule … Saturday")
--        • resolved_kind — task / note / event / shopping (null on failures)
...
--        • member        — attributed to a real family member (round-robin)
--        • created_at    — spanning ~90 days incl. a cluster today.
--
-- TABLE: public.voice_commands.   SCOPE: ALL families (all profiles).
--
-- IDEMPOTENT: voice_commands has no tag column, so seeded rows use DETERMINISTIC
--   ids — (md5('voiceseed:'||family_id||':'||i))::uuid, unique per family — which
--   are deleted before re-insert. Real commands have random uuids, so re-running
--   only ever replaces this seed's rows and never touches genuine history. It
--   also does NOT collide with seed_voice_one_family.sql (different id salt).
--
-- HOW TO RUN:
--   psql "$SUPABASE_DB_URL" -f supabase/seed_voice_commands_all_families.sql
--   (or paste into the Supabase SQL editor and press Run). Requires migration
--   0121_voice_commands.sql applied first.
--
-- VERIFY: open /dashboard/voice on any profile and hard-refresh — the "recent
--   commands" list fills with mixed kinds + statuses; each row re-runs or deletes.
-- ============================================================================
...
-- 2) Seed 60 voice_commands for every family ----------------------------------
do $$
declare
  f         record;
...
begin
  for f in select id from public.families loop
    ...
    if v_members is null or array_length(v_members, 1) is null then
      continue;  -- family with no active members: nothing to attribute to
    end if;
    nm := array_length(v_members, 1);
    -- created_by: a member's auth user where one exists (else null).
    select user_id into v_uid from public.family_members
      where family_id = f.id and is_active and user_id is not null limit 1;

    -- clear prior seed rows only (deterministic per-family ids), never real history
    delete from public.voice_commands
    where family_id = f.id
      and id in (select (md5('voiceseed:' || f.id::text || ':' || g))::uuid
                 from generate_series(0, per_family - 1) g);

    for i in 0..(per_family - 1) loop
      t := 1 + (i % 5);
      ...
      values
        ((md5('voiceseed:' || f.id::text || ':' || i))::uuid, f.id, v_member, v_transcript, v_kind, v_table, v_count, v_status, v_uid, v_when);
      seeded := seeded + 1;
    end loop;
  end loop;

  raise notice 'Seeded % voice commands across % families.', seeded, fams;
end $$;
...
from public.voice_commands v
where exists (
  select 1 from generate_series(0, 59) g
  where v.id = (md5('voiceseed:' || v.family_id::text || ':' || g))::uuid
);
