...
--
-- FIX INCLUDED:
--   public.family_apps is now protected by to_regclass(), so a missing
--   family_apps migration cannot abort the entire master seed.
--
-- SAFE TO RERUN:
--   Existing idempotency behavior and all original seed sections are preserved.
-- ============================================================================
...
begin
  -- Schema-safe guard: Production may not yet include the migration that
  -- creates public.family_apps. Skipping this section allows every other
  -- independent seed section in SEED_ALL to continue successfully.
  if to_regclass('public.family_apps') is not null then
    ...
    raise notice 'Skipped Family App Store seed: public.family_apps does not exist. Apply its migration, then rerun SEED_ALL.';
  end if;
end $$;
...
-- cascade / are tagged). Requires the anchored account family by email.
-- (Needs migrations 0186 and 0187.)
-- Where: Supabase → SQL Editor → paste → Run.
-- ============================================================================
do $$
declare
...
begin
  ...
  -- Buyer = the next existing member of the same family.
  ...
  if v_buyer is null then
    raise exception 'Marketplace hand-off seed requires two existing members for family %.', v_family;
  end if;

  -- Deterministic IDs make reruns additive and conflict-safe. This embedded copy
  ...
  for i in 0..(n_sets - 1) loop
    ...
    values (md5('familyos-seed-handoff-order-' || i)::uuid, v_family, lot, v_buyer, v_seller, 'buy',
    ...
    cev := null;
    if st = 1 then
      insert into public.calendar_events (id, family_id, title, description, location, category, starts_at, created_by)
      values (md5('familyos-seed-handoff-calendar-' || i)::uuid, v_family, 'Marketplace pickup · ' || loc || ' [seed:handoff]',
