...
The tree compiles, passes lint and type checking, passes 2,960 unit tests, builds all 250 static Next.js
...
gift state, and required audit records commit atomically, with server-side split allocation preserving
every cent.
...
Latest repair increment (2026-07-14): chore XP and badge persistence now fails closed on required read or
...
Latest repair increment (2026-07-14): child PIN failed-attempt counters now fail closed when durable
throttle persistence fails, and child-login provisioning rolls back Auth, member-link, and mapping state
when active-family preference persistence fails.

Latest repair increment (2026-07-14): social publishing now checks target/account reads, variant and
schedule writes, job/target/result/post state transitions, cleans incomplete pre-publish drafts, and
keeps provider exception details server-side. Usage-event writes remain explicitly best-effort telemetry.

Latest repair increment (2026-07-14): social account disconnect, inbox resolution, media creation,
workspace settings, and access grants now check durable mutation results. Access grants require the
manage-access permission, an active member of the current family, and a valid social role; media inputs
are bounded and URL protocols are restricted to HTTP(S).
...
- 385 unit-test files and 2,943 passing tests.
...
| Unit tests | PASS, 2,943 tests / 385 files | `npm.cmd exec vitest run` |
