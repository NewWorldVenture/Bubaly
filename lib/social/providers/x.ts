...
import { isXId, loadXAccessToken, XBoundaryError } from '../account-tokens';
...
export async function publishX(input: ConnectorPublishInput): Promise<ConnectorPublishOutput> {
  ...
  try { accessToken = await loadXAccessToken({ familyId: input.familyId, userId: input.userId }, input.accountId, input.providerAccountId); }
  catch (error) { return fail('account_unavailable', error instanceof XBoundaryError ? error.key : 'socialX.permissionDenied'); }
  // No retries after dispatch: a missing/malformed/late response is not proof of rejection.
  try {
    const response = await xJsonRequest('https://api.x.com/2/tweets', { method: 'POST', headers: { Authorization: `Bearer ${accessToken}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ text }) });
    if (response.status >= 400 && response.status < 500) return fail(response.status === 401 ? 'reconnect_required' : 'provider_rejected', response.status === 401 ? 'socialX.reconnectRequired' : 'socialX.publishRejected');
