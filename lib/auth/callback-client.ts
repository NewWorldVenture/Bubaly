...
export function captureCallbackOwnership(): CallbackOwnership | null {
  try {
    ...
    owners.set(owner, { ...readState(), phase: 'pending', capturedAt: Date.now() });
    return owner;
  } catch { return null; }
}
...
export function isCallbackOwnershipCurrent(owner: CallbackOwnership): boolean {
  try {
    ...
    if (state.key !== current.key || state.generation !== current.generation || canonical(state.verifier) !== canonical(current.verifier)) return false;
...
export async function callbackVerifierFingerprint(owner: CallbackOwnership): Promise<string> {
  const state = owners.get(owner);
  if (!state?.verifier.length || state.phase !== 'pending' || !isCallbackOwnershipCurrent(owner)) throw interrupted();
...
export async function adoptCallbackSession(tokens: CallbackTokens, owner: CallbackOwnership, canCommit: () => boolean): Promise<AuthTokenResponsePassword> {
  const state = owners.get(owner);
  if (!state?.verifier.length || state.phase !== 'pending' || !canCommit() || !isCallbackOwnershipCurrent(owner)) throw interrupted();
  ...
  try {
    ...
    if (readState().verifier.length) throw interrupted();
    state.verifier = [];
