...
export const findVendorTemplate: WorkflowTemplate = {
  ...
  steps: [
    ...
    },
    {
      ...
      input: (ctx) => ({ title: `Book a repair: ${issue(ctx)}`, notes: 'Use the contractor on file if there is one; otherwise pick a provider.', due_date: ctx.tomorrowKey, priority: 'high' }),
    },
    {
    ...
    },
    {
      ...
      input: (ctx) => ({ recipients: 'managers', type: 'maintenance_task', title: `Repair set up: ${issue(ctx)}`, body: 'The maintenance record is open and a task to book the repair is waiting, with the contractor on file if there is one.' }),
    },
  ],
};
