...
export async function loadTrustInputs(supabase: DB, familyId: string): Promise<{
  ...
  const [{ data: policies }, { data: grants }, { data: dels }, { data: emergencies }] = await settleAll([
    ...
    // lib/trust/engine.ts sorts by `priority` alone and `Array.prototype.sort`
    // is stable, so the highest-priority match it picks is whichever
    // equal-priority row arrived first — and a select with no ORDER BY has no
    // defined row order at all. Two policies that tie could therefore swap which
    // one governs between two identical requests. 0302 stops the specific case
    // that made this reachable (two live SYSTEM policies with one name), but a
    // family may still write two hand-made policies at the same priority, and
    // which of those governs should not depend on a query plan.
    //
    // Newest first on the tie — a policy written later is the family's more
    // recent word — with `id` as the final tiebreak, so the answer is the same
    // on every read. This is the same ranking 0302 used to pick its survivor.
