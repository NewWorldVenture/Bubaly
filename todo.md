...
- Commit: `9eb160b0` hides unsupported connection providers from the runtime hub after `14f0d14b` replaced the Independence empty-state roadmap phrase with â€œNo badges yetâ€; live provider and deployment evidence remains open
...
- Scenario: when Stripe Issuing was not configured, the Cards page said â€œcoming soon,â€ while the child wallet rendered Visa-like card art and masked card digits despite no payment card being issued.
...
- Implementation notes: the disabled Cards mode now uses explicit unavailable/configuration copy; the child surface now says â€œNo payment card issued,â€ â€œPreview,â€ and â€œLedger only,â€ with a link to card setup.
...
- Status: `[x]` Completed in code, tested, committed, and pushed to the audit branch; publication to `main` follows this documentation update.
- Severity: P1
- Category: Household operations / Supabase failure handling
- Feature: Announcements, Contacts, Screen Time, Celebrations, and Household Binder
- Route: corresponding family module routes under `/dashboard`
- File or files: `components/modules/announcements-module.tsx`, `components/modules/contacts-module.tsx`, `components/modules/screen-time-module.tsx`, `components/modules/celebrations-module.tsx`, `components/modules/binder-module.tsx`, `tests/household-read-boundaries.test.ts`
- Database objects: `family_announcements`, `announcement_reads`, `family_contacts`, `screen_time_entries`, `screen_time_limits`, `family_dates`, and `household_info`
- Affected roles: authenticated family members and household managers
...
- Required remediation: surface sanitized retryable errors before empty/derived states and refresh every required query together.
- Implementation notes: Announcements coordinates announcements and read receipts; Screen Time coordinates entries and limits; Contacts, Celebrations, and Binder now use shared ErrorState retry behavior.
- Test plan: focused household boundary suite, full Vitest, typecheck, lint, dependency audit, production build, migration/schema probes, and diff check.
- Tests performed: `tests/household-read-boundaries.test.ts` (4 focused assertions); full Vitest; typecheck; lint; clean production build; migration audit; schema probes; diff check.
- Evidence: full local gate passed with 477 files/3,239 tests, 0 production dependency vulnerabilities, and a clean 250-route build.
- Resolution: source repair validated in commit `943990b3`; live provider, RLS, browser, backup, and deployed verification remain open.
- Remaining dependencies: live provider callbacks, cross-family RLS, browser, backup, and deployed household verification.
...
- Resolution: source repair validated locally in commit `712ee9b3`; push and post-push ref verification pending.
- Remaining dependencies: live Super Admin role/browser outage drill and deployed Supabase evidence.
...
- Launch impact: onboarding could complete without a usable tenant/subscription state or mutate another familyâ€™s member presentation
...
- Scenario: channel, family, routing, or inbox persistence reads/writes failed while the UI or a communications callback could continue with null/empty state; inbound email was recently added but had the same unchecked-boundary pattern
- Launch impact: a family could see an unassigned identity, lose messages, or receive provider success responses while the inbox is unavailable
- Root cause: Supabase errors were discarded in channel helpers, page loads, routing lookups, provisioning writes, and inbox persistence; the email callback also parsed unbounded form bodies and hid reply failures
- Resolution: channel/page/routing reads are explicit; page renders ErrorState; SMS, voice, voicemail, and email routing return 503 on database failure; email form parsing is bounded; provisioning and inbox writes are checked; callback escalation failures are logged; focused Contact Center coverage added
- Tests performed: `tests/contact-center.test.ts` (15 focused tests); typecheck; lint; diff check
- Evidence: focused validation passes; final merged-tree gate was 450 files/3,176 tests, dependency audit 0 vulnerabilities, and 250-route build
- Remaining dependencies: run live Twilio/email test-mode callbacks, provider retries, remote RLS, and multi-role browser verification
...
- File or files: `app/(app)/wallet/send/page.tsx`, `app/(app)/wallet/activity/page.tsx`, `app/(app)/wallet/treasury/page.tsx`, `app/(app)/wallet/goals/page.tsx`, `app/(app)/wallet/allowance/page.tsx`, `app/(app)/wallet/cards/page.tsx`, `tests/wallet-read-boundary.test.ts`
...
- Affected roles: household managers, parents, and members with wallet visibility
- Scenario: a failed wallet read looked like an inactive wallet, while failed ledger, bucket, goal, rule, card, connected-account, child-wallet, or member reads silently produced activation prompts, zero balances, incomplete history, fallback names, or incomplete card controls
...
- Resolution: wallet failures now render a retryable error state; dependent failures are logged and surfaced in accessible page-specific data-health warnings across Send Money, Activity, Treasury, Goals, Allowance, and Cards while unaffected data remains visible
...
- Evidence: source commit `2f844cc3`; branch and `main` publication verified; live collection/RLS/role/browser workflow evidence remains open
...
- Evidence: source commit `2f844cc3`; branch and `main` publication verified; live following/RLS/role/browser workflow evidence remains open
...
- Severity: P1
- Category: Reliability / marketplace / discovery integrity
- Feature: Marketplace storefront directory, follows, ratings, and open-listing counts
- Route: `/marketplace/creators`
- File or files: `app/(app)/marketplace/creators/page.tsx`, `tests/marketplace-creators-read-boundary.test.ts`
- Database objects: `marketplace_stores`, `marketplace_follows`, `marketplace_reviews`, and `marketplace_listings`
- Affected roles: authenticated household members and Super Admin marketplace operators
- Scenario: storefront, follow, review, or open-listing reads failed while the page rendered an empty or partially ranked creator directory
- Launch impact: users could miss sellers or make trust decisions from incomplete ratings and inventory context
...
- Remaining dependencies: run isolated operational-read failure/browser drills
...
#### TODO-0282 Ã¢â‚¬â€ Onboarding replay can duplicate household records
...
truth for the build â€” work top to bottom, keep it in sync, and don't mark a feature `DONE`
...
- [ ] **Schema** â€” additive, idempotent migration with family-scoped RLS via `public.is_family_member`, validated on PG16.
- [ ] **Types** â€” hand-maintained rows added to `lib/database.types.ts` (`T<Row,Insert,Update>`).
- [ ] **Server lib** â€” pure/tested logic + Supabase reads/writes (no mock data anywhere in the path).
- [ ] **UI module** â€” production component, optimistic where sensible, undo/confirm on destructive.
- [ ] **Route** â€” `app/(app)/dashboard/<feature>/page.tsx` using `requireUserContext()`.
- [ ] **Nav** â€” entry in `lib/constants/navigation.ts` + Navigation Choices catalog.
- [ ] **Verified** â€” `tsc`, `eslint`, `vitest`, `next build` all green.

> **SEED CONVENTION (2026-07-14):** `supabase/SEED_ALL.sql` is **FROZEN** â€” do not
> append to it. Each feature's seed goes in its own standalone `supabase/seed_<feature>.sql`
> file, **â‰¤ 1000 lines**, as a self-contained idempotent DO-block (resolve the anchored
...
Legend: â˜ open Â· â— partial (scaffolding exists) Â· â˜‘ done

---

## â˜‘ SERVICE CATALOG TOOLTIPS â€” hover blurbs, super-admin editable (SHIPPED 2026-07-14)

The **"All Services"** picker now shows a **hover/focus tooltip** describing what each of the ~115
services offers. Copy defaults are versioned in code (`lib/services/descriptions.ts` â€” a crisp
...
migration PG16-verified idempotent Ã—2. âš ï¸ apply `0203` to prod (see `docs/PENDING_PROD_MIGRATIONS.md`).

---

## â˜‘ BLOG OVERHAUL â€” world-class articles + engagement (SHIPPED 2026-07-14)

Every `/blog` cover-story stub is now a **complete, fun, fully-written article** â€” 20 posts
total (9 stubs fleshed out in place + 11 new), 3â€“4 per category across all 6 categories, each
...
Supabase-wired: **â™¥ likes** (anonymous per-visitor via the durable `bubaly_vid` id, optimistic
...
upsert by slug) â€” both PG16-verified idempotent Ã—2. Routes `/api/blog/like|subscribe|unsubscribe`
...
âš ï¸ apply `0201` + `0202` to prod (see `docs/PENDING_PROD_MIGRATIONS.md`).

---

## ðŸ”’ SECURITY AUDIT â€” status & remaining items (2026-07-10) â€” READ FIRST
...
AI-1, PAY-4/5, AI-2 in round 2 â€” this session). The only remaining action is human-owned: **apply the
coupled migrations `0154`/`0155`/`0156` to prod with the deploy.** **Do not** re-audit from scratch.

**âœ… Fixed & merged**
- **Marketplace object-level authz** (SEC-1/SEC-2/REL-1/REL-3/RACE-1/RACE-2, A11Y, img) â€” PR #281 â†’
...
- **PAY-1 â€” card-authorization overspend** (concurrent auths approved against the same balance).
  Fixed: atomic `wallet_reserve_card_auth` RPC (per-child lock + sufficient-funds check + `processing`
  hold keyed by auth id) â†’ migration **`0155_wallet_auth_holds.sql`**; holds released on capture /
  reversal in `lib/stripe/webhook.ts`. PG16-verified (2nd concurrent auth declined; capture reconciles).
- **PAY-2 â€” dropped financial events**: money webhook recorded events *before* handling then 200'd on
  error, so a failed capture was never retried. Now `recordEvent` is replay-safe (processingâ†’processed;
  errored/unfinished events reprocessable) and the route returns 500 on handler error so Stripe retries.

**âš ï¸ Coupled prod migrations â€” apply with the deployed code** (see `docs/PENDING_PROD_MIGRATIONS.md`):
...
**âœ… Fixed & merged â€” round 2 (2026-07-10, this session)** â€” all remaining audit items closed:
- **AUTH-1 (High)** âœ… â€” Google Calendar OAuth is now CSRF-safe: `auth/route.ts` issues a random,
...
- **PAY-3 (Med)** âœ… â€” `app/api/billing/checkout/route.ts` now gates on `isAdmin(ctx.active.role)`
  (only a parent can start a subscription), matching change-plan/cancel.
- **AI-1 (Med)** âœ… â€” the scam classifier (`lib/guardian/scam-ai.ts`) is prompt-injection-hardened:
  ...
  **random-nonce fence** the model is told to treat strictly as data (injection attempts = a scam
  ...
  emit an off-list recommendation/scamType/confidence â€” it only informs the pipeline. `ai-screen.ts`
  got the same defense-in-depth rule. Test `tests/guardian-scam-ai.test.ts` (6) proves the validator.
- **PAY-4 (Low)** âœ… â€” `/api/webhooks/stripe` (subscription webhook) now dedups by Stripe event id via
  ...
  was already idempotent â€” only acts on `status = 'signed_up'`.)
- **PAY-5 (Low)** âœ… â€” billing success/return URLs now build from the trusted `NEXT_PUBLIC_APP_URL`
  first, not the caller-controlled `Origin` header (checkout, change-plan, portal).
- **AI-2 (Low)** âœ… â€” durable, cross-instance rate limiting: **`0156_rate_limits.sql`**
...
- Verified: tsc Â· eslint Â· **vitest (56 existing + 6 new)** Â· `next build` (218 pages); `0156`
...
## ðŸ‘¤ VISITOR INTELLIGENCE / LEAD CAPTURE â€” coverage matrix (2026-07-11)

Audited the "privacy-first visitor intelligence + lead capture" brief against the codebase. **Most of
it already exists**; the one central gap (consent) is now closed. Honest status below â€” **not** a
17-phase greenfield build. Pick up the â˜ items directly; don't rebuild the â˜‘ ones.

| Capability | Status | Where |
|---|---|---|
| Anonymous visitor spine (CDP) | â˜‘ exists | `mkt_visitors` (0058) + `/api/mkt/track` |
| Session + attribution (source/medium/campaign/landing) | â˜‘ exists | `mkt_sessions`, `mkt_touchpoints` (0058) |
| Multi-touch + conversion attribution | â˜‘ exists | `mkt_touchpoints.kind` |
| CRM / marketing contacts | â˜‘ exists | `crm_contacts`, `crm_deals`, marketing_* |
| Lead capture forms + submissions | â˜‘ exists | `marketing_forms`, `marketing_form_submissions`, `/api/forms/submit` |
| Segments / suppressions | â˜‘ exists | `marketing_segments`, `marketing_suppressions` |
| Exit-intent | â˜‘ exists | `marketing_exit_intent`, `components/marketing/exit-intent.tsx`, `/api/exit-intent/*` |
| Personalization rules | â˜‘ exists | `marketing_personalization_rules`, `lib/marketing/personalization.ts` |
| A/B experiments | â˜‘ exists | `ab_experiments`, `ab_events`, `lib/marketing/ab.ts`, `/api/ab/track` |
| Admin marketing intelligence page | â˜‘ **NEW** | `/admin/marketing/intelligence` + a dedicated **`/admin/marketing/visitor-intelligence`** funnel dashboard (visitors â†’ identified â†’ profiled â†’ scored â†’ hot/qualified) with consent posture + lead-band distribution; `lib/marketing/visitor-funnel.ts` (6 tests) |
| **Granular consent (necessary/analytics/personalization/marketing) + GPC + revocation** | â˜‘ **NEW** | `mkt_consent_events` (0160), `lib/marketing/consent.ts`, `/api/mkt/consent` |
| **Consent-gated tracking** | â˜‘ **NEW** | `/api/mkt/track` gates on analytics consent |
| Identity linking (anon â†’ contact) | â˜‘ **NEW** | `lib/marketing/identity.ts` â€” on signup/login stitches `mkt_visitors.contact_id` â†’ `crm_contacts` (dedup by email, non-downgrading), carries consent forward (`mkt_consent_events.contact_id`), forks on shared-device 2nd user (rotates anon id). Hooked from login/signup forms + `auth/callback`. 6 tests (`identity-stitch.test.ts`) |
| Progressive profiling UI | â˜‘ **NEW** | `crm_contact_profile` (0171) + `lib/marketing/progressive-profile.ts` (one-question-at-a-time engine, 10 tests) + `ProfileNudge` card on `/dashboard/settings`; saves role/priority/household/kids/interests, remembers skips |
| Consent banner / preference-center UI | â˜‘ **NEW** | `components/marketing/consent-manager.tsx` â€” banner (Accept all / Reject / Manage) + granular preference center wired to `/api/mkt/consent`; anon-id + GPC + local cache in `lib/marketing/visitor.ts`; fires the analytics touch to `/api/mkt/track`; re-openable from the footer |
| **Client visitor spine (anon-id + first-party touch)** | â˜‘ **NEW** | `lib/marketing/visitor.ts` â€” durable `bubaly_vid` cookie/localStorage; GPC detect; one consent-gated `/api/mkt/track` touch per session (was: endpoints had no client caller) |
| Lead scoring | â˜‘ **NEW** | `crm_lead_scores` (0172) + `lib/marketing/contact-score.ts` (0â€“100 score + itemized ledger, 7 tests) + `contact-score-compute.ts` (gathers sessions/recency/conversions/demo/consent/profile/lifecycle); admin `/admin/marketing/lead-scores` â€” ranked, expandable "why" ledger, Recompute |
| Abandoned-journey recovery | â˜‘ **NEW** | beyond checkout: `/api/cron/journey-recovery` sweeps stalled onboarding (`onboarding_abandoned`) + non-converting demo leads (`demo_abandoned`) and fires the follow-up workflow; `lib/marketing/journey-recovery.ts` (windowed selection, 8 tests) + 2 new automation triggers/copy |

**â˜ Next (privacy-safe, priority order)**
- â˜‘ **DONE (2026-07-12)** â€” Client **consent banner + preference center** (`consent-manager.tsx` +
...
- â˜‘ **DONE (2026-07-12)** â€” Identity stitch on signup/login (`lib/marketing/identity.ts`): sets
...
- â˜‘ **DONE (2026-07-12)** â€” Progressive-profiling capture, one field at a time (`crm_contact_profile`
  0171 + `progressive-profile.ts` engine + `ProfileNudge` on `/dashboard/settings`). âš ï¸ apply `0171`.
- â˜‘ **DONE (2026-07-12)** â€” Dedicated **`/admin/marketing/visitor-intelligence`** funnel dashboard
  (acquisition funnel + consent posture + lead-score bands), `visitor-funnel.ts` engine (6 tests).
  **The visitor-intelligence / lead-capture lane is now fully â˜‘.**

**Privacy invariants (already enforced â€” keep them):** no fingerprinting; no PII from anonymous
...
versioned, revocable (`mkt_consent_events`). **âš ï¸ Apply `0160_visitor_consent.sql` to prod with the deploy.**

**â˜‘ NEW (2026-07-11) â€” Demo email gate + upgrade exit (lead capture on the demo):** the one-click
**Test Account** demo now opens behind a **blurred email-capture pop-up**; the 5-minute clock only
starts once the visitor enters an email (stamped on `demo_sessions.email` â€” an identified-lead
signal). When the clock runs out the app blurs again behind an **upgrade pop-up** (Free 5-day /
Family Basic / Family+ â†’ routes into signup). Files: `components/demo/demo-experience.tsx`,
`app/(marketing)/demo/actions.ts` (`startDemoClockAction`, `choosePlanAfterDemoAction`),
`lib/demo/session.ts` (deferred clock + abandoned-session reaping). **âš ï¸ Apply
`0161_demo_session_email_gate.sql` to prod with the deploy** (nullable `expires_at` + `email`).
Follow-up (â—): stitch the captured demo email into `crm_contacts` / `mkt_visitors` for real lead flow.

**â˜‘ NEW (2026-07-11) â€” Demo is now ONE shared "Bubaly Demo" account (supersedes the throwaway-user
...
- **Single account, not per-visitor users.** `lib/demo/session.ts` rewritten: `ensureDemoAccount()`
  ...
  owning member + active-family pref set on first run only. **No new migration** â€” reuses the existing
...
- **Seed expanded to ~200 rows.** `lib/demo/seed.ts` now populates every Family+ surface (calendar 24 Â·
  transactions 24 Â· bills 12 Â· reminders 14 Â· pantry 20 Â· goals 10 Â· documents 10 Â· maintenance 8 Â·
  family_facts 12 Â· budgets/accounts Â· meals+plans+votes Â· groceries Â· chores Â· polls Â· marketplace
  listings+saves+reviews Â· autopilot/approvals/agent activity). Column choices mirror the PG16-validated
  `seed_demo_account.sql`; every insert best-effort (schema-drift safe). *(Previously the one-click demo
  seeded only ~30 rows â†’ empty Home. Fixed.)*
- **No teardown.** `endDemoSession()` no longer deletes anything â€” it only clears the `demo_sessions`
  row (`expires_at`/`email` â†’ null) so the next visitor gets a fresh email gate; the cron
  (`/api/cron/demo-cleanup` â†’ `cleanupExpiredDemoSessions`) does the same for expired clocks. Sign-in
  rotates a random password per login with a one-shot retry (`rotateDemoPassword`) to absorb a
  concurrent-login race â€” **no stored secret**.
...
- **Pricing card** (`app/(marketing)/pricing/pricing-content.tsx`): renamed **"Test Account" â†’
  "Demo Account"**, dropped the inaccurate **"No email"** copy â†’ *"No card needed â€” logs you straight
...
- âš ï¸ **Left-over cleanup (owner, optional):** old ephemeral `demo-<token>@demo.bubaly.app` auth users
  from the previous model may still linger in Supabase Auth â€” harmless, bulk-deletable.
- âš ï¸ **Coordination note for other bots:** a parallel session has an **unmerged branch** reworking the
  pricing hero layout (`pricing-content.tsx`). If it lands it will likely conflict with `dc02676` on
  that file â€” reconcile carefully (keep the "Demo Account" copy + card-left-of-hero layout).

**â˜‘ NEW (2026-07-11) â€” Demo DEEP DIVE: seed expanded to ~350 rows + two silent-empty surfaces fixed
...
- **BUG (fixed): Tasks + Groceries surfaces were silently EMPTY.** `todo_items`/`grocery_items` require
  ...
  silently â†’ two empty modules for every demo visitor. Now seed a parent `todo_lists` / `grocery_lists`
  first and stamp `list_id`; `todo_lists.created_by` correctly targets `family_members.id` (not
  `auth.users`) via the owning member.
- **Coverage went from ~30 â†’ ~52 tables (~350 rows).** Added health (medications Â· appointments Â·
  health_visits Â· immunizations), kids/school (homework Â· classes Â· teams Â· wishlist Â· screen_time Â·
  journal), home (pets Â· vehicles Â· home_warranties), trips (vacations Â· trips), relationship_dates,
...
**â˜‘ NEW (2026-07-11) â€” Payment is now ONE TAP from the demo (account creation + payment super easy).**
The demoâ†’upgradeâ†’signupâ†’billing path used to **drop the plan the visitor already chose**: picking
...
- `choosePlanAfterDemoAction` builds `redirect=/dashboard/billing?view=manage&checkout=<plan>` (was
  just `view=manage`), so the plan survives signup (works across email Â· phone Â· Google Â· Apple â€” the
...
- `BillingModule` reads `?checkout=basic|plus`: once the auto-provisioned family + subscription load and
  the family is still on Free, it fires `changePlan(<tier>_monthly)` **once** (admin-only) â†’ Stripe
  Checkout opens straight away. New accounts get a family auto-provisioned by `requireUserContext`, so
  the order signupâ†’billingâ†’Stripe just works. Falls back to a highlighted, scrolled-to plan card if
  auto-checkout can't run (non-admin / already subscribed). tsc + eslint clean.

**â˜‘ NEW (2026-07-11) â€” Demo deep-dive pass 2: five more surfaces that read live tables but had no seed.**
...
- **chore_assignments** â€” chores are now *assigned to kids* with a status spread (todoâ†’approved), so the
  Chores board shows who's doing what (was: chores existed but looked unclaimed). `chores` insert made
  capturing to get ids.
- **savings_goals** â€” 5 goals so Finances â†’ **Savings** tab isn't empty (distinct from the Goals module).
- **routine_templates** (+ **routine_template_items**) â€” Morning & Bedtime routines with 4 steps each, so
  the Routines panel renders a real day.
- **family_memories** (6) + **family_milestones** (4) â€” the grandparent-portal / planning memory surfaces.
...
## ðŸ“º KITCHEN DISPLAY V2 â€” ECHO-SHOW-GRADE (Amazon look & feel) â€” DONE (2026-07-14)

Owner ask: take `/display` industry-leading â€” Amazon look and feel, seed data built in, better than
Skylight/Hearth. Built on top of the v1 ambient shell (below):

- **Kitchen Timers widget** (`components/display/kitchen-timers.tsx`) â€” the feature wall-calendar
  competitors lack: multiple concurrent countdowns from one-tap presets (eggs/pasta/rice/pizza/tea/
  homeworkâ€¦) or custom minutes, Web-Audio chime + flashing card on done, progress fill, localStorage
...
- **Idle photo frame** (`photo-frame.tsx`): after `settings.idleMinutes` (Off/2/5/10, default 5) with no
  interaction the screen fades to a full-bleed family-photo slideshow with the big clock + "Next: â€¦"
...
- **Echo-style hints ticker** (`hints-ticker.tsx` + pure `buildHints` in the engine): a rotating bottom
  pill â€” "ðŸ“… Next: Piano Â· in 25 min", "ðŸ½ï¸ Dinner tonight: Chicken Tacos", "ðŸ›’ 12 items on the grocery
  list", "ðŸŽ‚ Sarah's birthday Jul 20" â€” crossfading every 8s, evergreen tips when there's nothing due.
- **Engine** additions (+8 tests â†’ 31): `TIMER_PRESETS`, `formatDuration`, `buildHints`, settings keys
  `background`/`idleMinutes` (normalized, validated). No new migration â€” the 0200 `settings` blob holds
  the new keys.
- **Seed** `[seed:display]` in `SEED_ALL.sql` â€” **NOW-RELATIVE** (current_date-based) so the display is
  alive whenever it's run: 6 curated events today (dentist 10am, piano 1pm, soccer 4:30â€¦) + 200 upcoming,
  ...
  photo bg + frame) â‰ˆ 502 rows. PG16-verified Ã—2 (idempotent), counts asserted.
- **Verified** tsc 0 Â· eslint 0 Â· vitest 2858 green Â· `next build` green (`/display` 16.6 kB).
...
> `lib/display/tiles.ts` (`tileRowSpan` Â· `isCompactTile` Â· `tileListLimit`, tested) map each size to its
> vertical room. `WeatherTile` now takes a `size`: a compact (1-row) tile shows just place + temp +
> condition (which is all that fits â€” the forecast strip was the piece clipping), while md/lg/hero add
...
> is ever hard-clipped regardless of size or data. +4 tests; 2996 green; tscÂ·eslintÂ·build clean.

## ðŸ“º KITCHEN DISPLAY â€” WORLD-CLASS BUILD-OUT â€” DONE (2026-07-14)

Owner ask: fully build out the Amazon/Echo-Show-style Kitchen Display (`/display`) â€” make it world-class.

- **Migration** `0200_display_settings.sql` (PG16-verified, idempotent Ã—2): adds `settings jsonb` to
  `display_layouts` so per-family display prefs (12/24h clock, seconds, Â°F/Â°C, background theme, ambient
...
- **Pure engine** `lib/display/ambient.ts` (+24 tests): day-part detection, day-part greeting, time-of-day
  ...
  12/24h clock formatter, temperature formatting (C-in and Â°F-native), settings normalization.
- **UI** â€” reworked the display into an always-on kitchen screen: ambient time-of-day background wash +
  ...
  location); rewrote `components/display/display-grid.tsx` â†’ `DisplayShell`; `app/(app)/display/page.tsx`
  now fetches up to 6 recipes for the rotating hero and passes settings. Weather is Â°F-native (open-meteo);
  the Â°C toggle converts.
- **Verified** tsc 0 Â· eslint 0 Â· vitest 2826 green Â· `next build` green (`/display` 13.7 kB). Migration doc
  row added (0200); next free number â†’ **0203** (0201/0202 = blog engagement + articles).

## ðŸ’¡ FEEDBACK / IDEA BOARD â€” DONE (2026-07-14)
...
> search filters, and per-idea actions â€” move it through the roadmap (**status + public note** in one
...
> (`feedbackAdminSummary` + `filterIdeasForAdmin`, **6 tests**); tscÂ·eslintÂ·2906 testsÂ·build green. No
> new migration (reuses 0197).

> **+ Founder growth alerts SHIPPED (2026-07-14).** The admin bell now also fires on the two events a
> founder most wants: a **new family completing onboarding** (`finalizeOnboardingAction` â†’ `family_signup`,
> only for freshly-created families) and a **new paid conversion** (Stripe webhook â†’ `subscription`,
> detected by the pure tested `isNewPaidConversion` â€” fires once on trial/free â†’ paid+active, never on
> renewals). Both best-effort/service-role. Migration **`0209_admin_growth_kinds.sql`** widens the kind
> CHECK (PG16 Ã—2). `lib/billing/conversion.ts` (**8 tests**); 3018 tests + build green. âš ï¸ apply `0209`.
...
> `/admin/*` page â€” new feedback + bugs, GitHub sync relays, **new support tickets** (contact form), and
> **marketplace Trust & Safety reports** â€” with an unread badge, a deep-linked dropdown, mark-all-read,
...
> **`0207_admin_notification_kinds.sql`** widens the kind CHECK (PG16 Ã—2). Pure `lib/admin/notifications.ts`
> (kind meta Â· unreadCount Â· badgeText, **4 tests**); 3011 tests + build green. âš ï¸ apply `0207`. (Also
> fixed the stale "next free number" note in `docs/PENDING_PROD_MIGRATIONS.md` â†’ 0208 and documented the
> GitHub env keys.)
...
> **notifies the super admin** â€” an `admin_notifications` platform feed on `/admin/feedback` **plus email**
> to all super-admin emails â€” and (b) is **mirrored to a GitHub issue tracker** with two label-driven
...
> each issue's open/closed + workflow labels back onto the idea's status (closedâ†’shipped, not-plannedâ†’
> declined, "in progress"/"planned" labelsâ†’status), and **relays a digest** to the super admin. Admin
...
> (`feedback_ideas.kind` + github_* columns; `admin_notifications`, service-role RLS; PG16 Ã—2). Pure
...
> (`GITHUB_TOKEN` + `GITHUB_FEEDBACK_REPO`) and ships dark â€” submissions still notify the super admin
> without it. 3006 tests + build green; tscÂ·eslint clean. âš ï¸ apply `0206` + set the GitHub keys (see
...
"Let's make life easierâ€”together" idea-board design 100%, fully Supabase-wired and production-ready.

- **Schema** `0197_feedback_ideas.sql` (PG16-verified, idempotent Ã—2): `feedback_ideas` + `feedback_votes`
  ...
  delete-your-own vote; **no public UPDATE on ideas** â€” status is super-admin (service-role) only. Public
...
- **Route** `app/(app)/feedback/page.tsx` (`requireUserContext`) â€” hero + winding-path motif, 3 mini-steps,
...
- **Seed** `SEED_ALL.sql` â€” **500** ideas + votes + comments (marker `[seed:feedback]`), decayed popularity,
  all statuses, anchored-user votes + team replies. PG16-verified, idempotent.
- **Verified** tsc 0 Â· eslint 0 Â· vitest 2794 green Â· `next build` green (`/feedback` 8.61 kB). Migration
  doc row added (0197); next free number â†’ **0198**.

## ðŸš€ INDUSTRY-FIRST CAPABILITIES â€” gap matrix vs. the codebase (owner directive, 2026-07-12)
...
Audited the routes â€” **9 of 10 already have a real surface**; depth varies. **The one true greenfield
...
|---|---|---|---|
| 1 | AI **completes** life administration (not just reminds) | â— partial | `/dashboard/agents`, `/dashboard/concierge`, `/dashboard/decisions`, `/dashboard/prep-plans`, `agent_activity`. Autonomous *execution* (vs. surfacing) is the deepen target. |
| 2 | **One phone # + one family email** managed by AI | â˜‘ exists | `/dashboard/front-desk`, `app/api/concierge-calls`, guardian inbound SMS/voice/WhatsApp (Twilio-verified), `/dashboard/inbox`. |
| 3 | AI **negotiates** appointments / bookings / schedule changes | â— partial | Concierge surface exists; true agentic negotiation loop is the deepen target (needs the outbound-call/agent tooling). |
| 4 | AI handles **forms / paperwork / insurance / school packets / registrations** | â˜‘ **DONE** (2026-07-12) | **Paperwork Inbox** `/dashboard/paperwork` (`0169_paperwork_items`, RLS): capture â†’ **triage** (`lib/paperwork/triage.ts`, pure, 9 tests: kind/dates/amount/actions/urgency) â†’ **one-tap materialize** each action into a real `calendar_events` / `family_reminders` row (idempotent, auditable) â†’ status lifecycle. Mobile-first module + composer; wired into front-desk + inbox; seed `seed_paperwork.sql` (500). **NEW: AI draft-reply** â€” `draftPaperworkReplyAction` (key-gated via `isAIConfigured`, grounded only in captured text, honest fallback) drafts a ready-to-send reply the parent can copy/edit ("AI fills it out for you"), stored on `meta.draft_reply`. |
| 5 | Unified **household CRM** for every relationship (schools, doctors, contractors, clubs) | â— partial | `/dashboard/connections` (`family_connections`), `family_contacts`, `/dashboard/relationship`. Not yet a first-class relationship-CRM with per-entity timelines. |
| 6 | **Verified buy/sell/borrow/rent marketplace** in family workflows | â˜‘ **DONE** | `/marketplace` â€” hardened + world-class this session (photos, item detail, storefronts, trust, offers, RLS ownership). |
| 7 | **AI family chief of staff** coordinating specialized AI agents | â˜‘ exists | `/dashboard/agents`, chief-of-staff via `/dashboard/graph` + reasoning, `agent_activity` orchestration. |
| 8 | **Household financial copilot** integrated with scheduling + life events | â— partial | Wallet/Finances (`/dashboard/billing?view=manage`), `app/api/ai/wallet`, `life_event_plans`. Tighter scheduleâ†”moneyâ†”life-event linkage is the deepen target. |
| 9 | **Predictive family planning** â€” identify problems before they occur | â˜‘ exists | `/dashboard/family-signals`, `daily_insights`, `/dashboard/family-digital-twin`, `/dashboard/prep-plans`. |
| 10 | Open **"Family App Store"** for AI-powered extensions | â˜‘ **DONE** (2026-07-12) | `/dashboard/app-store` â€” catalog (`family_apps`) + `family_app_installs` (RLS), `lib/appstore/catalog.ts` (+8 tests), install/uninstall/toggle actions, mobile-first grid + "Recommended for your family" rail + Installed section. Migration `0165`, seed `seed_family_apps.sql` (500 apps, 12 categories) in `SEED_ALL.sql`. PG16-verified. **Nav:** not added to the global sidebar (standing rule â€” owner to add `/dashboard/app-store` if desired). |

**â–¶ #10 Family App Store â€” build spec (start here, next session):**
...
- **Route** `/dashboard/app-store` (browse catalog, categories, install/uninstall, "installed" tab) â€” mobile-first,
...
- **Seed** `seed_family_apps.sql` â€” **500** catalog entries across categories (calendar, meals, chores, school,
  sports, health, finance, travel, safety, AI-agentsâ€¦), each with real metadata so the store renders at volume;
  add to `SEED_ALL.sql`. PG16-verify.
- Verify gate + ship. Then loop back to deepen the â— partials (execution loop for #1/#3, paperwork inbox #4,
  relationship-CRM timelines #5, scheduleâ†”money linkage #8).

---

## â˜… NORTH STAR â€” The Family Operating Layer (category-defining, 2026-07-05)

> **Thesis:** today's products (ours included, so far) are **systems of record** â€” they
...
> active users â€” it's **hours of coordination/decision effort removed**. Don't compete
> feature-for-feature; define the category others chase.

---

## â˜…â˜… STRATEGIC ALIGNMENT â€” MOATS OVER FEATURES (owner directive, 2026-07-07) â€” READ FIRST

> **Owner directive (verbatim intent):** *"I would not pursue more features next. I would pursue
> platform moats. Anyone can copy features in 6â€“18 months; it's much harder to copy the underlying
...
> feature matrix already **out-covers Cozi / FamilyWall / OurHome** on nearly every row â€” more
...
1. **Own the data model** â€” the Family Knowledge Graph (the brain).
2. **Own the intelligence layer** â€” the Family Reasoning Engine.
3. **Own the experience** â€” intent-based, AI-first journeys ("one assistant coordinates the rest").
4. **Own the ecosystem** â€” orchestrate the services families already use (don't duplicate them).
5. **Own the category** â€” measure success in **reduced mental load / time saved**, not app time.

### â–£ Honest audit â€” what's BUILT vs. the real GAP (as of 2026-07-07)
...
| **2. Reasoning Engine** | FOI + orchestrator questions, agents roster, decisions, prep-plans, autopilot â€” good *engines*, individually tested. | **No single reasoning engine.** ~8 engines each answer part of "what matters / what's likely forgotten / highest-impact / auto-completable / who needs help / what next" from their own inputs. Consolidate into ONE `lib/reasoning/*` core that reads the graph + snapshot and every surface consumes. |
| **3. Experience (one assistant)** | 26 AI/reasoning routes: assistant, agents, concierge, command-center, calm, graph, decisions, prep-plans, intelligence, outcomes, operating-index, moments, briefing, autopilot, front-desk, inbox, readiness, next-best-actions, weekly-briefing, family-ai-assistant, family-digital-twin, family-memory, knowledge, family-knowledge-graph, playbookâ€¦ | **This is the biggest misalignment.** The strategy is *"users interact with ONE assistant; it coordinates the rest."* We shipped 26 separate destinations â€” with **duplicates** (two knowledge-graphs, two assistants, two memory pages). Consolidate into **one Chief-of-Staff front door**; demote the rest to internal capabilities it calls. |
| **4. Ecosystem** | `0128` connections hub, 11-provider registry, Connect/Disconnect CRUD, `/dashboard/connections`. | Hub + model are ready but **no live OAuth/token exchange or two-way sync** â€” it's a directory, not an orchestrator. Needs provider keys (owner) + per-provider sync adapters (agent). |
...
| **Moments** | `lib/moments/*` engines + `/dashboard/moments`. | Moments is **one page among 70 modules**, not the organizing principle. Strategy: *"organize by moments, not modules"* (Morning Â· School Â· Dinner Â· Vacation Â· Birthday Â· Moving Â· Holiday Â· New Baby Â· Graduation Â· Emergency Â· Weekend). Elevate to a first-class organizing layer. |
| **Chief of Staff** | Autopilot (â‰¥90%-conf auto-exec), Calm digest, agents. | âœ… **CLOSED (2026-07-12, `f03500f`)** â€” Home now IS the assembled front door: greeting â†’ AskBar â†’ FrontDoorHero with **one-tap Approve/Decline in place** (canonical `decideApprovalAction`: multi-approver, audit-logged, auto-executes approved payloads; manager-only, optimistic) and "done for you" merging autopilot auto-executions (undoable) **with specialist-agent completions** (`mergeHandled`, pure + tested) â†’ time-saved metric â†’ time-of-day focus. |

### â–¶ THE REALIGNMENT BACKLOG (do these INSTEAD of new features â€” priority order)

**â—† LIVE STATUS ROLL-UP (updated 2026-07-09) â€” 22 of 22 backlog items shipped. R9's buildable-now slice (the adapter contract) is done; live Microsoft/Apple sync flips on when the owner provisions provider OAuth keys (B3).**

| # | Item | Phase | Status | Migration | Seed |
|---|------|-------|--------|-----------|------|
| R1 | `lib/reasoning/context.ts` â€” one graph-backed context loader | P1 | âœ… Shipped | â€” | `seed_reasoning_context.sql` |
| R2 | Re-point existing engines at the graph | P1 | âœ… Shipped | â€” | â€” |
| R3 | Auto-maintain the graph on-dirty | P1 | âœ… Shipped | `0134` | `seed_model_dirty.sql` |
| R4 | Consolidate 26 surfaces â†’ one Chief-of-Staff home | P2 | âœ… Shipped | â€” | â€” |
| R5 | The proactive front door | P2 | âœ… Shipped | â€” | `seed_front_door.sql` |
| R6 | Intent-based entry (Home AskBar) | P2 | âœ… Shipped | â€” | â€” |
| R7 | **Unify the reasoning engine (one core, six Qs)** | P3 | âœ… **Shipped 07-08** | `0149` | `seed_reasoning_snapshots.sql` |
| R8 | Deepen twin simulation (full activity projection) | P3 | âœ… Shipped 07-08 | `0147` | `seed_twin_simulations.sql` |
| R9 | Per-provider sync adapters (OAuth two-way) | P4 | âœ… **Contract shipped 07-09** (live sync key-gated, B3) | â€” | `seed_sync_microsoft.sql` |
| R10 | Family Intelligence â€” the hard signals | X-cut | âœ… Shipped | `0142` | `seed_family_signals.sql` |
| R11 | The category metric â€” "time saved / mental load" | X-cut | âœ… Shipped | â€” | `seed_time_saved.sql` |
| R12 | Moments as an organizing layer | X-cut | âœ… Shipped 07-08 | `0148` | `seed_moment_activations.sql` |
| T1 | Value-first onboarding re-sequence | P1 | âœ… Shipped | `0138` | (import-driven) |
| T2 | First-run "instant briefing" builder | P1 | âœ… Shipped | `0139` | `seed_meal_ideas.sql` |
| T3 | New-family home = outcome, never empty | P1 | âœ… Shipped | `0140` | `seed_home_briefs.sql` |
| T4 | Insight-of-the-day | P1 | âœ… Shipped | `0141` | `seed_daily_insights.sql` |
| T5 | Partner-tone pass | P1 | âœ… Shipped | â€” | `seed_notifications.sql` |
| T6 | AI-facilitated group decisions | P2 | âœ… Shipped | `0142*` | `seed_group_decisions_one_family.sql` |
| T7 | "Why this?" everywhere | P2 | âœ… Shipped | `0143` | `seed_ai_feedback_one_family.sql` |
| T8 | Premium-consistency sweep (Experience Scorecard) | P2 | âœ… Shipped | `0144` | `seed_experience_audits_one_family.sql` |
| T9 | "What Bubaly has learned" + life-event templates | P3 | âœ… Shipped | `0145` | `seed_life_events_one_family.sql` |
| T10 | TTFV metric (signupâ†’first outcome) | Instr. | âœ… Shipped | `0146` | `seed_activation_events.sql` |

> **All 22 backlog items are now built.** R9's engineering slice â€” the provider-agnostic adapter
> contract, the generic two-way engine, the Google + Microsoft adapters, the registry, and the
> `/api/sync/run` wiring â€” shipped 07-09. What remains is **owner-only**, not code: provisioning the
> per-provider OAuth client keys/secrets (see Â§B / decision B3) flips live Microsoft (and, once its
> adapter drops in, Apple/CalDAV) sync on. All migrations `0138â€“0149` are âš ï¸ **pending apply to prod**
> â€” see `docs/PENDING_PROD_MIGRATIONS.md`.

**P1 â€” Make the Knowledge Graph the brain (Phase 1; 6â€“12mo moat).**
- [x] **R1. `lib/reasoning/context.ts`** âœ… â€” one graph-backed context loader every AI surface calls.
  `loadFamilyContext(supabase, familyId)` â†’ graph (entities + edges) + live household snapshot +
  operating index, assembled by a pure, DB-free core (`assembleFamilyContext` + `entityForRow` rowâ†’node
...
- [x] **R2. Re-point existing engines at the graph** âœ… â€” FOI, Concierge, Briefing, Playbook, Calm,
  Decisions, Prep-Plans, Outcomes read graph relationships (Emmaâ†’Soccerâ†’Fieldâ†’Weatherâ†’Dinner) instead
  of isolated `.from()` calls. No new tables; rewire reads. Shipped one engine at a time behind tests.
  **Started:** pure `lib/reasoning/insights.ts::reasoningInsights(ctx)` (hub / ripple = graph Ã— snapshot /
  coverage; **4 tests**) built on R1's `FamilyContext`, and **Calm re-pointed** â€” the one prioritized
  ...
  **+ Agents / Chief of Staff re-pointed** onto the SAME shared engine (`loadFamilyContext` +
  ...
  graphÃ—snapshot ripple insight. Two engines now share one relationship-reasoning core.
  **+ Daily Briefing re-pointed** â€” the morning briefing renders a graph-backed "Relationships"
  section (`components/reasoning/relationship-insights.tsx`, server-computed via `reasoningInsights`,
  passed into `BriefingModule` like `recap`). **+ Concierge + Outcomes re-pointed** â€” both server
  ...
  `graphReasoningInsights(graph, band)` â€” a lean second entry point that reuses FOI's already-computed
  band (no double `buildSnapshot`); `reasoningInsights(ctx)` and it share one rule set. **DONE: all 9
  surfaces** reason over the graph through one core (Calm Â· Chief of Staff Â· Briefing Â· Concierge Â·
  Outcomes Â· Decisions Â· Prep-Plans Â· Playbook Â· FOI). Next: **R3** (auto-maintain the graph on-dirty).
  **+ Fragility / bus-factor insight âœ… SHIPPED (2026-07-14).** New pure graph capability
  `soleDependencies(index)` in `lib/graph/reason.ts` â€” detects when ONE person is the *only* connector
  ...
  severity when the week is stretched/overloaded: *"Mom is the only backup for 4 things â€” if she's
  ...
  (soleDependencies bus-factor + fragility insight + threshold/severity); 2770 tests green; tscÂ·eslint.
- [x] **R3. Auto-maintain the graph** âœ… â€” the `family_model_dirty` trigger (`0134`) flags changes;
  the graph now re-projects itself with no manual "Rebuild". Two layers: (1) the `model-refresh` cron
  already re-projects dirty families twice daily (backstop); (2) **new on-read auto-refresh** â€” every
  ...
  test-pure). Seed `seed_model_dirty.sql` (500 `family_places` â†’ fires the trigger â†’ materialize on next
  read). tsc/eslint/**2065 tests**/build green.

**P2 â€” One assistant / AI Operating Layer (Phase 2; 6â€“12mo, category-defining).**
- [x] **R4. Consolidate the 26 surfaces into ONE Chief-of-Staff home** âœ… â€” a single assistant that
  ...
  **De-dup DONE:** `family-ai-assistant` â†’ `assistant` (literal dupe); `family-knowledge-graph` â†’
  `graph` (second view of the same graph tables); `family-memory` â†’ `memories` (its family_memories
  journal + milestones already surface on grandparent-portal/planning, so non-lossy). All removed from
  nav / feature-catalog / service-categories. `knowledge` (family_facts) intentionally kept â€” it's
  ...
  demoting specialist reasoning surfaces from top-level nav is **owner-settled** (Â§A: default rail
  stays curated; specialists live in All Services + Navigation Choices) â€” no further change.
- [x] **R5. The proactive front door** âœ… â€” the Home hero now leads with "I already handled N
  things â€” M need your OK." Pure `lib/home/front-door.ts::buildFrontDoor` (7 tests) folds recent
  **auto-executed** autopilot actions (reversible â†’ link to Autopilot to undo) + **pending**
  approval_requests (most-urgent first â†’ link to the inbox) into one proactive summary;
...
- [x] **R6. Intent-based entry** âœ… â€” a prominent NL **AskBar** at the top of Home routes through the
  same pure `routeCommand` the âŒ˜K bar uses: a recognized goal jumps straight to its reasoning engine,
  a page name navigates, anything else hands to the assistant. Extended `detectIntent` with the two
  missing goals from the brief â€” **`check_availability`** ("who's free Saturday" â†’ Calendar) and
  **`plan_event`** ("plan Emma's party" â†’ Prep Plans) â€” with tests. `components/home/ask-bar.tsx`
...
**P3 â€” Digital Twin / Reasoning Engine depth (Phase 3; 12â€“18mo differentiation).**
- [x] **R7. Unify the reasoning engine** âœ… SHIPPED (2026-07-08). One core in `lib/reasoning/engine.ts`
  (`answerFamilyQuestions`, pure + deterministic, 8 tests) answers the six questions every surface
  consumes â€” **what matters most Â· what's likely forgotten Â· what to decide next Â· what Bubaly can
  just handle Â· who needs help Â· what's next** â€” by *composing* the existing engines: the FOI
  ...
  assembles them from live family data best-effort (any failing source â†’ calm) and persists one
  ...
  tsc Â· **vitest (8 reasoning)** Â· migration + seed validated on PG16 (500 rows, both idempotent).
  âš ï¸ apply `0149` to prod (see `docs/PENDING_PROD_MIGRATIONS.md`).
- [x] **R8. Deepen twin simulation** âœ… SHIPPED (2026-07-08). Pure `projectActivity` added to
  `lib/twin/simulate.ts` (7 tests) â€” the full "if Emma joins travel soccer, what has to move?"
  projection across **schedule Â· travel Â· cost Â· family time Â· homework Â· meals Â· vacation**, each a
  ...
  household (member events + budgets w/ period spend + `vacations` windows) and runs it â€” a safe
  ...
  **`twin_simulations`** (`0147`, family-scoped RLS). UI `components/twin/activity-projection.tsx`
  mounted on `/dashboard/family-digital-twin` (form â†’ per-dimension ripple â†’ Save; saved-scenario
  list). Seed `seed_twin_simulations.sql` (500 rows, verdict spread; in `SEED_ALL.sql`). Verified:
  tsc Â· eslint Â· **vitest (17 twin)** Â· `next build`; migration + seed validated on PG16 (500 rows,
  idempotent). *(Reads the family's real linked model â€” events/budgets/vacations; deeper graph-entity
  wiring can layer on later.)*

**P4 â€” Ecosystem orchestration (Phase 4; 18â€“24mo network effects).**
- [x] **R9. Per-provider sync adapters** âœ… SHIPPED (2026-07-09). Two complementary layers landed
  (parallel work streams), both key-gated per B3 â€” no engineering left, only owner OAuth keys:
  - **Sync-engine contract (the real two-way engine).** `lib/sync/adapter.ts` (`SyncProviderAdapter` +
    ...
    that engine â€” the proven Google pull/push/conflict/mapping loop, now driven purely through the
    ...
    + shared `lib/sync/hash.ts` (every adapter yields the SAME digest â€” proven by test). Wired via
    `POST /api/sync/run`. Seed `seed_sync_microsoft.sql` (connected Outlook account + 500 synced events +
    mappings; in `SEED_ALL.sql`). 17 adapter tests + 13 google-map (no regression); tsc Â· eslint Â· build;
    seed PG16-validated + idempotent. No migration â€” `microsoft` was already in the `sync_provider` enum
    (0018).
  - **Connections-hub contract (the higher-level directoryâ†’sync abstraction).** `lib/connections/adapter.ts`
    (`SyncAdapter`, normalized `NormalizedEvent`/`NormalizedMessage`, `AdapterContext`, pure `planSync`
    decision core: blocks on `needs_setup`/`not_connected`/`unsupported`, fullâ†’incremental once a cursor
    exists). Reference adapters `lib/connections/adapters/` â€” `google-calendar` + `gmail` â€” inert until
    `GOOGLE_OAUTH_*` keys land. Registry `adapterFor`/`syncableProviderIds`. 12 tests; tsc Â· eslint Â· build.
  - **Apple iCloud (CalDAV) adapter âœ… SHIPPED (2026-07-14).** The third provider, proving the contract
    ...
    calendar sync uses CalDAV `sync-collection` (its sync-token IS the engine's cursor â†’ incremental with
    zero engine changes); events map through the already-tested `lib/sync/ics.ts` VEVENT parser/generator.
    Reminders (VTODO) return a null default list, which the generic engine treats as "no task sync" â€”
    ...
    conformance) + shared adapter suite updated â€” 88 sync tests green; tsc Â· eslint clean. No migration
    (`apple` already in the `sync_provider` enum, 0018).
  - **Remaining (owner-only):** provision provider OAuth keys â†’ live Microsoft/Gmail/Calendar sync;
...
- [x] **R10. Family Intelligence â€” the hard signals.** âœ… SHIPPED (2026-07-07). Four pure detectors in
  `lib/intelligence/hard-signals.ts` (11 tests): **ignored-reminder** (same reminder overdue-without-
  done â‰¥3Ã—, grouped by normalized title), **stress windows** (eventsÃ—4 + clashesÃ—12 + overdueÃ—6 bucketed
  by day-type Ã— part-of-day â†’ the peak crunch window), **chore friction** (rejectionsÃ—2 + disputesÃ—3 +
  hand-offs, per chore), **routine adherence** (expected weekday occurrences vs actual completions â†’
  ...
  into **`family_signals`** (`0142`, family-scoped RLS) â€” **preserving dismissals**. Transparent +
  ...
  Verified: tsc Â· eslint Â· **vitest (11)** Â· `next build`; migration + seed validated on PG16 (500 rows,
  idempotent). Feeds the Playbook/reasoning layer (family_signals is now a readable substrate).
  - [x] **5th detector â€” budget drift âœ… SHIPPED (2026-07-10).** `detectBudgetDrift` (pure, +5 tests)
    ...
    **prior period was over too** (a real drift pattern, not FOI's single number). Reuses FOI's
    ...
    tsc Â· eslint Â· **vitest (16 hard-signals)** Â· `next build`. âš ï¸ apply `0157` to prod.
- [x] **R11. The category metric â€” surface "time saved / mental load."** âœ… Pure
  `lib/metric/time-saved.ts::computeTimeSaved` (4 tests) turns this week's system-handled actions â€”
  autopilot auto-executions (5 min), assistant-handled items (4 min), reminders delivered (2 min) â€”
  into a real "â‰ˆ N hours saved this week Â· M things handled for you" figure with a transparent
  ...
  `seed_time_saved.sql` (250 autopilot + 150 agent + 150 reminders = 550 â†’ ~35.8h). The thesis metric
  is now IN the product, not just marketing copy. tsc/eslint/**2078 tests**/build.
- [x] **R12. Moments as an organizing layer** âœ… SHIPPED (2026-07-08). Pure `lib/moments/organizer.ts`
  (`activeMoments`, 9 tests) defines the 10 canonical life moments (Morning Â· School Â· Dinner Â· Homework
  Â· Bedtime Â· Weekend Â· Vacation Â· Birthday Â· Holiday Â· Emergency), each orchestrating the handful of
  capabilities it needs, and decides which are LIVE now from the clock + family signals (upcoming
  birthday/trip/holiday, homework due tomorrow) â€” episodic-near outranks the daily rhythm; Emergency
  always reachable. `/dashboard/moments` now OPENS with a "Right now" organizing band (server-computed,
  deep-linked capability chips, dismiss-for-the-day) above the existing event-prep view â€” organize by
  ...
  `engageMomentAction`. Seed `seed_moment_activations.sql` (500 rows, every moment Ã— ~50 days, status
  spread; in `SEED_ALL.sql`). Verified: tsc Â· eslint Â· **vitest (9)** Â· `next build`; migration + seed
  validated on PG16 (500 rows, idempotent). Additive â€” the event-prep MomentsView is untouched.

### â›” What to STOP
- **No net-new feature modules.** The matrix is saturated; more rows don't move the moat and add
  surface area to maintain. Every new PR should cite which of R1â€“R12 (or a moat layer) it advances.
- **Stop adding standalone AI pages** â€” new AI capability goes *through* the one assistant, not as a
  27th destination.
- Marketing/nav polish (findability of Shopping, AI Inbox, Smart Imports, Kitchen Mode, etc.) is fine
  and cheap, but it is **positioning, not moat** â€” don't confuse it with the work above.

---

## â˜…â˜…â˜… TIME-TO-FIRST-VALUE â€” the 24-hour transformation (owner directive, 2026-07-07) â€” READ FIRST

> **Owner directive (verbatim intent):** *"The next level is to make Bubaly feel indispensable within
> the first 24 hoursâ€¦ invert the experience: deliver obvious value immediately, then let the platform
...
> value.**"* Directly serves the mission â€” **Less Managing Life. More Living It.**
...
> â€” the **first-run funnel**. They are complementary: the moat is the engine; TTFV is the ignition.
...
| **6. Onboarding as guided transformation** | Wizard is clean, 6 steps, a11y-announced, draft-persisted. | It guides through **settings**, not **outcomes**. Each step should end in visible value ("Let's make tomorrow easier" â†’ shows tomorrow's timeline). Re-sequence: value FIRST (import â†’ show), household-building SECOND/deferred. |
...
| **8. Premium feeling** | a11y pass (skip-link/WCAG), `MiniEmpty` empty states, toasts, optimistic updates in many modules. | Inconsistent: not every module has helpful empty states / error recovery / motion. Needs a **consistency sweep** (perf budget, predictable transitions, empty+error states everywhere) â€” measurable, not vibes. |
| **9. Compounding value** | Connections hub + graph make cross-domain links *possible*; twin links calendarâ†”mealsâ†”budget. | The cross-service wins (Calendar+Weatherâ†’travel, Shopping+Budgetâ†’savings, Health+Calendarâ†’med timing) aren't **shipped as visible insights**. This IS moat-work **R2** (re-point engines at the graph) surfaced as day-one wins. |
| **10. Partner tone** | Copy is warm in places; Calm digest is reassuring. | Notification/label voice is still **count-based** in modules ("17 notifications") rather than **partner-framed** ("You're in good shape â€” 3 quick approvals finish tomorrow"). Tone pass across surfaced counts/badges. |
...
### â–¶ THE TTFV BACKLOG (priority order â€” most is *wiring existing engines into the first run*)

**MEASUREMENT (2026-07-12): Onboarding time-to-value audit âœ… (backlog #9).** `lib/onboarding/ttv-audit.ts`
(pure `analyzeOnboarding` â€” step funnel, completion + activation rates, TTV median/p90, **% reaching value
â‰¤ 90s**, and where incomplete runs stall; **11 tests**) rendered at **`/admin/onboarding`** (super-admin, in
`ADMIN_NAV`): TTV stat tiles, an on/below-target callout, the step-completion funnel with per-step drop-off,
and a "where runs stall" chart â€” so the first-run funnel is measurable and the friction is visible to trim.
...
**P0 â€” The "magic first session" (the single biggest conversion/retention bet).**
- [x] **T1. Value-first onboarding re-sequence.** âœ… SHIPPED (2026-07-07). New flow order
  `profile â†’ family â†’ value â†’ about â†’ members â†’ pin â†’ done` â€” the `value` step comes right after the
  ...
  **pastes their calendar (.ics)** or **tries a sample family week**; we parse it purely
  (`lib/onboarding/ics.ts`, 7 tests) and compute an instant **first brief** â€” today's timeline Â·
  clashes Â· action list Â· time-saved (`lib/onboarding/first-brief.ts`, 9 tests) â€” shown BEFORE any
  ...
  first-value moment in **`onboarding_imports`** (`0138`, family-scoped RLS, PG16-validated) â€” the
  ...
  idempotent; wired into `SEED_ALL.sql`). Verified: tsc Â· eslint Â· **vitest (73 onboarding)** Â·
  `next build`. *(OAuth "Connect your calendar" stays owner-gated per R9/B3 â€” paste + sample deliver
  the value moment with zero external keys.)*
- [x] **T2. First-run "instant briefing" builder.** âœ… SHIPPED (2026-07-07). `lib/onboarding/first-brief.ts`
  produces today's timeline Â· likely conflicts Â· prioritized action list Â· **time-saved opportunities**
  Â· **3 dinner ideas**. Dinner ideas come from a curated, family-agnostic catalog **`meal_ideas`**
  (`0139`, reference data, PG16-validated) via the pure, deterministic `lib/onboarding/dinner-ideas.ts`
  (`pickDinnerIdeas` â€” quick meals on busy days, involved on weekends, rotates daily; 6 tests). The
  brief is now **rendered as the celebration/done screen** (`finalizeOnboardingAction` computes it
  server-side â€” with dinner ideas â€” and returns it; `DonePanel` shows headline Â· time-saved Â· today Â·
  clashes Â· dinner ideas instead of the old generic "Welcome"), and also inline in the value step.
  Seed `seed_meal_ideas.sql` (500 dishes Ã— cuisine Ã— effort, idempotent; in `SEED_ALL.sql`, now 26).
  Verified: tsc Â· eslint Â· **vitest (80 onboarding)** Â· `next build`. *(Deferred: rendering the brief
  on the AiHome empty state is T3; dinner ideas draw from the curated catalog, not yet the family's own
  learned tastes â€” that's a Playbook/R10 follow-up.)*
- [x] **T3. New-family home = outcome, never empty.** âœ… SHIPPED (2026-07-07). When `AiHomeDashboard`
  ...
  next best **getting-started steps** (Fill your week Â· Plan dinners Â· Invite family Â· Grocery list Â·
  Chores â€” each marked done from the family's real state, unfinished first), and **3 dinner ideas**.
  ...
  family-scoped RLS, one row/family/day â€” the "never empty" substrate + home-side TTFV signal),
  idempotent upsert on read (mirrors the FOI pattern). Seed `seed_home_briefs.sql` (500 daily
  snapshots, readiness trend, idempotent; in `SEED_ALL.sql`, now 27). Verified: tsc Â· eslint Â·
  **vitest (home + onboarding)** Â· `next build`. *(One ranked hero "insight of the day" is the
  separate T4; this ships the outcome + steps + dinners.)*

**P1 â€” One daily "wow" + partner tone.**
- [x] **T4. Insight-of-the-day.** âœ… SHIPPED (2026-07-07). The home surfaces exactly ONE ranked,
  ...
  **unacknowledged homework due tomorrow**, **grocery/meal savings**, plus conflicts Â· approvals Â·
  overdue reminders Â· renewals Â· documents Â· autopilot. Wired into `AiHomeDashboard` from live signals
  ...
  (the âœ• marks the row dismissed and the **next-best** insight surfaces). Seed `seed_daily_insights.sql`
  (500 rows, every kind Ã— ~56 days, status spread; in `SEED_ALL.sql`, now 28). Verified: tsc Â· eslint Â·
  **vitest (insight + home + onboarding)** Â· `next build`. *(The traffic+weather source for the
  leave-earlier insight is owner-gated on a Maps/weather key per B3 â€” the ranker + seed exercise the
  kind; it goes live when the key lands.)*
- [x] **T5. Partner-tone pass.** âœ… SHIPPED (2026-07-07). Centralized phrasing helper
  `lib/tone/partner-phrasing.ts` (18 tests) turns raw counts into partner voice: `notificationsLine`
  ("You're all caught up" â†’ "3 quick things to glance at" â†’ a full-inbox triage offer), `bellLabel`
  (spoken-friendly aria), `partnerStatus` (one "where you stand" line â€” "You're in good shape â€” 3 quick
  ...
  partner line from the unread count) â€” deliberately left `ai-home-dashboard` to the in-flight T-work to
  avoid a collision. Seed `seed_notifications.sql` (500 rows, all 10 types, ~40 unread; in `SEED_ALL.sql`,
  PG16-validated + idempotent). Verified: tsc Â· eslint Â· vitest Â· `next build`. (Area 10.)

**P2 â€” Collaboration, transparency, premium consistency.**
- [x] **T6. AI-facilitated group decisions.** âœ… Group Voting (`/voting`) now facilitates consensus:
  ...
  **required tags** (e.g. dietary needs); each option carries real **cost / travel / tags**. Pure
  ...
  Migration `0142_poll_facilitation.sql` (additive, idempotent, PG16-validated â€” no new tables so 0078
...
- [x] **T7. "Why this?" everywhere.** âœ… Reusable `components/ai/why-this.tsx` â€” one consistent inline
  ...
  **9 tests**) shapes each engine's output into a consistent `Explanation`. Consumed by **insight-of-day**
  ...
  Feedback persists via `recordAiFeedbackAction` â†’ **`ai_feedback`** (`0143`, append-only, family-scoped
  RLS, PG16-validated) â€” the learning loop a future model-refresh can weigh. Seed
  `seed_ai_feedback_one_family.sql` (600 rows across all 6 surfaces Ã— 5 signals). tsc/eslint/**1999 tests**/build. (Area 7.)
- [x] **T8. Premium-consistency sweep.** âœ… Made **measurable**: a Supabase-backed Experience Scorecard
  ...
  `lib/experience/scorecard.ts` (`scoreAudit`/`gradeFor`/`rollUpScorecard`, **13 tests**) â€” weighted
  composite â†’ letter grade, worst-first ordering, per-dimension averages, trend vs previous audit,
  ...
  `seed_experience_audits_one_family.sql` (540 rows / 30 surfaces Ã— 18 dates) with an upward trend +
  4 surfaces below the bar. tsc/eslint/**2012 tests**/build. *(Route not added to global nav per the
  standing rule â€” reachable at `/dashboard/experience`; add a nav entry on request.)* (Area 8.)
  (Area 8.)

**P3 â€” Felt switching cost (compounding context).**
- [x] **T9. "What Bubaly has learned" surface + life-event templates.** âœ… New `/dashboard/life-events`
  ("Life & Milestones") with two halves: (1) **What Bubaly has learned** â€” accumulated preferences/
  routines/traditions (reuses `family_facts`), fully editable inline (add/edit/pin/delete); (2)
  **Life-event playbooks** â€” 6 one-tap templates (New Baby, Moving, School Start, Vacation, New Pet,
  ...
  *(Route not in global nav per the standing rule â€” reachable at `/dashboard/life-events`.)* (Areas 5 + 6.)

**Instrumentation (proves the bet):**
- [x] **T10. TTFV metric.** âœ… Instrumented **time-from-signup-to-first-outcome-viewed** (median + p90)
...
**Alignment note:** T1â€“T3 + T8 are pure UX/wiring of existing capabilities (no new modules â€” consistent
...
## âš‘ OPEN ITEMS & DECISIONS â€” single source of truth (2026-07-06)

> ### â–£ EVERYTHING STILL OPEN â€” consolidated (as of 2026-07-06, evening)
>
...
> merged reasoning + moat layer: onboardingâ†’marketing wiring + **child logins
...
> on the onboarding Done screen (#239), and screen-reader **progress bars** â€”
> onboarding (#240) + **app-wide** (#241).
>
> **OPEN â€” owner / external (NOT code; the agent cannot execute these):**
> 1. **Apply pending prod migrations** â€” `supabase db push` (incl. `0105_child_logins`
>    + `0118`â†’`0135`, or paste `supabase/APPLY_PENDING_0118-0135.sql`). **THE blocker:**
...
> 2. **Set prod env vars** â€” `CHILD_LOGIN_SECRET` (child username+PIN sign-in) and
>    `CRON_SECRET` (`model-refresh` + `network-aggregate` crons).
> 3. **Provider keys** â€” each lights up an already-built path: **VAPID/FCM** (push) Â·
>    **Maps/ETA** (Friction #6 travel buffer) Â· **GIF provider** (Messages/photos picker) Â·
>    **Stripe Issuing** (real Wallet card balances) Â· **CI Supabase login** (authed e2e).
> 4. **Activate the `onboarding_completed` marketing workflow** so onboarding fires it.
> 5. **Intelligence Network launch gate** â€” insights only surface at **â‰¥100** consenting families.
>
> **OPEN â€” agent-doable follow-ups (small, no blocker; do when directed):**
> - [x] **Role density/tone, broader rollout âœ… (2026-07-10)** â€” the role-tailored greeting
>   (`roleGreeting`, tone-matched per role: parents formal Â· adults casual Â· kids a warm emoji line)
>   now leads the **personal dashboard** too, not just Home â€” extending the Friction #8 treatment
>   (Home focus strip density/`focusMax`/headline + greeting) to a second primary surface. Reuses the
>   pure `lib/ui/role-surface.ts` (already 10 tests); tsc Â· eslint Â· build.
> - [x] **Instrument real journey medians âœ… (2026-07-10, code side)** â€” the ubiquitous **QuickAdd**
>   flow (`components/family/quick-add.tsx`, used across every module) now emits `journey_events` via
>   `useJourney('quick_add')` (start on open Â· complete on save Â· abandon on cancel), so the
>   already-live per-family medians at `/dashboard/journeys` (pure `summarizeJourneys`, real data â€” no
...
>   a cross-family service-role aggregate â€” both need a running app + auth this sandbox lacks.
> - **Friction #6 travel buffer** â€” code-ready; needs the Maps/ETA key (owner item 3 above).
> - [x] **Twin projector â€” care providers âœ… (2026-07-10).** The cross-domain projector now links
>   **health providers** (doctors / dentists / specialists) from `health_providers` as `org` nodes the
>   member "sees" (`Emma â†’ Dr. Lee (Pediatrics)`), so the graph + reasoning layer can reason over health
...
>   tsc Â· eslint Â· **vitest (17 twin-project)** Â· build. *(Smart-home entities still deferred â€” no
>   device domain has landed yet.)*
> - **GIF picker** (Messages/photos) â€” code path exists; needs the provider key (owner item 3).
> - [x] **Backend production hardening (2026-07-07, reasoning/network lane)** âœ… â€” Intelligence
...
>   all clean (11/11 crons `CRON_SECRET`-guarded; `vercel.json` â†” cron routes match 1:1); Knowledge
...
>   â€” both now scale past thousands of families without hitting the 60s cron limit. Privacy-invariant
>   regression test added (noised count never leaks true cohort size). tsc/eslint/**1916 tests**/build green.
> - [x] **Social share cards (OpenGraph / Twitter)** âœ… (#245) â€” `app/opengraph-image.tsx` +
>   `app/twitter-image.tsx` render a brand-gradient 1200Ã—630 card (Bubaly wordmark + tagline)
...
> - [x] **JSON-LD + canonical SEO (2026-07-07)** âœ… (#246) â€” (a) JSON-LD structured data
>   (`components/marketing/structured-data.tsx`: Organization + WebSite + SoftwareApplication on
>   the homepage, FAQPage on `/faq`) for Google/Bing rich results â€” honest data only; (b)
>   self-referential canonical URLs (`alternates.canonical: './'` in the root layout resolves
>   per-route â€” verified homeâ†’`/`, `/pricing`â†’`/pricing`, `/faq`â†’`/faq`), preventing duplicate
>   indexing of www/non-www/trailing-slash/preview-domain variants. Build- and server-verified.
> - [x] **HSTS security header (2026-07-07)** âœ… (#247) â€” `Strict-Transport-Security:
>   max-age=63072000; includeSubDomains` added to the global header block in `next.config.mjs`
>   (no `preload` â€” avoids the irreversible preload-list commitment). Rounds out the existing
>   X-Frame-Options / nosniff / Referrer-Policy / Permissions-Policy set.
> - [x] **Skip-to-content link, WCAG 2.4.1 (2026-07-07)** âœ… (#248) â€” `components/a11y/skip-link.tsx`
...
- [x] **Intelligence Network aggregation** âœ… â€” decisions signed off + pipeline **built**
  ...
  cohort = kids-band Ã— household-size, **no** geography, launch gate **â‰¥100 families**.
  Seed `seed_network_aggregates.sql`. (`docs/INTELLIGENCE_NETWORK_DESIGN.md` now marked SIGNED OFF + BUILT.)
- [x] **Global nav entries added this session** âœ… CONFIRMED (owner 2026-07-06): keep the left
  navigation as-is and keep all four â€” Reasoning Graph, Decision Engine, Prep Plans, Intelligence
  Network (all `minLevel: 0`). They live in **All Services + the Navigation Choices catalog** (users
  pin/add them); **deliberately NOT promoted into the default `PRIMARY_NAV` rail** â€” the default rail
  stays curated. No further change.

### B. Operational â€” prod go-live (owner action, not code)
- [ ] **Apply migrations 0125â†’0135 to prod** (`supabase db push`, or paste `supabase/APPLY_PENDING_0118-0135.sql`).
  THE blocker â€” everything reads these; silently no-ops until applied.
- [ ] **Set `CRON_SECRET` in prod** â†’ activates the `model-refresh` + `network-aggregate` crons.
- [ ] Keys that light up already-built paths: **VAPID/FCM** (push) Â· **Maps/ETA** (travel buffer) Â·
  **GIF provider** (Messages picker) Â· **Stripe Issuing** (Wallet card balances).

### C. Genuinely-open product items (agent-doable)
- [x] **FOI financial dimension** âœ… â€” already wired: `countOverspentBudgets` (real budgets vs
  this-period expenses) + real accounts-below-zero in `lib/operating-index/server.ts`. (Stale entry â€” verified done.)
- [x] **Onboarding pre-family telemetry** âœ… â€” `0133_onboarding_events.sql` + `lib/analytics/onboarding.ts`
...
- [x] **AI Concierge write-back** âœ… â€” already wired: create plans, update status
  (planning/completed/cancelled), delete, and convert a plan â†’ real `calendar_events`. (Stale entry â€” verified done.)
- [x] **Twin auto-refresh trigger** âœ… â€” `0134_model_dirty.sql`: `family_model_dirty` flag +
...
- [ ] **Messages GIF picker** â€” blocked on the provider key. *(external key â€” not agent-doable)*
- [x] **Kid-login brute-force hardening** âœ… (2026-07-07) â€” the no-email child sign-in (username +
  ...
  *(Phone-number deep-dive deferred: the parallel session is actively rewriting `phone-auth`/OTP on
  main â€” building there now would collide. Revisit once that settles.)*

> **Section C status: closed except the GIF picker (needs an external provider key).** All four
> agent-doable items are done â€” 2 built this session (onboarding telemetry, twin trigger), 2 were
> already wired (FOI financial, Concierge write-back).

### D. â˜… Moat layer shipped this session (all validated: 1838 tests / tsc / eslint / build)
- [x] **Knowledge Graph** â€” `0129`, `lib/graph/reason.ts` (16 tests, impact propagation), `/dashboard/graph`, `seed_graph.sql`.
- [x] **Twin cross-domain projector** â€” `lib/twin/project.ts` (8 tests) + `projectTwinAction` (pillar #2's linked model).
- [x] **Decision Engine** â€” `0130`, `lib/decisions/engine.ts` (9 tests), `/dashboard/decisions`, `seed_decisions.sql`.
- [x] **Chief of Staff â†” graph** â€” `lib/agents/graph-insight.ts` (6 tests): hubs + ripple into briefings.
- [x] **Prep Plans (autonomous planning)** â€” `0131`, `lib/planning/prep.ts` (10 tests), `/dashboard/prep-plans`, `seed_prep_plans.sql`.
- [x] **Life Readiness (horizon rollup)** â€” `lib/readiness/assess.ts` (11 tests), embedded on `/dashboard/readiness`.
- [x] **Intent-Based UX** â€” `lib/intent/detect.ts` + command-bar integration (18 tests).
- [x] **Auto-refresh cron** â€” `lib/planning/refresh.ts` (8 tests) + `app/api/cron/model-refresh` + `vercel.json`.
- [x] **Intelligence Network (full)** â€” `0132` consent + k-anonymity (`lib/network/insights.ts`, 9 tests)
...
1. **AI Operating Layer** â€” a *platform layer* (not per-feature AI) that continuously reasons
...
2. **Household Digital Twin** âœ… SHIPPED â€” one continuously-updated model linking people,
   relationships, calendars, home, vehicles, pets, schools, finances, etc. **Both halves now built:**
   (a) **decision simulation** (`lib/twin/simulate.ts`, `/dashboard/family-digital-twin`) â€” "if we
   accept this tournament, what has to move?"; and (b) the **cross-domain linked model** â€” the
...
â˜… **Knowledge Graph â€” the reasoning substrate** âœ… SHIPPED (moat build) â€” migration `0129`
   ...
   relationships (Emmaâ†’Soccerâ†’Fieldâ†’Weatherâ†’Dinner) instead of retrieving isolated rows.
3. **Family Intelligence Layer / Playbook** â€” every interaction improves understanding; build a
   family playbook (favorite meals, birthday/holiday traditions, travel styles, homework habits,
   shopping patterns, communication styles). Family stays in control (visible + editable â€” the
   Knowledge Base `family_facts` is the seed of this).
4. **Outcomes, not features** â€” organize around goals (**Run Today Â· Feed the Family Â· Plan a Trip
   Â· Prepare for School Â· Manage Money Â· Keep Everyone Healthy Â· Celebrate Together Â· Prepare for
   the Unexpected**); the AI picks the underlying capabilities automatically.
5. **Family Command Center** â€” a concise morning operational briefing (today's priorities,
   ...
   `/dashboard/command-center`, `/dashboard/briefing` â€” unify + make the briefing generative.)*
6. **Specialized AI agents behind one interface** âœ… SHIPPED â€” Chief of Staff Â· Scheduler Â· Meal
   Planner Â· Budget Coach Â· Household Manager Â· School Coordinator Â· Health Guide Â· Travel Planner Â·
   Memory Keeper Â· Communications Assistant. User sees one assistant; agents collaborate internally.
   ...
   - Migration `0127_agent_activity.sql` â€” durable, family-scoped RLS log of what each agent
     surfaces/does (kind/severity/status, done/dismissible). PG16-validated (RLS, constraints, trigger).
   - `/dashboard/agents` (`components/modules/agents-module.tsx`): server reads a real snapshot â†’
     live briefings; roster grid w/ status dots; per-agent items + recent activity (Done/Dismiss
     writes back). Nav: Suggested â†’ Family Assistant (`Bot`, free). 100% Supabase.
   - Seed `supabase/seed_pillar6_agents.sql` â€” 500 activity records across all 10 agents (idempotent).
   - Verified: tsc Â· eslint Â· **1723 vitest** Â· build.
7. **Family Operating Index (FOI)** â€” measure how well the household is *functioning* (not to
   ...
   suggestions**, not vanity scores. **â† FIRST SLICE, building now.**
8. **Design for Calm** âœ… SHIPPED â€” fewer notifications, ONE prioritized inbox, an AI daily digest,
   ...
   - Pure `lib/calm/inbox.ts` (**7 tests**): `buildCalmInbox(items)` folds every signal source
     (agents Â· autopilot Â· Operating Index Â· approvals Â· reminders) into ONE de-duplicated, ranked
     list â€” `needsYou` (action only, capped), `today` (attention, capped), and a `quieted` count for
     the rest; `calmDigest` writes the gentle one-liner; `noiseReduced` = notifications spared.
   - `/dashboard/calm` (`components/modules/calm-module.tsx`): server reads the real sources â†’ the
   ...
   - Nav: Suggested â†’ Calm (`Leaf`, free). Verified: tsc Â· eslint Â· **1730 vitest** Â· build.
9. **Family API** âœ… SHIPPED (hub) â€” the orchestration hub that *connects* external services
   (calendars, email, banking, grocery/delivery, smart home) rather than replacing them.
   - Pure `lib/connections/providers.ts` (**8 tests**): the provider registry (11 providers Ã— 5
   ...
   - Migration `0128_family_connections.sql` â€” durable, family-scoped RLS connection records
     (provider/category/status/account_label/last_synced_at; `unique(family_id,provider,external_account_id)`).
     PG16-validated. **No tokens stored here** â€” those belong in a secret store.
   ...
   - Nav: Suggested â†’ Connections (`Network`, free). Seed `supabase/seed_pillar9_connections.sql`
     (500 records, all providers Ã— statuses, idempotent).
   - **Gated for later (needs provider keys):** the actual OAuth/token exchange + live two-way data
     ...
     its OAuth against these records. Verified: tsc Â· eslint Â· **1738 vitest** Â· build.

### â–¶ Slice 1 âœ… SHIPPED: Family Operating Index â€” the measurable core of the Operating Layer
- [x] **Schema** `0125_family_operating_index.sql` â€” append-only daily snapshots (composite +
...
- [x] **Engine** `lib/operating-index/score.ts` (pure, **12 tests**) â€” `computeOperatingIndex(snapshot,
  now)` â†’ 7 dimension scores (0â€“100) + weighted composite + band (thriving/steady/stretched/overloaded)
...
- [x] **Server** `lib/operating-index/server.ts` â€” builds the snapshot from real tables (calendar,
  ...
  composite for the trend arrow. **âœ… FULLY WIRED (2026-07-05):** every input now reads live data â€”
  `overspentBudgets` (budgets vs this-period expenses, pure/tested `lib/operating-index/inputs.ts`, 7
  tests), `negativeBalances` (financial_accounts < 0, excl. credit), `lowInventory` (pantry â‰¤
...
- [x] **Route** `/dashboard/family-operating-index` â€” composite ring dial + band, trend vs. yesterday,
...
- [x] **Nav** entry (Operating Index, `Gauge`, minLevel 0 â†’ free + auto in Navigation Choices catalog).
- [x] **Verified** tsc Â· eslint Â· **1661 vitest** Â· `next build`; migration validated on PG16.

### â–¶ Slice 2 âœ… SHIPPED: "Since yesterday" evening recap (pillar #5, off the FOI snapshot)
- [x] **`lib/operating-index/summary.ts`** (pure, **8 tests**) â€” `summarizeChange(current, prior)`
  diffs two FOI snapshots â†’ headline + composite delta + improved/declined dimensions (â‰¥5-pt moves)
...
- [x] **"Since yesterday" card** on `/dashboard/family-operating-index` (headline + cleared/new/Â±dim
  chips). Reuses the state vector â€” zero contended files touched. tsc/eslint/**1669 tests**/build.

### â–¶ Pillar #4 âœ… SHIPPED: Outcomes-not-features launcher
- [x] **Engine** `lib/outcomes/launcher.ts` (pure, **10 tests**) â€” the eight outcomes (Run Today Â·
  Feed the Family Â· Plan a Trip Â· Prepare for School Â· Manage Money Â· Keep Everyone Healthy Â·
  Celebrate Together Â· Prepare for the Unexpected); `buildOutcomePlan(id, ctx)` assembles each
...
- [x] **Route** `/dashboard/outcomes` â€” server reads the real family snapshot (count queries +
  birthday compute), builds badged plans, hands them to `components/modules/outcomes-launcher.tsx`
  (goal-first picker â†’ the exact capabilities to get it done, with live badges). 100% Supabase, no
  new migration (reads existing tables).
- [x] **Nav** entry (Suggested â†’ Outcomes, `Wand2`, minLevel 0 â†’ free + in Navigation Choices catalog).
- [x] **Verified** tsc Â· eslint Â· **1709 vitest** Â· `next build`.

**Next slices (build order):** #1 wire the five orchestrator questions ("what's likely to go wrong
tomorrow / who's overloaded / what to decide next / what info is missing") off the same snapshot Â·
#2 turn the snapshot into the Digital Twin's state vector for simulation Â· adopt `summarizeChange`
inside the existing Command Center / Daily Briefing surfaces (ui lane â€” coordinate vs. parallel session).

*(Why FOI first: it's the one pillar with no existing surface, it forces the reasoning core to
read across the entire household â€” the seed of pillar #1 â€” and it makes the north-star metric
...
## â˜ OPEN WORK TRACKER (single source of truth for what's left â€” keep in sync)

### ðŸŒ± Test-data / seed coverage (as of 2026-07-05)
**37 seed files** exist under `supabase/` â€” most domains have a 500+-record `*_one_family.sql`
(scoped to The Kramer Family `92298eb2â€¦`, idempotent): calendar Â· tasks Â· chores Â· meals(+extras) Â·
finances Â· finance-hub Â· memories Â· messages Â· wallet(+ledger) Â· voice Â· location Â· family Â· family-safety Â·
files Â· planning Â· food Â· home Â· roles(#8) Â· pillars 1â€“6 & 9. **NEW `seed_operating_index_one_family.sql`**
(â‰ˆ613 records) lights up the FOI (#7) end-to-end â€” every wired input fires (overspent budgets, a
...
- [x] **Remaining seed gaps closed âœ… (2026-07-10).** The two genuine gaps are now seeded at 500:
  **FOI snapshot history** â€” `seed_operating_index_history.sql` (500 daily `family_operating_index`
  snapshots, gentle wave + band spread) so the FOI **trend line** + day-over-day recap render (was
  "first reading" until history existed); and **`meal_votes`** â€” `seed_meal_votes.sql` (500 votes +
...
- â˜‘ **NEW `seed_group_decisions_one_family.sql`** (â‰ˆ1,300 rows: 100 polls / 400 options / ~800 votes)
  fully exercises T6 AI-facilitated group decisions â€” categories, budgets, dietary tags, and biased
...
- â˜‘ **NEW `seed_ai_feedback_one_family.sql`** (600 rows) exercises T7 "Why this?" feedback log â€”
  all 6 surfaces (insight/autopilot/agent/voting/decision/briefing) Ã— 5 signals, over the last ~60 days.
- â˜‘ **NEW `seed_experience_audits_one_family.sql`** (540 rows) exercises T8 Experience Scorecard â€”
  30 surfaces Ã— 18 dates over ~68 days, upward trend, accessibility weakest, 4 surfaces below the bar.
- â˜‘ **NEW `seed_life_events_one_family.sql`** (567 rows) exercises T9 Life & Milestones â€” 44 plans
  (active/completed/archived) Ã— ~493 dated checklist items + 30 learned facts. Run it, open `/dashboard/life-events`.
- â˜‘ **NEW `seed_activation_events.sql`** (~820 rows / 220 cohorts) exercises T10 TTFV â€” signupâ†’first-value
...
- [x] **Slice 1 â€” Family Operating Index** (engine + page + 0125). PR #227. âœ…
- [x] **Slice 2 â€” "Since yesterday" evening recap** (pillar #5, off the FOI snapshot). PR #228. âœ…
- [x] **Slice 3 â€” Orchestrator questions** (pillar #1) âœ…: the five daily questions â€”
  *what's most likely to go wrong tomorrow Â· what can auto-complete today Â· who's
  overloaded this week Â· what should we decide next Â· what info is missing before an
  important event.* Pure `lib/operating-index/orchestrator.ts` (**10 tests**) over the
  FOI snapshot + a tomorrow slice; reads open â‰¥90-confidence `autopilot_suggestions`
  for Q2 (read-only reuse, no engine edit); "Your family chief of staff" section on the
  FOI page. tsc/eslint/**1679 tests**/build.
- [x] **Slice 4 â€” Command Center adopts the recap** (pillar #5) âœ…: shared server-safe
...
- [x] **Slice 5 â€” Household Digital Twin: decision simulation** (pillar #2) âœ…: pure
  `lib/twin/simulate.ts` (**10 tests**) `simulateDecision(decision, ctx)` â€” safe "what-if" that
  runs against real data but writes nothing. Two scenarios: **add a commitment** ("if we accept this
  tournament, what has to move?" â†’ overlaps as blockers, tight turnarounds, heavy-week load) and
  **a big spend** ("can we add two nights and stay in budget?" â†’ real budget-vs-transactions
...
- [x] **Slice 6 â€” Family Playbook** (pillar #3) âœ…: Bubaly learns durable preferences/traditions
  from real usage and proposes them; the family confirms â†’ real `family_facts` rows.
  Migration `0126_family_playbook.sql` (`family_playbook_suggestions`: category/label/value/
  evidence/confidence/signature/status suggestedâ†’accepted/dismissed/`fact_id`, `unique(family_id,
  signature)`, family-scoped RLS). Pure `lib/playbook/learn.ts` (**10 tests**) `learnPlaybook(signals)`
  â†’ deduped/ranked/capped suggestions from meal/grocery/favorite/tradition signals. Server actions
  `refreshPlaybookAction` (mines meal_plans+meals, grocery_items, family_favorites, yearly
  calendar_events â†’ upsert, `ignoreDuplicates` so dismissed/accepted never resurface) /
...
- [x] **Slice 7 â€” Outcomes launcher** (pillar #4) âœ…: goal launcher at `/dashboard/outcomes`
  ("Run Today / Feed the Family / Plan a Trip / â€¦") auto-selecting capabilities. Seed `seed_pillar4_outcomes.sql`.
- [x] **Slice 8 â€” Specialized agents behind one interface** (pillar #6) âœ…: `lib/agents/roster.ts`
  (10-agent roster + Chief of Staff, 11 tests), `agent_activity` (`0127`), `/dashboard/agents`.
  **Now graph-aware** â€” the Chief of Staff reasons over relationships (`lib/agents/graph-insight.ts`).
  Seed `seed_pillar6_agents.sql`.
- [x] **Slice 9 â€” Design for Calm** (pillar #8) âœ…: `lib/calm/inbox.ts` (one prioritized inbox +
  digest, 7 tests), `/dashboard/calm`. Seed `seed_pillar8_calm.sql`.
- [x] **Slice 10 â€” Family API** (pillar #9) âœ… (hub, functionally key-gated): `0128_family_connections.sql`,
...
- [x] FOI financial dimension: real **budget-overspend** (budgets vs expenses) + **negative
  ledger balance** detection âœ… â€” all 5 previously-hardcoded inputs now read live data via pure
...
- [x] Onboarding: anonymous **pre-family telemetry** âœ… â€” shipped as `0133_onboarding_events.sql` +
  ...
  `lib/analytics/activation.ts`). (Was a stale duplicate of Section C â€” verified done.)
- [x] AI Concierge: **write-back of accepted recommendations** âœ… â€” the module creates/updates/deletes
  `concierge_plans` and converts a plan â†’ real `calendar_events`. (Stale duplicate of Section C â€” verified done.)

### C. Human-owned / blocked (NOT agent-doable â€” surfaced, not buildable here)
- [ ] **Apply pending prod migrations 0125â†’0132** (`supabase db push`, or paste each in the
  Supabase SQL editor). **Highest leverage** â€” the whole reasoning layer (FOI, playbook, agents,
  ...
  *Agent cannot execute â€” no prod DB creds/CLI in sandbox (verified).*
- [ ] **Set `CRON_SECRET` in prod** â†’ activates the `model-refresh` cron (twin + prep-plan auto-refresh).
- [ ] **VAPID/FCM keys** â†’ lights up the push path (already built).
- [ ] **Maps/ETA key** (Friction #6 travel buffer) Â· **GIF provider key** (Messages picker) Â·
  **Stripe Issuing** (real-time Wallet card balances) Â· **CI Supabase login** (authed e2e).

---

## Roadmap features (from the product roadmap)

### 1. Marketplace â€” "Buy, sell, rent, borrow within the platform"  â˜‘ DONE

> **â–¶â–¶ World-class buildout (2026-07-13) â€” 8 vertical slices shipped this session, full lifecycle
> beyond eBay/Craigslist.** Each is migration (where needed) + pure engine + tests + realtime/SSR UI +
> â‰¥500-row seed (where stateful) + docs, PG16-validated, pushed to `main`. Lifecycle now:
> **discover â†’ price-check â†’ watch/price-track â†’ bid/offer â†’ agree â†’ hand off â†’ return â†’ review**, with
> a seller cockpit + a safety/moderation layer. The slices (details in the checklist items below):
> 1. **Live Auctions** (`0183`, +`0184`/`0185` hardening) â€” proxy bidding, reserve, Buy-It-Now,
>    anti-snipe, `close-auctions` cron.
> 2. **Make an Offer** (`0186`, +`0187` hardening) â€” Best-Offer negotiation, atomic accept.
> 3. **Pickup & Hand-off** (`0190`) â€” safe-meetup scheduling, family calendar, in-person code.
> 4. **Price History + Drop Watch** (`0191`) â€” trigger-driven change log + watcher alerts.
> 5. **Returns + Overdue** (`0192`) â€” rent/borrow due reminders + `return-reminders` cron.
> 6. **Seller Cockpit** (`/selling`, no migration) â€” ranked "what needs you" view.
> 7. **Trust & Safety** (`0193`) â€” report a listing + super-admin moderation queue.
> 8. **Price Coach** (no migration) â€” comp-band "is this a fair price?" for buyers.
> **Prod:** apply `0183`â€“`0193` + set `CRON_SECRET` (see `docs/PENDING_PROD_MIGRATIONS.md`); all safe
...
- [x] `lib/marketplace/listings.ts` â€” labels, money math, filter/rank, offer/claim state machine (pure). Tests `tests/marketplace-listings.test.ts` (10 tests).
- [x] `components/modules/marketplace-module.tsx` â€” browse+filter, post/edit, interest/claim, owner offer review (acceptâ†’hand-off, decline), withdraw/complete. 100% Supabase via `useRealtimeQuery`.
...
#### â–¶ Marketplace V2 â€” the AI-first marketplace (owner design, 2026-07-09) â€” FULL INCORPORATION CHECKLIST

> **Owner directive:** incorporate the provided design as the `bubaly.com` marketplace page.
> "The world's easiest AI-first marketplace â€” buy, sell, rent, borrow, lend & more, all in one
> trusted community." Keep the **bubaly global header**; the section rail's **Home** returns to the
> bubaly landing; **no marketplace-specific Messages** â€” the Messages entry routes to the ONE bubaly
> Messages surface. 100% Supabase-wired, 100% production ready.

**Schema + engines (shipped âœ…):**
- [x] Migration **`0151_marketplace_v2.sql`** â€” `marketplace_stores` (one storefront/member) Â·
  `marketplace_follows` Â· `marketplace_saves` (â™¥) Â· `marketplace_collections`+`_items` Â·
  `marketplace_orders` (requestedâ†’confirmedâ†’activeâ†’returnedâ†’completed/cancelled) Â·
  `marketplace_reviews` (two-sided, 1â€“5â˜…, one per side per order) Â· listing kinds widened with
...
- [x] Pure **`lib/marketplace/trust.ts`** â€” `computeTrustScore` (volume-scaled ratings + completed
  exchanges + activity âˆ’ disputes; new members start at 50 "Building"), `ratingSummary`.
- [x] Pure **`lib/marketplace/discover.ts`** â€” `aiPicks` (badges: AI Match Â· Hot Rental Â· Great Deal Â·
  Borrow Nearby Â· Trending Â· New Today, variety-first), `activityFeed` ("Sarah rented a dress Â· 2 min
  ago"), `rankCreators`. **14 tests** across both engines.
- [x] Server actions: `toggleSaveAction` Â· `toggleFollowAction` Â· `upsertStoreAction` Â·
  `setOrderStatusAction` (legal-transition-enforced) Â· `leaveReviewAction` (completed orders only,
  both parties, one review per side).
- [x] Accepting an offer now records a **`marketplace_orders`** row (best-effort) â€” offersâ†’ordersâ†’
  reviewsâ†’trust is one connected loop.

**Surfaces (shipped âœ…):**
- [x] **`/dashboard/marketplace`** â€” the V2 home inside the bubaly global frame: hero ("AI-first
  marketplace" + trust chips + featured cards) Â· 8-tile action grid (Sell/Rent/Lend/Borrow/Request/
  Donate/Swap/Create Store) Â· **AI Picks for You** (badged, â™¥-save) Â· AI Buyer Assistant + Request &
  Get Matched (live match count) + Verified-Trusted-Safe row Â· Browse by Category Â· Popular
  Collections Â· right rail: **AI Marketplace Assistant** (prompt chips â†’ the ONE bubaly assistant) Â·
  Recent Activity Â· Top Creators (Follow) Â· **Your Trust Score** Â· Safety First.
- [x] **Single left rail (fix, 2026-07-09):** the Marketplace rail now REPLACES the global bubaly
  ...
  Bubaly logo + global top header stay and there's ONE rail â€” not the global nav + a second rail.
  "Your Trust Score" moved to the bottom of the rail (client-computed via `SidebarTrustScore`),
  matching the design; the page's right rail is now Assistant Â· Nearby Activity Â· Top Creators Â·
  Safety First. `next build` green (9 routes).
- [x] Marketplace **section rail** (`components/marketplace/marketplace-nav.tsx`): Marketplace Â·
  **Home â†’ `/dashboard`** (bubaly landing) Â· AI Assistant Â· Browse Â· Requests Â· Rentals Â· Borrow &
  Lend Â· Buy & Sell Â· Donate Â· Swap Â· Collections Â· Creators Â· My Store Â· My Listings Â·
  **Messages â†’ `/dashboard/messages`** (global bubaly Messages â€” no marketplace inbox) Â· Orders Â·
  Reviews Â· Saved Â· **Verifications â†’ `/dashboard/trust`** Â· Post an Item.
- [x] **`/browse`** â€” the full board (existing realtime module) + match strip; rail-driven via
  `?kind=` `?cat=` `?q=` `?post=1&kind=` (deep-linked post modal).
- [x] **`/saved`** Â· **`/orders`** (lifecycle controls + leave-review) Â· **`/reviews`** (received/given
  + your rating) Â· **`/collections`** (grid + detail) Â· **`/creators`** (ranked storefronts + Follow) Â·
...
- [x] Seed **`seed_marketplace_v2.sql`** (~1,200 rows: stores, follows, 6 collections + 150 items,
  ...
  PG16-validated Ã—2 (idempotent). Verified: tsc Â· eslint Â· **vitest (34 marketplace)** Â· `next build`
  (all 9 routes).
- [x] âš ï¸ apply **`0151`** to prod (see `docs/PENDING_PROD_MIGRATIONS.md`). Safe before apply: every
  V2 read is best-effort â€” the home renders with empty rails and the 0120 board still works.

- [x] **Marketplace top-level URL move âœ… SHIPPED (2026-07-09, #275).** The whole surface moved from
...
- [x] **Listing photos âœ… SHIPPED (2026-07-09, #277).** Cards + post/edit now render `photo_url`:
  ...
  `seed_marketplace.sql` gives all 500 listings a photo (`picsum.photos`). PG16-verified Â· tsc Â·
  eslint Â· `next build`. *(Still open: a Supabase Storage **upload** path â€” bucket + policies â€” so
  owners can attach a file instead of pasting a URL; owner decision on storage.)*

- [x] **Listing detail page âœ… SHIPPED (2026-07-09, #278).** Clicking a listing now opens a real
  detail page â€” **`/marketplace/item/[id]`** (`app/(app)/marketplace/item/[id]/page.tsx`),
  ...
  seller's real reviews received + completed orders + listings posted â†’ score, band, star rating, top
  factor; links to the seller's storefront when present), and this listing's reviews. Actions: **Save
  (â™¥)** + an **Interest/Claim** CTA via the new guarded server action **`makeOfferAction`** (rejects
  ...
  Verified: tsc 0 Â· eslint 0 Â· `next build` 0 (`/marketplace/item/[id]` registered) Â· Vercel preview
  deployed green.

**Remaining to reach the full design vision (open):**
- [x] **Listing photo uploads âœ… SHIPPED (2026-07-14).** Real file upload, not paste-a-URL.
  Migration **`0194_marketplace_photos_bucket.sql`** â€” idempotent public **`marketplace-photos`**
  ...
  New **`components/marketplace/photo-upload.tsx`** â€” pick/drop an image â†’ uploads to
  `{userId}/{ts}-{rand}.{ext}` â†’ public URL becomes `photo_url`; client-side mime + 10 MB validation,
  **Remove** deletes the owned object (no storage leak on abandoned drafts), paste-URL fallback kept.
  ...
  Verified: tsc 0 Â· eslint 0 Â· vitest (quick-post 9 + listings 10). âš ï¸ apply `0194` to prod (idempotent).
- [x] **Real LLM marketplace assistant âœ… SHIPPED.** `components/marketplace/market-assistant.tsx`
  (rendered on `/marketplace`) drives `askMarketAssistantAction` â€” a **two-tier** conversational flow
  ...
  `isAIConfigured()`. Graceful degrade, no second chat surface â€” one assistant. (LLM tier lights up when
  the owner provisions the LLM key per B3; the grounded engine ships value today.)
- [x] **"Post in under 60 seconds with AI" âœ… SHIPPED.** `components/marketplace/quick-post.tsx`
  (rendered on `/marketplace`) â€” one sentence â†’ the deterministic `draftListing` engine fills
...
- [ ] **Secure payments** â€” real checkout/escrow is Stripe-gated (B3/owner); today amounts are
  recorded on orders and settled off-platform (family context makes this acceptable pre-keys).
- [ ] **Geo "near me" distances** â€” needs the Maps key (B3); location is stored free-text today.
- [ ] **Cross-family network marketplace** â€” the design's "community" reach beyond one household
  rides on the Intelligence-Network consent rails (`network_consent`) â€” a strategy decision (Â§A).

- [x] **Match intelligence (next level) âœ… SHIPPED (2026-07-09).** The board is now proactive:
  it connects open **`wanted`** requests to the supply already posted (`sell`/`free`/`rent`/`borrow`)
  â€” *"You're looking for a bike; Mom listed a balance bike for free."* Pure `lib/marketplace/matches.ts`
  (`computeMatches`/`scoreMatch`/`titleTerms`, **10 tests**) scores each wantedâ†”supply pair by shared
  ...
  `seed_marketplace_matches.sql` (500 rows, status spread; in `SEED_ALL.sql`). Verified: tsc Â· eslint Â·
  **vitest (10 match + 10 listings)** Â· `next build`; migration + seed validated on PG16 (500 rows,
  idempotent). âš ï¸ apply `0150` to prod (see `docs/PENDING_PROD_MIGRATIONS.md`).

- [x] **Saved Searches & Alerts âœ… SHIPPED (2026-07-09).** "Alert me when someone lists X." A member
  ...
  `countNewSince`/`describeSearch`, **12 tests**) â€” AND-combined criteria, all-terms keyword match over
  ...
  **`marketplace_saved_searches`** (`0152`, family-scoped RLS, PG16-validated) + types; server actions
  ...
  `AlertActions`; nav entry (Alerts, `BellRing`, additive). Pull-based (no notification hook â†’ zero
  collision with the board). Seed `seed_marketplace_saved_searches.sql` (500 alerts, varied criteria,
  idempotent). Verified: tsc Â· eslint Â· **vitest (12)** Â· `next build` (`/marketplace/alerts`).
  âš ï¸ apply `0152` to prod.

- [x] **Marketplace Pulse âœ… SHIPPED (2026-07-09).** A read-only intelligence view at
  **`/marketplace/insights`** â€” supply by kind, **where demand outruns supply** (open `wanted` vs
  available per category), **price benchmarks** (median/avg per category), and **what's hot** (saves Ã—2
  + offers Ã—3). Pure `lib/marketplace/insights.ts` (`marketplaceInsights`, **6 tests**); reads existing
  listings/saves/offers (NO new schema â€” the existing marketplace seeds populate it). Nav entry (Pulse,
  `Activity`, additive). Verified: tsc Â· eslint Â· **vitest (6)** Â· `next build` (`/marketplace/insights`).

- [x] **Listing Q&A âœ… SHIPPED (2026-07-09).** "Ask a question" on any listing â€” public within the
  ...
  **5 tests**). Nav entry (Questions, `MessageSquare`; one surgical mount on the detail page). Seed
  `seed_marketplace_questions.sql` (~400 Q&A, ~half answered by the owner). Verified: tsc Â· eslint Â·
  **vitest (5)** Â· `next build`. âš ï¸ apply `0153` to prod.

- [x] **Following feed âœ… SHIPPED (2026-07-09).** Seller-based discovery to complement keyword Alerts:
  **`/marketplace/following`** shows the latest browsable listings from the creators/stores you follow,
  ...
  Verified: tsc Â· eslint Â· **vitest (5)** Â· `next build` (`/marketplace/following`).

- [x] **Curated demo dataset âœ… SHIPPED (2026-07-09).** **`seed_demo_account.sql`** â€” ~200 curated,
  ...
  Family, 1111â€¦), **schema-drift safe** (each table guarded so an unmigrated table is skipped, not
  fatal) and additive (tagged `Demo Â· ` / `[demo]`, never disturbs seed_prod). PG16-validated (populates
  present tables, skips absent, idempotent). *(NOTE: superseded as the demo's data source by the
  single-account rework at the top of this file â€” the one-click demo now seeds via `lib/demo/seed.ts` /
...
- [x] **Live Auctions â€” real proxy bidding âœ… SHIPPED (2026-07-13). The eBay-beating gap.** Turns the
  marketplace into a live auction house â€” the signature eBay behavior Craigslist has never had and our
  fixed-price board lacked. Full vertical slice, realtime, atomic, 100% Supabase:
  - Migration **`0183_marketplace_auctions.sql`** â€” auction columns on `marketplace_listings`
    (`sale_format` fixed|auction Â· `auction_starts_at`/`ends_at` Â· `starting_bid_cents` Â· `reserve_cents`
    Â· `buy_now_cents` Â· `current_bid_cents` Â· `bid_count` Â· `highest_bidder_*` Â· `highest_max_cents` Â·
    `anti_snipe_minutes` Â· `auction_closed_at`) + new **`marketplace_bids`** (proxy `max_cents` vs
    visible `amount_cents` Â· status active/outbid/won/lost/retracted Â· is_auto). RLS: a bidder sees their
    own bids + the owner sees all. **`marketplace_place_bid()`** RPC (`SECURITY DEFINER`,
    `SELECT â€¦ FOR UPDATE` row-lock) implements **eBay-style max/proxy bidding**: tiered min increments,
    hidden reserve, own-listing/too-low/ended guards, and **anti-sniping** (a late bid extends the clock).
    PG16-verified idempotent Ã—2 + proxy/reserve/anti-snipe scenarios (Bob 8000 leads at 5000; Cara 6000
    â†’ Bob auto-covers 6100; Cara 12000 â†’ leads 8100; reserve 9000 unmet; anti-snipe extended=true).
  ...
  - Server actions `placeBidAction`/`buyNowAction` (`app/(app)/marketplace/auctions/actions.ts`) â€” bid via
  ...
  - Realtime **`AuctionPanel`** on the item page (ticking countdown Â· current bid Â· reserve state Â·
    proxy-bid input + quick-bid ladder Â· Buy-It-Now Â· live bid history via a `marketplace_bids` Realtime
    channel). Live board **`/marketplace/auctions`** (stat tiles Â· soonest-ending-first grid Â· countdowns
    Â· reserve/Buy-Now flags) + nav entry (Live Auctions, `Gavel`, additive).
  - **`close-auctions` cron** (`app/api/cron/close-auctions/route.ts`, CRON_SECRET, `*/5 * * * *` in
    `vercel.json`): closes expired auctions â€” winner (reserve met) â†’ confirmed order + bids won/lost +
    notify winner & seller; reserve-not-met â†’ withdrawn + notify. Atomic close (only if still available;
    guards a concurrent Buy-It-Now).
  - Seed **`seed_marketplace_auctions.sql`** (60 auctions + ~456 bids â‰ˆ 516 rows; throwaway "Auction
    Bidders" family so bids are valid cross-family; live/ending-soon/scheduled spread, 20 with reserve,
    15 with Buy-Now, 8 zero-bid lots for the empty state; in `SEED_ALL.sql`). PG16-validated Ã—2 (60 lots
    / 456 bids, idempotent). Verified: tsc Â· eslint Â· **vitest (11 auction)** Â· `next build`.
  - âš ï¸ apply **`0183`** + set **`CRON_SECRET`** in prod (see `docs/PENDING_PROD_MIGRATIONS.md`). Safe
    before apply: the board + panel read best-effort and show nothing until the table exists.

- [x] **Make an Offer â€” Best Offer negotiation âœ… SHIPPED (2026-07-13). The eBay "Best Offer" gap.**
  Today a buyer could send ONE offer and the seller could only accept/decline â€” no counters, no
  thread (Craigslist has none of this). Now it's a real two-sided negotiation. Full slice, realtime,
  atomic, 100% Supabase:
  - Migration **`0186_marketplace_negotiations.sql`** â€” `marketplace_negotiations` (one thread per
    listing+buyer) + `marketplace_negotiation_rounds` (the offer/counter/â€¦ history) + two
    ...
    declines / withdraws, strict turn model). **Accept is atomic** â€” claims the listing, writes a
    ...
    idempotent Ã—2 + full offerâ†’counterâ†’counterâ†’accept flow with own-listing / turn / competing-thread /
  ...
  - Seed **`seed_marketplace_negotiations.sql`** (100 sale listings + 100 threads across every status
    + ~320 rounds + 20 orders â‰ˆ 540 rows; throwaway "Offer Makers" buyer family; in `SEED_ALL.sql`).
    PG16-validated Ã—2 (status distribution + last-round-kind integrity confirmed). Verified: tsc Â·
    eslint Â· **vitest (14 negotiation + 11 auction)** Â· `next build` (`/marketplace/negotiations`).
  - âš ï¸ apply **`0186`** to prod (see `docs/PENDING_PROD_MIGRATIONS.md`). Safe before apply: the panel +
    inbox read best-effort and show nothing until the tables exist.

- [x] **Pickup & Hand-off Coordinator âœ… SHIPPED (2026-07-13). Closes the transaction loop.** After a
  deal is struck (accepted offer / won auction / claimed listing â†’ order), nothing coordinated the
  actual exchange â€” eBay leans on shipping labels, Craigslist on "text me". Now `/marketplace/orders`
  walks it through:
  - Migration **`0190_marketplace_handoffs.sql`** â€” `marketplace_handoffs` (one per order: proposer +
    role Â· meet_at Â· safe-spot label/kind Â· status proposed/confirmed/completed/cancelled Â· confirm_code
    Â· calendar_event link). Family-scoped RLS (4 policies via `is_family_member`), 3 indexes,
    `set_updated_at` trigger, `on delete cascade` from both order and listing. PG16-verified idempotent Ã—2.
  - Pure engine **`lib/marketplace/handoff.ts`** (curated SAFE meetup spots [police safe-exchange zone,
    grocery entrance, libraryâ€¦] Â· sensible meet-time slots Â· hand-off code gen/normalize/match Â·
  ...
  - Server actions `proposeHandoffAction`/`confirmHandoffAction`/`cancelHandoffAction`/
    ...
    **family calendar** + mints a short **hand-off code**); the exchange **completes in person by entering
    ...
    input) â€” both buyer & seller members drive it from their side. Safety copy nudges to public spots.
  - Seed **`seed_marketplace_handoffs.sql`** (160 orders + 160 hand-offs across all four statuses + 40
    backing calendar events â‰ˆ 520 rows; second family member as buyer; in `SEED_ALL.sql`). PG16-validated
    Ã—2 (40 per status). Verified: tsc Â· eslint Â· **vitest (11 handoff + 14 negotiation + 11 auction)** Â·
    `next build`.
  - âš ï¸ apply **`0190`** to prod (see `docs/PENDING_PROD_MIGRATIONS.md`). Safe before apply: the Orders
    page reads best-effort and shows no pickup panel until the table exists.

- [x] **Price History + Drop Watch âœ… SHIPPED (2026-07-13). eBay's "price reduced on an item you're
  watching".** The Saved (â™¥) heart was static â€” a buyer who hearts an item never heard when the seller
  cut the price. Now:
  - Migration **`0191_marketplace_price_history.sql`** â€” `marketplace_price_history` (append-only
    oldâ†’new log) + an **`AFTER UPDATE OF price_cents` trigger** (`SECURITY DEFINER`) that fires on ANY
    edit path (quick-post, module edit, seed, admin): logs the change, and on a **drop** to a still-live
    listing inserts a notification for **every family watching it** (â™¥ via `marketplace_saves`).
    Family-scoped RLS. PG16-verified idempotent Ã—2 + trigger behavior (logs every change; notifies
    exactly on drops â€” a raise is logged but not notified; message formats `$100.00 â†’ $75.00`).
  - Pure engine **`lib/marketplace/price-history.ts`** (drop % Â· total drop Â· lowest-ever + `isAtLowest`
    Â· recent-drop window Â· `priceDropBadge` Â· history lines, **8 tests**). Type added to
    `lib/database.types.ts`.
  - Item page now shows a **"â†“ Price dropped X%"** badge + **"Lowest ever"** flag next to the price and
    an expandable **Price history** list (drops in red).
  - Seed **`seed_marketplace_price_history.sql`** (130 fixed-price listings + declining 2â€“4 step ladders
    â‰ˆ 390 history rows + 65 watcher saves â‰ˆ 585 rows; throwaway "Price Watchers" family; in
    `SEED_ALL.sql`). PG16-validated Ã—2 (ladders end at current price, all latest changes are drops).
    Verified: tsc Â· eslint Â· **vitest (8 price + 11 handoff + 14 negotiation + 11 auction)** Â· `next build`.
  - âš ï¸ apply **`0191`** to prod (see `docs/PENDING_PROD_MIGRATIONS.md`). Safe before apply: the item page
    reads best-effort and shows no badge/history until the table exists.

- [x] **Rent/Borrow Returns + Overdue tracking âœ… SHIPPED (2026-07-13). A family lending library.** The
  marketplace supports `rent`/`borrow` listings and orders carry `ends_on`, but nothing closed the loop â€”
  no "due back Friday" nudge, no overdue flag. Now:
  - Migration **`0192_marketplace_returns.sql`** â€” additive `marketplace_orders` columns
    (`due_reminder_sent_at`, `overdue_notified_at`, `returned_at`) + a partial due-date index for the
    cron. No new table. PG16-verified idempotent Ã—2.
  - Pure engine **`lib/marketplace/returns.ts`** (`returnStatus` upcoming/due_soon/due_today/overdue/
    returned Â· `daysUntilDue` Â· `returnLabel` with tones Â· cron `needsDueReminder`/`needsOverdueAlert`,
  ...
  - Seed **`seed_marketplace_returns.sql`** (250 rent/borrow listings + 250 orders across all five
    return states, 50 each â‰ˆ 500 rows; borrower = 2nd family member; in `SEED_ALL.sql`). PG16-validated
    Ã—2 (even 50-per-bucket spread). Verified: tsc Â· eslint Â· **vitest (8 returns + 8 price + 11 handoff +
    14 negotiation + 11 auction)** Â· `next build`.
  - âš ï¸ apply **`0192`** + ensure **`CRON_SECRET`** in prod (see `docs/PENDING_PROD_MIGRATIONS.md`). Safe
    before apply: the badge/cron no-op until the columns exist (the Orders page still renders).

- [x] **Seller Cockpit â€” "Selling" âœ… SHIPPED (2026-07-13). eBay's Seller Hub.** One command center for
  everything you're selling, **ranked by what needs you** â€” the capstone that surfaces the depth built
  ...
  - Pure engine **`lib/marketplace/selling.ts`** (`attentionItems` â†’ tone-coded chips Â· `attentionScore`
    ranking [overdue â‰« questions â‰« offers-to-reply â‰« pickups â‰« ending-soon â‰« interest] Â· `needsAttention`
    Â· `sellerTotals` roll-up, **9 tests**).
  - Page aggregates per listing (batched, keyed by listing_id): **watchers** (saves), **open offers +
    ...
    **unanswered questions**, **auction bids + ending-soon**, **pickups to confirm** (proposed handoffs),
    and **overdue returns** â€” then ranks the board and shows stat tiles (active Â· needs-attention Â·
    watchers Â· offers Â· questions). Each row deep-links to the item. Nav entry (Selling, `LayoutDashboard`).
  - Family-scoped + member-scoped (your listings only) via RLS. Verified: tsc Â· eslint Â· **vitest (9
    selling + 8 returns + 8 price + 11 handoff + 14 negotiation + 11 auction)** Â· `next build`
    (`/marketplace/selling`). No prod migration needed â€” works the moment the underlying tables exist.

- [x] **Trust & Safety â€” report a listing âœ… SHIPPED (2026-07-13). Safer than Craigslist.** Members can
  flag a listing; the platform super-admin moderates. Pairs with the safe-meetup hand-off to make safety
  a real differentiator:
  - Migration **`0193_marketplace_reports.sql`** â€” `marketplace_reports` (reason prohibited/scam/
    miscategorized/offensive/spam/duplicate/other Â· details Â· status open/reviewing/actioned/dismissed Â·
    ...
    trigger. PG16-verified idempotent Ã—2.
  - Pure engine **`lib/marketplace/reports.ts`** (reason/status vocab Â· `canReport` [not your own] Â·
  ...
  - `reportListingAction` (family-scoped, dedupe-aware) + **ReportButton** dialog on the item page
    ...
    **ReportModeration** controls: start-review Â· action (optionally withdrawing the listing) Â· dismiss
    with a note â€” gated by the `/admin` layout + `isSuperAdmin`, service-role writes.
  - Seed **`seed_marketplace_reports.sql`** (500 reports across all 7 reasons Ã— 4 statuses, one per
    listing; throwaway "Safety Reporters" family; in `SEED_ALL.sql`). PG16-validated Ã—2 (125 per status).
    Verified: tsc Â· eslint Â· **vitest (7 reports + â€¦68 marketplace total)** Â· `next build`
    (`/admin/marketplace/reports`, `/marketplace/item/[id]`).
  - âš ï¸ apply **`0193`** to prod (see `docs/PENDING_PROD_MIGRATIONS.md`). Safe before apply: the Report
    button + queue read best-effort and stay dormant until the table exists.

- [x] **Price Coach â€” "is this a fair price?" âœ… SHIPPED (2026-07-13).** Answers the buyer's core
  question from real comparable listings â€” no LLM key, **no migration** (reads existing listings as
  comps). Closes the todo pricing gap ("is this a fair price?"):
  - Pure engine **`lib/marketplace/price-coach.ts`** (`percentile` Â· `priceBand` [p25/median/p75,
    condition-normalized to a 'good' baseline then re-adjusted, needs â‰¥3 comps] Â· `assessPrice`
    great/good/fair/above-market Â· `dealLabel` Â· `bandSummary`, **9 tests**). Sibling to quick-post's
    single-point `suggestPriceCents` â€” this returns the whole band + a verdict.
  - Item page (fixed-price sale, non-owner) shows a **deal badge** next to the price ("Great price" /
    "Good price" / "Fair price" / "Above similar items") + a **"Similar items: $Xâ€“$Y Â· based on
    comparable listings"** line, from reachable same-category `sell` comps (RLS/circles). Owners and
    auctions don't show it. Verified: tsc Â· eslint Â· **vitest (9 coach + â€¦77 marketplace total)** Â·
    `next build`. No prod migration â€” lights up on existing marketplace data.

- [x] **Deals feed â€” `/marketplace/deals` âœ… SHIPPED (2026-07-13).** Turns the Price Coach from a
  ...
  only) to `lib/marketplace/price-coach.ts` (**+2 tests â†’ 11**). Grid with a **"N% under"** flame badge,
  the verdict, and the "Similar: $Xâ€“$Y" range; nav entry (Deals, `Tag`). Verified: tsc Â· eslint Â·
  **vitest (11 coach)** Â· `next build` (`/marketplace/deals`).

- [x] **All Services â†’ Pin any/ALL services to the sidebar âœ… SHIPPED (2026-07-13).** Owner request +
  bug fix. Two parts:
  - **Bug fix (the visible error toast):** `dashboard_layouts` pin/customize saves threw *"there is no
    unique or exclusion constraint matching the ON CONFLICT specification"* â€” the 0089 unique indexes are
    **partial** (`WHERE scope=â€¦ AND deleted_at IS NULL`), which supabase-js's bare column-list `onConflict`
    ...
    PG16-verified (reproduced the exact error, then both upserts succeed + coexist; idempotent Ã—2).
  - **Feature:** the â­ in **All Services** now pins a service to the **sidebar rail itself**
    (`user_preferences.notification_prefs.sidebarNav`, 100% Supabase via `saveSidebarNavAction`), not the
    Home Quick-Access tiles â€” so pins actually appear in the rail. Keyed by **href**, so **every** in-plan
    ...
    `MAX_SIDEBAR_NAV` raised 20â†’200; pure `addNavKeys`/`removeNavKeys` helpers (**+5 tests**). Live sync
    across the desktop rail + mobile drawer via the `SIDEBAR_NAV_EVENT` broadcast + localStorage cache.
    Verified: tsc Â· eslint Â· **vitest (18 nav-customize)** Â· `next build`.
  - âš ï¸ apply **`0195`** to prod (see `docs/PENDING_PROD_MIGRATIONS.md`) â€” coupled with this deploy.

### 2. Wallet â€” "Full family financial OS"  â— (already wired)
...
- [ ] Spending cards / real-time balance (`money-cards-view.tsx`, `child-detail-view.tsx`) say "coming soon" â€” needs Stripe Issuing (legit future infra; keep honest until it lands).

### 3. Allowance â€” "AI allowance coaching"  â˜‘ DONE
Has `allowance_rules`, `lib/wallet/allowance.ts`, `lib/wallet/coach.ts`; surfaced under `/wallet`.
- [x] Confirmed: allowance schedule posts to the immutable ledger (cron `/api/cron/wallet-allowance` â†’
  `creditChildWallet`, split-allocated, advances `next_run_on`) and the AI coach reads real ledger balances.
- [x] **Parent control added:** `runDueAllowancesAction()` â€” a "Run due now" banner on `/wallet/allowance`
...
### 4. AI Concierge â€” "Become category defining"  â— (already wired)
`/dashboard/concierge`, `lib/concierge/digest.ts`; module has 6 `.from()` + realtime. No mock data.
- [x] **Deeper write-back of accepted recommendations âœ… SHIPPED (2026-07-10).** An accepted concierge
  plan now materializes across MORE surfaces than the calendar â€” **calendar event Â· reminder Â· prep
  task** â€” via `applyConciergePlanAction`, each logged to **`concierge_plan_actions`** (`0158`,
  ...
  Verified: tsc Â· eslint Â· **vitest (7)** Â· `next build`; migration + seed PG16-validated (idempotent).
  âš ï¸ apply `0158` to prod.
- [x] **Onboarding deep dive â€” lifecycle + marketing-signal layer âœ… SHIPPED (2026-07-11).** Deep
  ...
  added â€” fixed; (2) the `onboardingComplete` flag was **write-only dead data** read nowhere;
  (3) the huge **auto-provisioned cohort** (users `ensureActiveFamily` gives a space to, skipping the
  wizard â€” no questionnaire/goals/**no marketing profile**) was **invisible** with no path to finish;
  (4) **value-step engagement** (imported? events? time saved â€” the strongest activation signal) never
  ...
  **`onboarding_progress`** table (`0159`, one row per account, self-scoped RLS) is the queryable
  ...
  score + what's-left + questionnaire against the EXISTING family â€” never creates a second one) +
  **`resetOnboardingAction`**. Lifecycle rows now come only from real Auth users;
  ...
  inserts violated GoTrue invariants. Verified: tsc Â· **vitest (20: 9 completeness
  + 11 funnel)**; migration PG16-validated (idempotent Ã—2; cascade, CHECK, unique,
  trigger, RLS confirmed). âš ï¸ apply `0159` to prod.

### 5. Family Memory â€” "Build persistent family knowledge graph"  â˜‘ DONE
`/dashboard/family-memory` + `/dashboard/family-knowledge-graph`, `lib/memories/*`.
- [x] Persistent store shipped â€” **Family Knowledge Base**: migration `0123_family_facts.sql` (family-scoped RLS; sizes/allergies/contacts/preferences/accounts, member-tagged or family-level, pinnable), pure `lib/memory/facts.ts` (8 tests: filter/search/group), `components/modules/knowledge-base-module.tsx` at `/dashboard/knowledge` (search + member/category filters, add/edit/pin/copy/delete). Nav: Family AI OS, `Brain` icon. Validated on PG16; verified tsc/eslint/1637 tests/build. *(The `/dashboard/family-knowledge-graph` visualization can now read this store.)*
- [x] **Learning layer shipped** â€” the **Family Playbook** (North Star slice 6, see above) mines real household usage into suggested facts the family confirms into this same `family_facts` store. `/dashboard/playbook` + `0126_family_playbook.sql`.

### 6. Voice Control â€” "Full conversational interface"  â˜‘ DONE
- [x] `lib/voice/command-router.ts` â€” wake-word stripping + intent routing â†’ capture kind, reusing `suggestKind`/`parseEvent`. Tests `tests/voice-command-router.test.ts` (12 cases).
- [x] Migration `0121_voice_commands.sql` â€” family-scoped RLS history log. Validated on PG16 (RLS, 4 policies, check constraint, idempotent).
- [x] `components/modules/voice-module.tsx` â€” Voice Command Center: `useSpeechRecognition` mic + editable transcript + live routing preview â†’ `saveCapture` (real rows) â†’ logs `voice_commands` â†’ toast w/ Undo. Recent-commands history (realtime) with one-tap re-run + delete. Graceful type-only fallback when Web Speech unsupported.
- [x] Route `/dashboard/voice` + nav (Family AI OS, `Mic` icon; auto-included in NAV_CATALOG / Navigation Choices).
- [x] **Test seed** `supabase/seed_voice_one_family.sql` + `db:seed:voice` â€” 500 `voice_commands` for the
...
### 7. Phone Concierge â€” "AI receptionist for families"  â— (already wired)
`/dashboard/front-desk`; module has 8 `.from()` + realtime. Wired.
- [x] **OUTBOUND counterpart â€” "Bubaly calls FOR you" (closes opportunity-table gap #2: AI makes phone calls on behalf of families).** Full vertical slice, mobile-first, realtime, 100% Supabase:
  - Schema `0173_concierge_calls.sql` â€” `concierge_calls` (task_kind Â· callee Â· goal Â· details/brief jsonb Â· full lifecycle status Â· outcome/transcript/duration/attempts/provider_ref). Family-scoped RLS (4 policies via `is_family_member`), 3 indexes, `set_updated_at`. PG16-verified idempotent Ã—2.
  - Engine `lib/concierge-calls/brief.ts` â€” pure `buildCallBrief()` â†’ opening Â· key points Â· per-task-kind questions Â· success criteria Â· fallback (never throws). **7 tests pass.** Types/labels/tones exported.
  ...
  - Telephony **provider-gated** (`TWILIO_*`): `/api/concierge-calls/place` (CRON_SECRET) places queued calls when a provider is configured, else honestly flips to `action_needed` â€” usable end-to-end without a phone provider, lights up when one is added.
  - Seed `seed_concierge_calls.sql` (**500 rows**, every task_kind Ã— status, briefs on every row) + added to `SEED_ALL.sql`. Idempotent Ã—2 on PG16.
  - Linked from **AI Front Desk** (inbound â†” outbound), no global-nav change (standing rule). Verified tsc/eslint/vitest.

### 8. Email Concierge â€” "AI inbox management"  â— (already wired)
`/dashboard/inbox` Communications Hub; module has 10 `.from()` + realtime. Wired.

### 9. Predictive Planning â€” "Recommend next best actions"  â— (wired via Moments)
`lib/opportunities/deadlines.ts` + Moments engine surface next-best-actions on Home. Wired.
- [x] Dedicated "Next Best Actions" feed page â€” `lib/opportunities/next-actions.ts` (pure, 7 tests: bucketing overdueâ†’someday, priority tie-break, human reasons) + `components/modules/next-actions-module.tsx` merging real events + open tasks + open opportunities into one ranked worklist at `/dashboard/next-best-actions` (nav: Suggested, `Target` icon). Inline task-complete. No migration (reads existing tables). Verified tsc/eslint/1629 tests/build.

### 10. AI Automation â€” "Multi-step autonomous workflows"  â— (already wired)
`/dashboard/family-automation`, `/dashboard/autopilot`, `lib/autopilot/engine.ts`. Wired.
- [x] #4 Fold â‰¥90%-confidence moment-prep steps into the Autopilot engine (reversible auto-exec) â€” `momentPrepSuggestions(snapshot)` in `lib/autopilot/engine.ts` reuses the shared `buildMomentPrep` to emit, for imminent (â‰¤2d) moments, the two SAFE reversible steps at auto-tier: a **leave-by reminder** (conf 92 â†’ reuses the existing `create_reminder` executor) and a **snacks/supplies grocery list** (conf 91 â†’ new reversible `add_groceries` executor in `scan.ts` that inserts grocery_items). Weather/packing/photo stay as on-screen suggestions. 5 new engine tests (36 total). Verified tsc/eslint/1649 tests/build.

### 11. Home Management â€” "Maintenance and inventory automation"  â— (already wired)
`/dashboard/home`, `lib/home/maintenance.ts`, `lib/home/devices.ts`; module has 10 `.from()`. Wired.

### 12. Vehicle Management â€” "AI maintenance scheduling"  â— (already wired)
`vehicles` + `vehicle_registrations/inspections` (0037), `/dashboard/auto/vehicles`, `lib/auto/*`. Wired.

### 13. Family Operating System â€” "Own this positioning"  (meta)
- [x] Cross-feature cohesion (nav/customization): audited `NAV_CATALOG` (only `minLevel 0` items are customizable in Settings â†’ Navigation Choices). This session's Knowledge Base + Next Best Actions were already free (0); per user decision, **Marketplace + Voice Control lowered to `minLevel 0`** so all four new features are free-tier and selectable in Navigation Choices (the pages were never plan-gated anyway).

---

## â˜… "Largest opportunities" table audit (owner screenshot, 2026-07-12)
Compared all 10 opportunities against the codebase. **8 already shipped** (life-admin autopilot Â·
knowledge graph Â· digital twin Â· verified marketplace Â· connected services Â· proactive agents Â·
approvals Â· unified identity â€” plus AI-drafted replies via inbox `generateReply`). **Gap #2
(outbound AI phone calls)** is now BUILT this session â€” see roadmap #7 OUTBOUND counterpart above.
- [ ] **Remaining gap #10 â€” Open developer platform / third-party app ecosystem** (public API, OAuth
  app registration, webhooks-out, marketplace of integrations). This is a **strategic + human-owned**
  decision (API surface, partner program, security review) rather than a single agent-buildable slice â€”
  surfaced here, not silently built. All the internal engines it would expose are already Supabase-wired.

## â˜… "Missing Competitor Features" matrix (owner screenshot, 2026-07-12) â€” full 28-row audit
Compared every row against the codebase (Cozi/Skylight/FamilyWall/TimeTree/Hearth/Maple/Ohai/
Family Assistant/OurHome benchmark). Verdicts grounded in actual routes/libs, not the table's guesses â€”
several "No" rows were already built here.

**Already built (table said Partial/No â€” verified in code):**
| # | Feature | Where it lives |
|---|---|---|
| 1 | Natural-language family planning | âŒ˜K command bar + `lib/capture` parser + Voice + assistant (create events/reminders/items from one sentence) |
...
| 8 | **Photo-to-calendar** (table said No) | `/dashboard/scan` + `/api/ai/flyer` â€” AI extracts events from a photographed flyer |
...
**Genuine gaps â†’ BUILT THIS SESSION (2026-07-12):**
- [x] **#3+#4 Household workload balancing + family workload analytics** (Hearth) â€” `/dashboard/workload`:
...
- [x] **#16 Calendar heat maps** (TimeTree) â€” `lib/calendar/heatmap.ts` (pure busyness engine over the
  recurrence-expanded events, 0â€“4 levels, overload detection + advice) + Busyness card in Calendar. No new
  table â€” rides the already-seeded 500 events.
- [x] **#19 Child independence progression** (Hearth) â€” `/dashboard/independence`: age-banded milestone
...
- [x] **#13 Offline mode â€” v1 (read-side) SHIPPED** (FamilyWall's headline behavior: "local caching
  ...
  (offline amber / "back online â€” syncing" flash); **cache wiped on sign-out** (privacy).
  - [ ] v2 (owner decision still): write-behind queue for offline MUTATIONS + conflict resolution â€”
    genuinely infrastructural, per-module opt-in.
- [ ] **#28 Family operating system API** â€” same as opportunity-table #10 (open developer platform):
...
- [x] #2 Universal âŒ˜K natural-language command bar â€” DONE. `lib/command-bar/route.ts` (pure, 8 tests) +
  `components/app/command-bar.tsx` (global palette in app-shell, âŒ˜K / "/" open, navigate/capture/assistant,
  reuses the Voice/capture parser). See the "âŒ˜K + ðŸ’¸" handoff block.
- [ ] #6 ETA-based travel buffer for leave-by (needs a maps/ETA API key).
- [x] #9 Admin-mobile table overflow â€” DONE. Audited every `<table>` under `app/(app)/admin` (27 total):
...
- [x] #7 Time-of-day Home Mission Control â€” DONE. `lib/home/time-of-day.ts` (pure, 6 tests: dayPhase /
  phaseGreeting / phaseBlurb / focusForPhase) + `components/home/time-of-day-focus.tsx` "Focus now" strip
  at the top of `/home` (morning: schedule/weather/school Â· night: tomorrow/get-ready/reflect). Additive â€”
  did not refactor the contended grid. Server-time based (no per-user tz yet).
- [x] #8 Role-tailored surfaces â€” **slices 1â€“4 shipped** (2026-07-10): (1) pure `lib/ui/role-surface.ts` (10 tests) â€”
  `roleSurface(role)` â†’ { density, tone, canManage, focusMax } + `roleGreeting`/`focusHeadline` â€” applied
  ...
  Services, and mobile tabs (super-admins still see them) â€” the page guards already redirect those roles,
  so the links were dead-ends. (3) **Density rollout**: pure `focusChipClasses(role)` (3 tests) maps
  `roleSurface().density` â†’ chip sizing, wired into the Home "Focus now" strip â€” kids ('playful') get
  ...
  `roleGreeting` now also leads the **personal dashboard** (parents formal Â· adults casual Â· kids a warm
  emoji line), extending the treatment beyond Home to a second primary surface. tsc/eslint/build green.
  **Slice 5 â€” APP-WIDE density (2026-07-12):** density was per-chip only; now it scales the *whole app*.
  `<RoleDensity/>` (mounted in AppShell) stamps `data-density` on `<html>` from the member's role, and
  a globals.css block scales the root font size (Tailwind rem cascades â†’ text + padding + gaps together):
  parents 100% Â· teens/guests 104% Â· kids 110%. Added `resolveDensity` + `DENSITY_FONT_PCT`/labels to
  `lib/ui/role-surface.ts` (+7 tests) and a **Settings â†’ Display comfort** control (`display-comfort.tsx`,
  Auto/Standard/Cozy/Relaxed override, localStorage, instant apply). No migration â€” client pref + CSS.
- [x] #10 Recurring-routine templates â€” **DONE**. Migration `0122_routine_templates.sql`
  ...
  â‰¥3 weeks; `materializeRoutine` expands a template â†’ concrete `calendar_events`) +
  `components/modules/routines-panel.tsx` in the calendar right rail (detect â†’ "Save as routine",
...
- [x] Onboarding â†’ first value (audited + first win): the **PIN step was mandatory** (Continue disabled until a 4-digit PIN matched), forcing every new user through 2 extra fields + validation before reaching the app â€” even though `completeProfileOnboardingAction` already treats an absent PIN as valid. Made it **"Skip for now"** (defers PIN to Settings â†’ App Lock). Onboarding is now 1 required field (name) â†’ skip â†’ done. *(Onboarding telemetry doesn't fit `journey_events`: no family_id exists until completion â€” would need an anonymous/pre-family analytics path.)*

## â˜… Seed coverage â€” 500-record test data per feature (2026-07-05)

> **Goal:** every feature testable at volume (â‰¥500 rows). The DB has **354 base tables**;
...
**âœ… DONE â€” secondary user-facing tables now seeded at 500 rows** (all validated on PG16,
idempotent, stable across re-runs):
- [x] Messages â€” `family_messages` (all kinds) â†’ `seed_messages.sql`
- [x] Chores â€” `chores` + `chore_assignments` (full status lifecycle) â†’ `seed_chores.sql`
- [x] Meals â€” `meals` + `family_recipes` + `meal_plans` â†’ `seed_meals.sql`
- [x] Documents â€” `documents` (categories + expiry + secure) â†’ `seed_documents.sql`
- [x] Location / Safety â€” `family_places` + `member_locations` + `location_events` â†’ `seed_location.sql`
- [x] Finance hub â€” `financial_accounts` + `transactions` + `bills` â†’ `seed_finance.sql`
- [x] Memories / trips â€” `family_memories` + `trip_memories` â†’ `seed_memories.sql`
- [x] Autopilot â€” `autopilot_suggestions` + `approval_requests` â†’ `seed_autopilot.sql`
- [x] Family Vault â€” `family_credentials` (all 9 categories, migration 0119 applied + validated) â†’ `seed_vault.sql`

**â˜‘ One-paste master runner:** `supabase/SEED_ALL.sql` runs all 46 seeds in dependency order
(one paste â†’ every surface at â‰¥500 rows). Idempotent; validated on PG16.

**â˜‘ Validated 500-row seeds (paste-ready in `supabase/`):**
...
| Marketplace | `marketplace_listings`, `marketplace_offers` | `lib/marketplace/seed-sql.ts` â†’ `/dashboard/marketplace/seed` |
| Voice Control | `voice_commands` | `seed_voice_one_family.sql` |
| Wallet / Allowance | child ledger | `seed_wallet_ledger_one_family.sql` |

> **Honest note:** seeding + validating *all* 354 tables in one pass isn't feasible â€” most are
...
- [ ] **Messages â†’ GIF picker** (FUTURE FOLLOW-UP â€” needs an external provider key). Stub:
  `components/modules/messages-module.tsx` (~line 876) â€” the "GIF" button toasts "coming soon".
  **Implementation plan** (code-complete + key-gated, mirror the Family-API pattern):
  1. Pick a provider â€” **Giphy** or **Tenor** (both have free tiers + search endpoints).
  2. Add the key to env: `GIPHY_API_KEY` (server) â€” proxy GIF search through a route
     (`app/api/gif/search/route.ts`) so the key never ships to the client.
  3. Build a `GifPicker` popover (query input â†’ grid of results â†’ pick â†’ send as a `family_messages`
...
- **2026-07-08 â€” One-click demo ("Test Account").** Pricing-page button â†’ a throwaway
...
- **2026-07-06 (eve) â€” onboarding + role-tailoring + a11y run (7 PRs).** #235 onboardingâ†’marketing
...
- **Voice Control** shipped (migration 0121, `lib/voice/command-router.ts`+12 tests, `voice-module.tsx`, `/dashboard/voice`, nav). Web Speech â†’ route â†’ real capture rows + `voice_commands` history. Verified: tsc/eslint/1578 tests/build. **+ 500-row `voice_commands` seed** (`seed_voice_one_family.sql` / `db:seed:voice`, PG16-validated).
- **Universal âŒ˜K command bar** (Friction #2) shipped: `lib/command-bar/route.ts` (pure, 8 tests) + `components/app/command-bar.tsx` (global palette in app-shell; âŒ˜K / "/" open; navigate/capture/assistant; reuses the Voice/capture parser). 1588 tests / tsc / eslint / build green.
...
- **Time-of-day Home Mission Control** (Friction #7) shipped: `lib/home/time-of-day.ts` (pure, 6 tests) + `components/home/time-of-day-focus.tsx` "Focus now" strip at the top of `/home`, phase-adaptive (morning: schedule/weather/school Â· night: tomorrow/get-ready/reflect). Additive; 1599 tests / tsc / eslint / build green.
- **Recurring-routine templates** (Friction #10) shipped: migration `0122`, `lib/routines/detect.ts`
  (13 tests), `RoutinesPanel` in the calendar rail (detect â†’ save â†’ apply-to-week w/ Undo). Commit on
...
- **Next up:** all 12 roadmap features built + wired + seeded. Remaining agent-doable Friction Backlog:
  **#8 role-tailored surfaces** (density/language per `family_members.role`; `ui`) Â· **#4 Autopilot
  self-completion of â‰¥90%-confidence moment-prep** (`engine`, touches shared `lib/autopilot/*`) Â· the
  **onboarding time-to-value < 90s** audit. Blocked/human-owned: #6 ETA travel buffer (maps API key),
  Stripe Issuing cards, applying pending prod migrations (0118/0120/0121â€¦) in the Supabase SQL editor,
...
## ðŸŽ¨ SITE-WIDE UI/UX AUDIT â€” every dashboard page, graded (2026-07-12, this session)
...
bottom cohort. Tiers: **â˜…â˜…â˜… world-class** (rich module, full states, stats/AI/realtime,
mobile-first) Â· **â˜…â˜… solid production** (wired + stated + clean; missing one premium layer,
usually by design) Â· **â˜… upgrade** (thin for its traffic â€” rebuilt this session) Â· **â†ª redirect**.

| Page | Tier | Notes |
|---|---|---|
| `/dashboard/activity` | â˜…â˜… | Server-merged cross-surface feed; wired, clean. Realtime not needed (snapshot feed) |
| `/dashboard/agents` | â˜…â˜… | 5/9 signals, 297 LoC UI |
| `/dashboard/announcements` | â˜…â˜… | 7/9 signals, 183 LoC UI |
| `/dashboard/app-store` | â˜…â˜… | 6/9 signals, 197 LoC UI |
| `/dashboard/assistant` | â˜…â˜… | 7/9 signals, 711 LoC UI |
| `/dashboard/auto` | â˜…â˜… | 7/9 signals, 258 LoC UI |
| `/dashboard/autonomous-family-management` | â˜…â˜… | 6/9 signals, 347 LoC UI |
| `/dashboard/autopay` | â˜…â˜… | 6/9 signals, 182 LoC UI |
| `/dashboard/autopilot` | â˜…â˜…â˜… | 8/9 signals, 272 LoC UI |
| `/dashboard/behavior` | â˜…â˜…â˜… | 8/9 signals, 263 LoC UI |
| `/dashboard/billing` | â˜…â˜…â˜… | 8/9 signals, 2237 LoC UI |
| `/dashboard/bills` | â˜…â˜… | 6/9 signals, 182 LoC UI |
| `/dashboard/binder` | â˜…â˜… | 6/9 signals, 127 LoC UI |
| `/dashboard/briefing` | â˜…â˜…â˜… | 8/9 signals, 979 LoC UI |
| `/dashboard/budgets` | â˜…â˜… | 7/9 signals, 126 LoC UI |
| `/dashboard/calendar` | â˜…â˜…â˜… | 8/9 signals, 905 LoC UI |
| `/dashboard/calm` | â˜…â˜… | Calm inbox; engine-driven minimal by design |
| `/dashboard/care` | â˜…â˜…â˜… | 8/9 signals, 305 LoC UI |
| `/dashboard/celebrations` | â˜…â˜… | 7/9 signals, 156 LoC UI |
| `/dashboard/chores` | â˜…â˜…â˜… | 8/9 signals, 787 LoC UI |
| `/dashboard/command-center` | â˜…â˜… | TV/large-screen dashboard; wired |
| `/dashboard/concierge-calls` | â˜…â˜…â˜… | 8/9 signals, 305 LoC UI |
| `/dashboard/concierge` | â˜…â˜…â˜… | 8/9 signals, 644 LoC UI |
| `/dashboard/conflicts` | â˜…â˜… | 6/9 signals, 274 LoC UI |
| `/dashboard/connections` | â˜…â˜… | 7/9 signals, 167 LoC UI |
| `/dashboard/contacts` | â˜…â˜…â˜… | 8/9 signals, 538 LoC UI |
| `/dashboard/decisions` | â˜…â˜… | 7/9 signals, 398 LoC UI |
| `/dashboard/dental` | â˜…â˜…â˜… | 8/9 signals, 726 LoC UI |
| `/dashboard/devices` | â˜…â˜… | 7/9 signals, 139 LoC UI |
| `/dashboard/dining` | â˜… | UPGRADE: read-only, zero interactivity, dining_out has NO seed |
| `/dashboard/documents` | â˜…â˜…â˜… | 8/9 signals, 678 LoC UI |
| `/dashboard/due` | â˜…â˜… | 6/9 signals, 182 LoC UI |
| `/dashboard/expenses` | â˜…â˜…â˜… | 8/9 signals, 250 LoC UI |
| `/dashboard/experience` | â˜…â˜… | 6/9 signals, 229 LoC UI |
| `/dashboard/family-access` | â˜…â˜… | 4/9 signals, 187 LoC UI |
| `/dashboard/family-ai-assistant` | â†ª | Redirects to Assistant (de-dup, by design) |
| `/dashboard/family-automation` | â˜…â˜… | 7/9 signals, 468 LoC UI |
| `/dashboard/family-cfo` | â˜…â˜… | 4/9 signals, 230 LoC UI |
| `/dashboard/family-coo` | â˜…â˜… | 7/9 signals, 480 LoC UI |
| `/dashboard/family-digital-twin` | â˜…â˜…â˜… | 8/9 signals, 799 LoC UI |
| `/dashboard/family-emergency` | â˜…â˜… | 7/9 signals, 460 LoC UI |
| `/dashboard/family-health` | â˜…â˜… | 4/9 signals, 218 LoC UI |
| `/dashboard/family-knowledge-graph` | â†ª | Redirects to Graph (de-dup, by design) |
| `/dashboard/family-memory` | â†ª | Redirects to Memories (R4 de-dup, by design) |
| `/dashboard/family-operating-index` | â˜…â˜… | 4/9 signals, 366 LoC UI |
| `/dashboard/family-operations` | â˜…â˜… | 4/9 signals, 216 LoC UI |
| `/dashboard/family-school` | â˜…â˜… | 4/9 signals, 216 LoC UI |
| `/dashboard/family-signals` | â˜…â˜… | Signal cards + ack/dismiss; engine-driven |
| `/dashboard/family-sports` | â˜…â˜… | 4/9 signals, 220 LoC UI |
| `/dashboard/family-stress` | â˜…â˜… | 7/9 signals, 379 LoC UI |
| `/dashboard/family-tree` | â˜…â˜… | 7/9 signals, 422 LoC UI |
| `/dashboard/family` | â˜…â˜… | 7/9 signals, 597 LoC UI |
| `/dashboard/favorites` | â˜…â˜… | 7/9 signals, 150 LoC UI |
| `/dashboard/focus` | â˜…â˜… | 6/9 signals, 165 LoC UI |
| `/dashboard/food` | â˜…â˜… | 5/9 signals, 218 LoC UI |
| `/dashboard/front-desk` | â˜…â˜…â˜… | 9/9 signals, 769 LoC UI |
| `/dashboard/goals` | â˜…â˜…â˜… | 8/9 signals, 246 LoC UI |
| `/dashboard/grandparent-portal` | â˜…â˜… | Digest-engine server page; warm + wired |
| `/dashboard/graph` | â˜…â˜… | 7/9 signals, 413 LoC UI |
| `/dashboard/grocery` | â˜…â˜… | 7/9 signals, 475 LoC UI |
| `/dashboard/habits` | â˜…â˜… | 7/9 signals, 405 LoC UI |
| `/dashboard/health` | â˜…â˜…â˜… | 8/9 signals, 906 LoC UI |
| `/dashboard/home` | â˜…â˜…â˜… | 8/9 signals, 566 LoC UI |
| `/dashboard/homework` | â˜…â˜… | 7/9 signals, 281 LoC UI |
| `/dashboard/inbox` | â˜…â˜…â˜… | 9/9 signals, 802 LoC UI |
| `/dashboard/independence` | â˜…â˜… | 5/9 signals, 251 LoC UI |
| `/dashboard/insurance` | â˜…â˜…â˜… | 8/9 signals, 352 LoC UI |
| `/dashboard/intelligence` | â˜…â˜… | 6/9 signals, 224 LoC UI |
| `/dashboard/journal` | â˜…â˜… | 7/9 signals, 257 LoC UI |
| `/dashboard/journeys` | â˜…â˜… | 4/9 signals, 214 LoC UI |
| `/dashboard/kitchen` | â˜…â˜… | 7/9 signals, 612 LoC UI |
| `/dashboard/knowledge` | â˜…â˜… | 6/9 signals, 249 LoC UI |
| `/dashboard/life-events` | â˜…â˜…â˜… | 8/9 signals, 319 LoC UI |
| `/dashboard/locator` | â˜…â˜… | 7/9 signals, 533 LoC UI |
| `/dashboard/meals` | â˜…â˜…â˜… | 8/9 signals, 821 LoC UI |
| `/dashboard/medical` | â˜…â˜…â˜… | 8/9 signals, 913 LoC UI |
| `/dashboard/medications` | â˜…â˜… | 7/9 signals, 521 LoC UI |
| `/dashboard/memories` | â˜…â˜… | 6/9 signals, 451 LoC UI |
| `/dashboard/messages` | â˜…â˜… | 7/9 signals, 1397 LoC UI |
| `/dashboard/migrate` | â˜…â˜… | 6/9 signals, 318 LoC UI |
| `/dashboard/moments` | â˜…â˜…â˜… | 8/9 signals, 437 LoC UI |
| `/dashboard/money-timeline` | â˜…â˜… | Timeline module; wired |
| `/dashboard/more` | â˜…â˜… | Deliberate simple links hub (mockup screen 11); premium enough for its job |
| `/dashboard/next-best-actions` | â˜…â˜… | 7/9 signals, 196 LoC UI |
| `/dashboard/notes` | â˜…â˜…â˜… | 8/9 signals, 513 LoC UI |
| `/dashboard/notifications` | â˜…â˜… | 7/9 signals, 270 LoC UI |
| `/dashboard/nutrition` | â˜…â˜… | 7/9 signals, 161 LoC UI |
| `/dashboard/onboarding-funnel` | â˜…â˜… | 4/9 signals, 253 LoC UI |
| `/dashboard/outcomes` | â˜…â˜… | Outcome launcher picker; engine-driven, purposeful |
| `/dashboard/pantry` | â˜…â˜…â˜… | 9/9 signals, 318 LoC UI |
| `/dashboard/paperwork` | â˜…â˜… | 4/9 signals, 305 LoC UI |
| `/dashboard/passwords` | â˜…â˜… | 7/9 signals, 323 LoC UI |
| `/dashboard/payments` | â˜…â˜… | 6/9 signals, 100 LoC UI |
| `/dashboard/pets` | â˜…â˜…â˜… | 8/9 signals, 419 LoC UI |
| `/dashboard/photos` | â˜…â˜…â˜… | 8/9 signals, 615 LoC UI |
| `/dashboard/planning` | â˜…â˜… | Server planning surface; wired |
| `/dashboard/playbook` | â˜…â˜…â˜… | 9/9 signals, 264 LoC UI |
| `/dashboard/prep-plans` | â˜…â˜… | 7/9 signals, 223 LoC UI |
| `/dashboard/profile` | â˜… | UPGRADE: thin links menu on a high-traffic surface â€” no personal stats/identity |
| `/dashboard/readiness` | â˜…â˜… | 4/9 signals, 253 LoC UI |
| `/dashboard/reasoning` | â˜…â˜… | Unified reasoning view; engine-driven |
| `/dashboard/recipes` | â˜…â˜…â˜… | 9/9 signals, 739 LoC UI |
| `/dashboard/relationship` | â˜…â˜…â˜… | 8/9 signals, 603 LoC UI |
| `/dashboard/reminders` | â˜…â˜…â˜… | 9/9 signals, 897 LoC UI |
| `/dashboard/renewals` | â˜…â˜… | 7/9 signals, 295 LoC UI |
| `/dashboard/rewards` | â˜…â˜…â˜… | 8/9 signals, 343 LoC UI |
| `/dashboard/rides` | â˜…â˜… | 7/9 signals, 327 LoC UI |
| `/dashboard/savings` | â˜…â˜… | 7/9 signals, 148 LoC UI |
| `/dashboard/scan` | â˜…â˜… | 6/9 signals, 203 LoC UI |
| `/dashboard/school` | â˜…â˜…â˜… | 9/9 signals, 653 LoC UI |
| `/dashboard/screen-time` | â˜…â˜…â˜… | 8/9 signals, 244 LoC UI |
| `/dashboard/security` | â˜… | UPGRADE: bare CRUD event list â€” no stat tiles, no severity summary, no AI |
| `/dashboard/settings` | â˜…â˜… | 6/9 signals, 464 LoC UI |
| `/dashboard/setup` | â˜…â˜… | 7/9 signals, 378 LoC UI |
| `/dashboard/signups` | â˜…â˜…â˜… | 8/9 signals, 320 LoC UI |
| `/dashboard/social-feed` | â˜…â˜… | 7/9 signals, 448 LoC UI |
| `/dashboard/social` | â˜…â˜… | 7/9 signals, 333 LoC UI |
| `/dashboard/sports` | â˜…â˜…â˜… | 9/9 signals, 468 LoC UI |
| `/dashboard/subscriptions` | â˜…â˜…â˜… | 8/9 signals, 176 LoC UI |
| `/dashboard/sync` | â˜…â˜… | 5/9 signals, 218 LoC UI |
| `/dashboard/tax-vault` | â˜…â˜… | 6/9 signals, 165 LoC UI |
| `/dashboard/timetable` | â˜…â˜… | 7/9 signals, 261 LoC UI |
| `/dashboard/todos` | â˜…â˜…â˜… | 8/9 signals, 677 LoC UI |
| `/dashboard/trip-intel` | â˜…â˜… | 6/9 signals, 739 LoC UI |
| `/dashboard/trip-memories` | â˜…â˜… | 7/9 signals, 192 LoC UI |
| `/dashboard/trips` | â˜…â˜… | 7/9 signals, 425 LoC UI |
| `/dashboard/trust` | â˜…â˜… | 7/9 signals, 848 LoC UI |
| `/dashboard/utilities` | â˜…â˜…â˜… | 8/9 signals, 211 LoC UI |
| `/dashboard/vacations` | â˜…â˜…â˜… | 8/9 signals, 204 LoC UI |
| `/dashboard/voice` | â˜…â˜… | 6/9 signals, 254 LoC UI |
| `/dashboard/voting` | â˜…â˜… | 6/9 signals, 354 LoC UI |
| `/dashboard/weather` | â˜…â˜… | 7/9 signals, 335 LoC UI |
| `/dashboard/weekend` | â˜…â˜…â˜… | 8/9 signals, 271 LoC UI |
| `/dashboard/weekly-briefing` | â˜…â˜… | 7/9 signals, 443 LoC UI |
| `/dashboard/wishlists` | â˜…â˜…â˜… | 8/9 signals, 256 LoC UI |
| `/dashboard/workload` | â˜…â˜… | 5/9 signals, 289 LoC UI |

**Tally:** 43 world-class Â· 87 solid-production Â· 3 upgraded this session Â· 3 redirects.

**Upgrades built this session (all with 500-seed where a table backs them):**
- [x] **Profile** â†’ premium identity surface: avatar hero, personal stats (points, chores done,
  streak, upcoming events), theme toggle + quick links preserved.
- [x] **Security** â†’ stat tiles (open/critical/7-day), severity timeline styling, AI-style rollup
  headline from `summarizeSecurity`; CRUD preserved.
- [x] **Dining Out** â†’ interactive: favorite toggle + log-a-visit (server actions), stat tiles,
  `seed_dining_out.sql` (500 rows: restaurants + visits).

### ðŸŽ¨ Audit second pass â€” â˜…â˜…/â˜… upgrade sweep (2026-07-12, same session)
Owner directive: upgrade every â˜…/â˜…â˜… page. Every low-scoring page was manually re-reviewed:
- [x] **Payments** (`/dashboard/payments`) â€” UPGRADED: "this month" stat tiles (in/out/net/count,
...
- [x] **Family Intelligence** (`/dashboard/family-signals`) â€” UPGRADED: at-a-glance strip
  (active patterns Â· avg confidence Â· most-common kind Â· handled count) above the signal cards.
- **Re-graded â˜…â˜…â†’â˜…â˜…â˜… on manual review** (the signal scan under-scored engine-driven server pages â€”
  ...
  (5-table inbox; minimal ON PURPOSE â€” its design brief is "does not maximize engagement", so no
...
- **Left â˜…â˜… by design** (verified, do NOT chrome-inject): `more` (menu mockup), binder/devices/
  focus/tax-vault (grouping/reveal/status-cycling already present), finance re-export routes.

### ðŸª¨ Full stone-turn pass (2026-07-12, late) â€” build + links + auth + scoping
- `next build` production build: **PASS** (exit 0, all 136+ routes compile).
- Dead-link sweep (172 distinct internal hrefs): 2 dead â†’ fixed (`/dashboard/routines` â†’
  `/dashboard/calendar` in moments/prep engines; `/dashboard/shopping` â†’ `/dashboard/grocery`).
- **API auth audit (95 routes): 4 real security fixes.** (1â€“2) `guardian/screen` +
  `guardian/status/voicemail` Twilio callbacks accepted unsigned requests (inbound trio validated,
  these didn't) â†’ now validate `x-twilio-signature` in production, full-URL-with-query form.
  (3) `guardian/escalate` **failed open** when `GUARDIAN_INTERNAL_SECRET` unset â€” could blast
  emergency SMS/calls to every parent â†’ fails closed (secret or CRON_SECRET required; no internal
  callers broken â€” verified none exist yet). (4) `guardian/escalate/twiml` was an open
  text-to-TwiML reflector â†’ Twilio-signature-gated. Plus `mkt/track` (service-role public ingest)
  had NO rate limit while every sibling tracker did â†’ 60/min/IP.
...
### ðŸ”— Open-item #3 build-out â€” provider calendar two-way sync (2026-07-12, latest)
Everything up to the OAuth keys is now COMPLETE â€” adding keys lights it up with zero code changes:
- [x] **Provider-generic OAuth routes** `/api/sync/[provider]/auth|callback|disconnect` â€” registry-
...
- [x] **Scheduled background sync** `/api/cron/provider-sync` (CRON_SECRET, every 4h in vercel.json):
  oldest-synced accounts first, bounded batch of 25, per-account audit rows â€” manual "Sync now"
  becomes real continuous two-way sync.
- [x] **UI** â€” Microsoft Connect button (was missing); generic `ProviderControls` (Sync now via
...
- [x] **Env-doc bug fixed**: `.env.example` documented `MICROSOFT_CLIENT_ID/SECRET/TENANT_ID` +
  `/api/microsoft/callback` â€” variables NOTHING reads and a route that doesn't exist. Now documents
...
### ðŸ’³ Open-item #5 build-out â€” Stripe Issuing cards + real-time balances (2026-07-12, latest)
Deep audit verdict: the pipeline was already MUCH deeper than the table said â€” real-time
...
- [x] **Auth-decision unit tests were MISSING** (code claimed "unit-tested separately" â€” no test
  existed): `tests/stripe-issuing-auth.test.ts` (7 tests â€” approve within balance, decline on
...
- [x] **PAN reveal was promised but not built** (issuing.ts header: "parents reveal full details
  via an ephemeral Stripe.js session" â€” nothing existed): `prepareCardRevealAction` +
  `createCardRevealAction` (manager-gated, family-scoped, capability-gated, audited to
  wallet_audit_logs) + `CardRevealModal` (Stripe.js v9 Issuing display Elements â€” nonce â†’
  ephemeral key â†’ number/expiry/CVC render in Stripe-hosted iframes; the PAN never touches our
  servers) + a Reveal button on every card row.
- [x] **Stale "Coming soon Â· real-time balance check"** on the child virtual card â†’ now links to
...
- [x] **`STRIPE_MONEY_WEBHOOK_SECRET` was undocumented** in .env.example (the money webhook reads
  it; falls back to STRIPE_WEBHOOK_SECRET) â†’ documented with the /api/webhooks/money endpoint note.
...
### ðŸ“‹ Master autonomous-build prompt â€” execution record (2026-07-13)
Owner supplied the full-platform "MASTER AUTONOMOUS BUILD PROMPT". Most phases were already
executed across this session's lanes with evidence (Phases 1â€“2: 136-page audit + competitor
...
- [x] **Documentation ledger created (10 files, grounded, generated from the repo where possible):**
  ...
  correct 0118 universal-RLS model), `feature-inventory.md` (133 features â†’ direct table deps),
...
- [x] **Final-validation anti-pattern sweep re-run on the merged tree**: clean â€” no TODO/FIXME
...
- [x] **Fixed a CI-blocking type error a parallel lane introduced**: `vitest.config.ts` carried an
  `oxc` key that isn't in this Vitest version's types â€” every `tsc --noEmit` failed. Removed
  (inert: the test glob is .ts-only); 2508 tests still green.
- Honest deferrals per the prompt's own priority rules: E2E suite (needs CI Supabase login â€”
  open item #24), RLS CI matrix, visual regression â€” scoped in `testing-plan.md`.

### âœ… Open-item #2 (Chief-of-Staff proactive front door) â€” VERIFIED COMPLETE (2026-07-13)
...
- `FrontDoorHero` renders on `/home`; `pending-approvals.tsx` calls the REAL
  `decideApprovalAction` (manager-gated + re-enforced server-side) â€” one-tap Approve/Decline in
  place; "Done for you" links to Autopilot for review/undo.
- Small connective add this pass: a "See everything in Calm â†’" link from the hero to the deeper
  quiet inbox (`/dashboard/calm`), so the home assembly leads onward to the full surface.
Net: every 4â€“5 star agent-buildable item on the owner's open-items table is now built. Remaining
...
- Description: Required service-role reads were destructured without checking errors, so a database
  outage rendered â€œNo reportsâ€ or â€œNo ticketsâ€ and hid operational work.
