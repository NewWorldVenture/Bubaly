...
- Scenario: a planned adapter contract returned â€œnot available yet,â€ was referenced only by its own tests, and duplicated the real `lib/sync/registry.ts` path used by `/api/sync/*`.
