...
| `agent-05` | 2026-07-18 14:35 | **Calendar (A-06 cross-lane, direct user request) PLA-0823**: mini-calendar now highlights the FOCUSED day (`days[mobileDayIndex]`), not the week Monday (was showing the 13th on the 18th); added opt-in **"Side-by-side view"** — focused day splits into one column per visible member (own + shared events), unified `gridCols` render path. Guard `tests/calendar-split-view.test.ts` (5). tsc/eslint/build all green. Commits `2a7f59d5`, `6018db1f`. **Now: authored `docs/MARKETING_PLATFORM_COORDINATION.md` to support codex's marketing buildout (image-uniqueness + Supabase-wiring findings inside).** Prior: A-05 §23 a11y (PLA-0822). | `components/modules/calendar-module.tsx` (done), `docs/MARKETING_PLATFORM_COORDINATION.md` (new) | Resume A-05 §23 a11y after marketing handoff |
