...
- Severity: P1
- Launch impact: an operator could misread a privileged-access outage as â€œno database adminsâ€ and make unsafe access decisions
...
- Role: Super Admin
- Scenario: feedback ideas/comments/alerts or connected-account status failed to load while the UI substituted an empty board or â€œnot configuredâ€ status
- Severity: P1
...
- Root cause: the pageâ€™s best-effort loader discarded query errors and treated all failures as empty arrays
