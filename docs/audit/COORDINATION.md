...
Last board update: **2026-07-17 14:10 UTC** · by `agent-04`
...
| A-06 | `agent-01` | IN_REVIEW | 2026-07-16 21:58 | 2026-07-16 22:02 | `PLA-0480` | VERIFIED: CRUD wired; per-op RLS family-scoped (collaborative-by-design, isolation proven PLA-0415); ICS import SSRF-guarded (fetchPublicCalendarText, no raw fetch) + guard test; ICS export token-scoped outbound. Open: recurrence UX, provider sync (A-18), live E2E |
| A-07 | `agent-01` | IN_REVIEW | 2026-07-16 21:22 | 2026-07-17 00:22 | `PLA-0450` | FIXED HIGH sec bug: child self-approve chore→mint reward (approve/reject lacked role gate). isManager gate + guard test. NOTE: DB-level status restriction is intentionally DEFERRED — `submitProofAction` legitimately sets status='approved' via the CHILD session on AI auto-approve (canAutoApprove), so a trigger blocking child→approved would break auto-approval; safe path = route the auto-approve reward write through service-role FIRST, then add the trigger. App gate already closes the manual-approve exploit. Open: that refactor, allowance/points |
| A-08 | `agent-01` | IN_REVIEW | 2026-07-16 21:16 | 2026-07-17 11:45 | `PLA-0440,0580` | Money-integrity: overspend/idempotency (PLA-0440); FOUND+FIXED CRITICAL wallet mint (0217/PLA-0580) + HIGH economy-token mint (0218/PLA-0590) — child could INSERT credits directly (RLS was is_family_member FOR ALL). Both proven live + guarded; economy keeps child redemption-request INSERT. Prod exploitable until 0217/0218 applied (LB-010). |
...
| A-11 | `agent-03` | IN_REVIEW | 2026-07-16 21:24 | 2026-07-17 00:35 | `46556214` | A-11 agent-doable surface COMPLETE: docs/storage/messages RLS verified+guarded (0442/0551); family-media bucket shipped+write-isolated (0216); seeds >=500 confirmed (seed_documents n:=500). 2 owner-gated findings: LB-009 (public-read) + PLA-0581 (Secure Vault label-only). Residual = owner/live-infra only
