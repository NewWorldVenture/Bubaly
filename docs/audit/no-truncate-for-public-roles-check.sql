-- ── 0304: the public roles hold no TRUNCATE privilege ───────────────────────
--
...
begin
  ...
  if n <> 0 then
    raise exception '0304: % table(s) still grant TRUNCATE to anon/authenticated (e.g. %) — RLS does not cover this', n, sample;
  end if;
  ...
  select count(*) into n
   ...
   where table_name in ('sync_tokens', 'social_account_tokens')
     ...
     and grantee in ('anon', 'authenticated');
  if n <> 0 then
    raise exception '0304: a credential store still grants TRUNCATE to a public role (% grant(s))', n;
  end if;

  raise notice '0304 OK: no TRUNCATE for anon or authenticated anywhere in public';
end $$;
