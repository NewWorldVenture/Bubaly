-- ── 0312: a social restriction is not self-service ──────────────────────────
--
...
-- back UP.
--
-- Measured before 0312, as an `adult` deliberately set to `read_only`:
...
declare
  ...
  up  uuid := 'f0312000-0000-4000-8000-00000000c001';  -- parent, the grantor
  ud  uuid := 'f0312000-0000-4000-8000-00000000c002';  -- adult, restricted to read_only
  n int; role_now text;
begin
  insert into public.families (id, name) values (fam, '0312 social access') on conflict do nothing;
  insert into auth.users (id, email) values
    (up, 'p0312@example.test'), (ud, 'd0312@example.test') on conflict do nothing;
  ...
  if auth.uid() is distinct from ud then
    raise exception '0312: impersonation failed — auth.uid() is %, expected the restricted member', auth.uid();
  end if;
  ...
  if public.social_has_permission(fam, 'publish_posts') then
    raise exception '0312: a read_only social role may publish — the fixture is not restricting anyone';
  end if;

  delete from public.social_access_permissions where family_id = fam and user_id = ud;
  get diagnostics n = row_count;
  if n <> 0 then
    raise exception '0312: a member deleted the row restricting them (% row(s))', n;
  end if;
  ...
  if role_now is distinct from 'read_only' then
    raise exception '0312: the restricted member now resolves to % — they fell back up', role_now;
  end if;
  if public.social_has_permission(fam, 'publish_posts')
    ...
    raise exception '0312: the restricted member can publish, manage settings or connect accounts';
  end if;
  ...
  get diagnostics n = row_count;
  if n <> 0 then
    raise exception '0312: a restricted member cleared % access row(s) for the whole family', n;
  end if;
  ...
  if not public.is_family_admin(fam) then
    raise exception '0312: the fixture parent is not a family admin — the next assertion proves nothing';
  end if;
  delete from public.social_access_permissions where family_id = fam and user_id = ud;
  get diagnostics n = row_count;
  if n <> 1 then
    raise exception '0312: a family ADMIN could not remove an access row (% row(s)) — the fix went too far', n;
  end if;

  reset role;
  raise notice '0312 OK: only the adults who manage social access may remove a row, and a restricted member stays restricted';
end $$;
