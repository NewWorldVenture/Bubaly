-- ── 0323: deleting a review is rewriting it ─────────────────────────────────
--
-- 0322 stopped a member rewriting another member's review; the DELETE policies
...
do $$
declare
  fam uuid := 'f0323000-0000-4000-8000-00000000fa01';
  ua  uuid := 'f0323000-0000-4000-8000-00000000c001';  -- author / seller, parent
  uc  uuid := 'f0323000-0000-4000-8000-00000000c003';  -- review subject, parent
  uk  uuid := 'f0323000-0000-4000-8000-00000000c005';  -- an uninvolved child
  ma uuid; mc uuid; mk uuid; lst uuid; str uuid; rev uuid; off uuid; sav uuid; fol uuid;
  n int;
begin
  insert into public.families (id, name) values (fam, '0323 delete authorship') on conflict do nothing;
  insert into auth.users (id, email) values
    (ua, 'a0323@example.test'), (uc, 'c0323@example.test'), (uk, 'k0323@example.test')
  ...
  if auth.uid() is distinct from uc then
    raise exception '0323: impersonation failed — auth.uid() is %, expected the review''s subject', auth.uid();
  end if;

  delete from public.marketplace_reviews where id = rev;
  get diagnostics n = row_count;
  if n <> 0 then
    raise exception '0323: the SUBJECT of a review deleted it (% row(s)) — and they are a MANAGER, which is the loophole the predicate names', n;
  end if;
  ...
  get diagnostics n = row_count;
  if n <> 0 then
    raise exception '0323: a member deleted another member''s save (% row(s))', n;
  end if;
  delete from public.marketplace_follows where id = fol;
  get diagnostics n = row_count;
  if n <> 0 then
    raise exception '0323: a member deleted another member''s follow (% row(s))', n;
  end if;
  ...
  get diagnostics n = row_count;
  if n <> 0 then
    raise exception '0323: a member with no stake in the listing deleted a competing offer (% row(s))', n;
  end if;
  ...
  get diagnostics n = row_count;
  if n <> 1 then
    raise exception '0323: the LISTING OWNER could not remove an offer on their own listing (% row(s)) — the fix went too far', n;
  end if;
  ...
  get diagnostics n = row_count;
  if n <> 1 then
    raise exception '0323: a member could not un-save their own listing (% row(s)) — toggleSaveAction is broken', n;
  end if;
  delete from public.marketplace_follows where id = fol;
  get diagnostics n = row_count;
  if n <> 1 then
    raise exception '0323: a member could not unfollow their own store (% row(s)) — toggleFollowAction is broken', n;
  end if;
  ...
  get diagnostics n = row_count;
  if n <> 1 then
    raise exception '0323: the AUTHOR could not delete their own review (% row(s)) — the fix went too far', n;
  end if;
  ...
  get diagnostics n = row_count;
  if n <> 1 then
    raise exception '0323: a MANAGER could not moderate a review that is not about them (% row(s)) — the fix went too far', n;
  end if;

  reset role;
  raise notice '0323 OK: an author may withdraw their review, a manager may moderate one that is not about them, and the subject may do neither';
end $$;
