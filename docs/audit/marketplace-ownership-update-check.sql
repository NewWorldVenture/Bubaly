-- ── 0306: an UPDATE policy checks the row you end with, not just the one you
--          started from ───────────────────────────────────────────────────────
--
...
--
-- Measured before 0306, as the BUYER on a completed order:
...
declare
  ...
  ua  uuid := 'f0306000-0000-4000-8000-00000000c001';
  uc  uuid := 'f0306000-0000-4000-8000-00000000c003';
  ma uuid; mc uuid; lst uuid; ord uuid; off uuid;
  n int; refused boolean;
begin
  insert into public.families (id, name) values (fam, '0306 marketplace ownership') on conflict do nothing;
  insert into auth.users (id, email) values
    (ua, 'a0306@example.test'), (uc, 'c0306@example.test') on conflict do nothing;
  ...
  if auth.uid() is distinct from uc then
    raise exception '0306: impersonation failed — auth.uid() is %, expected the buyer; this probe is not testing what it claims', auth.uid();
  end if;
  ...
  if not refused then
    raise exception '0306: a BUYER made themselves the seller of record on their own completed order';
  end if;
  ...
  if n <> 0 then
    raise exception '0306: the buyer shows % completed sale(s) they never made', n;
  end if;
  ...
  if not refused then
    raise exception '0306: an offer was reassigned to a member who did not make it';
  end if;
  ...
  if n <> 1 then
    raise exception '0306: a party could not advance their own order (% row(s)) — the fix went too far', n;
  end if;
  -- And the offer's own author may still withdraw it.
  update public.marketplace_offers set status = 'withdrawn' where id = off;
  get diagnostics n = row_count;
  if n <> 1 then
    raise exception '0306: the offer author could not withdraw their own offer (% row(s)) — the fix went too far', n;
  end if;

  reset role;
  raise notice '0306 OK: a party may advance their own deal and may not rewrite who the deal was with';
end $$;
...
do $$
declare
  fam_a uuid := 'f0306100-0000-4000-8000-00000000fa01';
  fam_b uuid := 'f0306100-0000-4000-8000-00000000fa02';
  usr   uuid := 'f0306100-0000-4000-8000-00000000c001';
  lst uuid; n int; refused boolean; offenders text; owner_after text;
begin
  insert into public.families (id, name) values
    (fam_a, '0306 using-as-check A'), (fam_b, '0306 using-as-check B') on conflict do nothing;
  insert into auth.users (id, email) values (usr, 'u0306b@example.test') on conflict do nothing;
  ...
  insert into public.todo_lists (family_id, name) values (fam_a, '0306 list') returning id into lst;
  ...
  if not refused then
    raise exception '0306: a `using`-only UPDATE policy let a row be moved into another family';
  end if;
  reset role;

  -- (b) The same policy shape in isolation, with the blank filled in.
  drop table if exists public.zz_0306_using_as_check;
  create table public.zz_0306_using_as_check (id int primary key, owner text);
  alter table public.zz_0306_using_as_check enable row level security;
  insert into public.zz_0306_using_as_check values (1, 'authenticated');
  grant select, update on public.zz_0306_using_as_check to authenticated;
  ...
  create policy sel on public.zz_0306_using_as_check for select using (true);
  create policy upd on public.zz_0306_using_as_check for update using (owner = current_user);

  refused := false;
  begin
    set local role authenticated;
    update public.zz_0306_using_as_check set owner = 'someone_else' where id = 1;
  exception when insufficient_privilege then refused := true;
  end;
  reset role;
  if not refused then
    raise exception '0306: the isolated `using`-only policy did not check the new row — (a) proved nothing about the class';
  end if;

  alter policy upd on public.zz_0306_using_as_check with check (true);
  begin
    set local role authenticated;
    update public.zz_0306_using_as_check set owner = 'someone_else' where id = 1;
  ...
  reset role;
  select owner into owner_after from public.zz_0306_using_as_check where id = 1;
  if owner_after is distinct from 'someone_else' then
    raise exception '0306: `with check (true)` did NOT switch the implicit check off — the ratchet below is guarding a hazard that does not exist, and says something false about PostgreSQL';
  end if;
  drop table public.zz_0306_using_as_check;
  ...
  if offenders is not null then
    raise exception '0306: permissive UPDATE/ALL policies write `with check (true)`, which switches the implicit `using` check off: %', offenders;
  end if;

  raise notice '0306 OK: an omitted `with check` is the `using` clause, measured both ways — and no policy overrides it with `true`';
end $$;
