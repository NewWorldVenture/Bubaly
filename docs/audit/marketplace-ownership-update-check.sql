...
do $$
declare
  fam uuid := 'f0321000-0000-4000-8000-00000000fa01';
  ua  uuid := 'f0321000-0000-4000-8000-00000000c001';
  uc  uuid := 'f0321000-0000-4000-8000-00000000c003';
...
do $$
declare
  fam_a uuid := 'f0321100-0000-4000-8000-00000000fa01';
  fam_b uuid := 'f0321100-0000-4000-8000-00000000fa02';
  usr   uuid := 'f0321100-0000-4000-8000-00000000c001';
...
begin
  ...
  drop table if exists public.zz_0321_using_as_check;
  create table public.zz_0321_using_as_check (id int primary key, owner text);
  alter table public.zz_0321_using_as_check enable row level security;
  insert into public.zz_0321_using_as_check values (1, 'authenticated');
  grant select, update on public.zz_0321_using_as_check to authenticated;
  ...
  create policy sel on public.zz_0321_using_as_check for select using (true);
  create policy upd on public.zz_0321_using_as_check for update using (owner = current_user);

  refused := false;
  begin
    set local role authenticated;
    update public.zz_0321_using_as_check set owner = 'someone_else' where id = 1;
  exception when insufficient_privilege then refused := true;
  end;
  reset role;
  if not refused then
