...
| `a-child-cannot-spend-the-same-dollar-twice-check.sql` | **Two parts.** PART 1 RACES it: two real connections fire two $8 spends at one $10 wallet, held at a `pg_advisory_lock` starting gate and released together — the parent polls `pg_locks` until both are provably parked, so "simultaneous" is evidence rather than hope and non-overlap is a bug in the probe, FAILED rather than skipped. Exactly one may be approved, $8.00 debited not $16.00, the balance never negative, the loser told `insufficient_funds` with the cents that are left. PART 2 asserts what a race cannot reach, on one connection in one rolled-back transaction: a live card hold counts against an in-app spend (`bucketBalanceCents` totals `completed` only), the exact remainder still posts, a held request stays inert, `forbidden` holds for a stranger a child and another family, the key is idempotent only when complete, the audit row is inside the transaction. **Both halves carry a negative control that must fire**: part 1 replaces `wallet_debit_spend_bucket`'s body with the pre-0342 read-then-insert, runs the SAME race through the SAME gate and REQUIRES two approvals and -$6.00, then restores the real body and verifies the restore byte for byte against the definition it saved; part 2 replays the same shape inline on its own bucket. Mutation-tested: a de-locked RPC that still carries the words `for update` **in a comment** passes 0342's own source-shape guard and is caught only here — 0342. |
