-- ── 0310: a review belongs to whoever wrote it ──────────────────────────────
--
...
--
-- Measured before 0310: a member rewrote another member's one-star review of
...
begin
  insert into public.families (id, name) values (fam, '0310 review authorship') on conflict do nothing;
  ...
  if auth.uid() is distinct from uc then
    raise exception '0310: impersonation failed — auth.uid() is %, expected the review''s subject', auth.uid();
  end if;
  ...
  if n <> 0 then
    raise exception '0310: the SUBJECT of a review rewrote its rating (% row(s))', n;
  end if;
  select rating into got from public.marketplace_reviews where id = rev;
  if got is distinct from 1 then
    raise exception '0310: the one-star review now reads % stars', got;
  end if;
  ...
  get diagnostics n = row_count;
  if n <> 0 then
    raise exception '0310: a member rewrote another member''s follow (% row(s))', n;
  end if;
  update public.marketplace_saves set member_id = mc where id = sav;
  get diagnostics n = row_count;
  if n <> 0 then
    raise exception '0310: a member rewrote another member''s save (% row(s))', n;
  end if;

  -- ── The author may revise their own, and may not move it ──────────────────
  perform set_config('request.jwt.claim.sub', ua::text, true);
  if auth.uid() is distinct from ua then
    raise exception '0310: impersonation failed — auth.uid() is %, expected the author', auth.uid();
  end if;

  update public.marketplace_reviews set rating = 3, comment = 'Sorted it out' where id = rev;
  get diagnostics n = row_count;
  if n <> 1 then
    raise exception '0310: the AUTHOR could not revise their own review (% row(s)) — the fix went too far', n;
  end if;
  ...
  if not refused then
    raise exception '0310: an author re-pointed their review at a different member';
  end if;

  reset role;
  raise notice '0310 OK: the author may revise a review, the subject may not, and nobody may move one';
end $$;
...
do $$
declare
  fam uuid := 'f0310100-0000-4000-8000-00000000fa01';
  usr uuid := 'f0310100-0000-4000-8000-00000000c001';
  lst uuid; caught text;
begin
  insert into public.families (id, name) values (fam, '0310 typo guard') on conflict do nothing;
  ...
  reset role;
  drop trigger zz_0310_typo on public.marketplace_listings;
  delete from public.marketplace_listings where family_id = fam;

  if caught is null or caught not like '%names a column that does not exist%' then
    raise exception '0310: a misspelled immutable column passed silently (%) — the guard cannot fail', coalesce(caught, 'no error');
  end if;
  raise notice '0310 OK: a misspelled immutable column raises instead of guarding nothing';
end $$;
