...
begin
  ...
  -- 0311: the OAuth token store answers to NOBODY holding a JWT, adult
  ...
  select count(*) into n from public.social_account_tokens where family_id = fam;
  if n <> 0 then
    raise exception '0311: an ADULT reads % social OAuth token row(s) — service-role only', n;
  end if;
  refused := false;
  begin
    insert into public.social_account_tokens (family_id, account_id, platform, provider_account_id, access_token_enc)
  ...
  if not refused then
    raise exception '0311: an ADULT inserted a social OAuth token row';
  end if;

  reset role;
  raise notice '0297+0311 OK: a child is refused a parent licence and a sibling login and keeps their own licence; parent and adult keep those; NOBODY with a JWT reaches the OAuth tokens';
end $$;
