-- ── 0308: the binder's sensitive rows are not child-readable ────────────────
--
...
declare
  ...
  par uuid := 'f0308000-0000-4000-8000-00000000c001';
  kid uuid := 'f0308000-0000-4000-8000-00000000c003';
  n int; refused boolean;
begin
  insert into public.families (id, name) values (fam, '0308 binder boundary') on conflict do nothing;
  insert into auth.users (id, email) values
    (par, 'p0308@example.test'), (kid, 'k0308@example.test') on conflict do nothing;
  ...
  if auth.uid() is distinct from kid then
    raise exception '0308: impersonation failed — auth.uid() is %, expected the child; this probe is not testing what it claims', auth.uid();
  end if;

  select count(*) into n from public.household_info where family_id = fam and is_sensitive;
  if n <> 0 then
    raise exception '0308: a child reads % sensitive binder row(s) — alarm codes and wifi keys', n;
  end if;
  ...
  if n <> 1 then
    raise exception '0308: a child reads %/1 ordinary binder row(s) — the fix went too far', n;
  end if;
  ...
  if n <> 0 then
    raise exception '0308: a child un-flagged % sensitive row(s) to read them', n;
  end if;
  ...
  if not refused then
    raise exception '0308: a child inserted a sensitive binder row';
  end if;

  perform set_config('request.jwt.claim.sub', par::text, true);
  if auth.uid() is distinct from par then
    raise exception '0308: impersonation failed — auth.uid() is %, expected the parent', auth.uid();
  end if;
  select count(*) into n from public.household_info where family_id = fam;
  if n <> 2 then
    raise exception '0308: a PARENT sees %/2 binder rows — the grown-ups were locked out', n;
  end if;

  reset role;
  raise notice '0308 OK: a child keeps the bin day and never sees the wifi key; a parent keeps both';
end $$;
