-- ── 0320: the binder's sensitive rows are not child-readable ────────────────
--
...
do $$
declare
  fam uuid := 'f0320000-0000-4000-8000-00000000fa01';
  par uuid := 'f0320000-0000-4000-8000-00000000c001';
  kid uuid := 'f0320000-0000-4000-8000-00000000c003';
  n int; refused boolean;
begin
  insert into public.families (id, name) values (fam, '0320 binder boundary') on conflict do nothing;
  ...
  if auth.uid() is distinct from kid then
    raise exception '0320: impersonation failed — auth.uid() is %, expected the child; this probe is not testing what it claims', auth.uid();
  end if;

  select count(*) into n from public.household_info where family_id = fam and is_sensitive;
  if n <> 0 then
    raise exception '0320: a child reads % sensitive binder row(s) — alarm codes and wifi keys', n;
  end if;
  ...
  if n <> 1 then
    raise exception '0320: a child reads %/1 ordinary binder row(s) — the fix went too far', n;
  end if;
  ...
  if n <> 0 then
    raise exception '0320: a child un-flagged % sensitive row(s) to read them', n;
  end if;
  ...
  if not refused then
    raise exception '0320: a child inserted a sensitive binder row';
  end if;

  perform set_config('request.jwt.claim.sub', par::text, true);
  if auth.uid() is distinct from par then
    raise exception '0320: impersonation failed — auth.uid() is %, expected the parent', auth.uid();
  end if;
  select count(*) into n from public.household_info where family_id = fam;
  if n <> 2 then
    raise exception '0320: a PARENT sees %/2 binder rows — the grown-ups were locked out', n;
  end if;

  reset role;
  raise notice '0320 OK: a child keeps the bin day and never sees the wifi key; a parent keeps both';
end $$;
