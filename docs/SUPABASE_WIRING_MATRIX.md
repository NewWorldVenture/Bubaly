...
| Notifications | notifications, devices, preferences, `admin_notifications` | family/user ownership plus Super Admin operations | push, email, realtime, cron digest, provider sync, batch crons | Partial | Admin digest, provider-sync, and batch cron routes now surface local failures; job-by-job live delivery/replay audit pending |
