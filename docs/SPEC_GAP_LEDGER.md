...
- **§33 (in part), the two surfaces a family would actually ask about now leave a
  ...
  **Six more surfaces followed** — the meal planner, insights, assist, the weekly
  briefing, the health coach and the habit coach — each named individually in the coverage test, because "fewer
  ...
  **The remaining 42 silent surfaces are counted, not ignored.**
  `tests/ai-observability-coverage.test.ts` caps them at exactly 42 — not a round
  number above it, because slack in a ratchet is room for new silent surfaces to
  slip in green, and the ceiling comes down with every adoption (52 → 48 → 44 → 42) — and asserts the scanner finds something, so a broken
  scanner cannot satisfy the cap vacuously. Verified: adding one new
  provider-calling route fails it.

  Still open in §33: the other 42 surfaces, and the admin view over runs, which
  this does not touch.
