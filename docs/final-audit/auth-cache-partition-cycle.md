...
- Independent browser stores outside `lib/offline/cache.ts` remain outside this bounded repair. The caller audit initially found 22 queries omitting errors; the concurrent DATA-003 finance repair handles five, leaving 17 for further investigation. No claim is made that all 231 workflows are complete merely because the central cache contract passes.
