...
Application and test source: `21bbfae1b3fdba8c6ca8520826edfff674df3dca`. The implementation starts at `669c521d`; the final change rejects duplicate rows for a single meal slot. Main is integrated through `9719a486` in merge `674f7a24`.
...
Final frozen unit and combined browser gates pass in the existing private installation under Node 24.21.0: 1,178 files / 14,021 unit tests in 52.47 seconds with eight workers, and 496 Chromium cases across 21 suites in 1.1 minutes with four workers. The final build/type/lint/audit lane is running; the preceding meal source built 249 pages and passed strict types, lint, localization and query audits. No dependency installation or local preview startup was performed.
...
`tests/e2e/weekly-meal-durable.spec.ts` adds one hosted disposable-backend phone journey. It signs in through the real password form, saves a recipe, replaces another dinner with a custom dish, reloads, verifies persisted ingredients and exact grocery amounts, retries without duplicate additions, removes only one selected slot and verifies the remaining records. It refuses remote origins, creates only its own Auth/REST fixtures, disables credential-bearing artifacts and cleans its own family/account. Discovery and lint pass; runtime execution is pending publication.
