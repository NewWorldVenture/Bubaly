...
| 0166 | `0166_workload_snapshots.sql` | `workload_snapshots` (household workload balancing + analytics) | **New feature — closes competitor gaps #3/#4 (Hearth mental-load).** `/dashboard/workload` measures who is carrying the household (chore est_minutes + tasks + organized events = invisible labor), fairness score, and one-tap rebalance moves (reassigns the open chore, audited). This table persists weekly per-member snapshots for the week-over-week trend. Family-scoped RLS; unique (family, member, week_start). Seeded by `seed_workload.sql` (500 rows). Safe before apply: the live report still computes; only history parking no-ops. Additive/idempotent. PG16-verified (idempotent ×2; seed 500 ×2). |
| 0167 | `0167_independence_milestones.sql` | `independence_milestones` (child independence progression ladder) | **New feature — closes competitor gap #19 (Hearth child independence).** `/dashboard/independence`: 90-milestone age-banded ladder (6 domains × 5 bands, content in `lib/independence/progression.ts`); this table stores each child's progress (suggested → in_progress → achieved/skipped, evidence + achieved_at). Family-scoped RLS; unique (family, member, domain, title). Seeded by `seed_independence.sql` (500 rows). Safe before apply: page renders suggestions read-only. Additive/idempotent. PG16-verified (idempotent ×2; seed 500 ×2). |
