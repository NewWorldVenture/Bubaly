...
> shippable without an external key. All shipped to `main`, each verified (tsc · eslint · full
> vitest · `next build`; migrations validated on a throwaway PG16). In order:
>
> **New roadmap features (100% Supabase-wired):**
> - **Marketplace** — migration `0120_marketplace.sql` (listings + offers, family RLS), `lib/marketplace/*`
>   (tested), `/dashboard/marketplace` + admin `/dashboard/marketplace/seed` (500-record seed, `MARKETPLACE_SEED_SQL`).
> - **Voice Control** — migration `0121_voice_commands.sql`, `lib/voice/command-router.ts` (tested),
>   `/dashboard/voice` (Web Speech → route → real capture rows + history).
> - **Family Knowledge Base** (persistent family memory, roadmap #5) — migration `0123_family_facts.sql`,
>   `lib/memory/facts.ts` (tested), `/dashboard/knowledge` + admin `/dashboard/knowledge/seed`
>   (`FAMILY_FACTS_SEED_SQL`, seeds only empty families — non-destructive). The Knowledge **Graph**
>   (`/dashboard/family-knowledge-graph`) now also renders `family_facts` as nodes.
> - **Next Best Actions** (Predictive Planning #9) — `lib/opportunities/next-actions.ts` (tested),
>   `/dashboard/next-best-actions` (merges events + tasks + opportunities, no migration).
> - **Outcomes launcher** (North Star pillar #4, 2026-07-05) — `lib/outcomes/launcher.ts` (tested),
>   `/dashboard/outcomes`: eight family goals → auto-assembled, live-badged capability steps. No migration.
>
> **Friction backlog items closed:**
> - **#2 command bar** — shipped by a PARALLEL session (`lib/command-bar/route.ts`); my dup was discarded.
> - **#4 Autopilot self-completes moment prep** — `momentPrepSuggestions` (see line directly below).
> - **Journey telemetry** — migration `0124_journey_events.sql`, `lib/analytics/journey.ts` +
>   `useJourney(key)` hook; instruments capture / add_memory / voice_command / next_actions;
>   super-admin `/dashboard/journeys` shows real medians (replaces Scorecard estimates).
> - **Voice messages** (Messages) — `MediaRecorder` → `sendFile` (`kind='audio'`) → `<audio>` playback.
> - **Upload size guards** — pre-checks + honest 25 MB copy in documents / trip-memories.
> - **Onboarding** — PIN step made **"Skip for now"** (was mandatory); 1 required field → done.
> - **Nav cohesion** — Marketplace + Voice set to `minLevel 0` (free/customizable) **per explicit user
>   approval** (sanctioned exception to the standing global-nav rule).
> - **PR #203** (draft global-sidebar rewrite) — **closed** at the user's request. No open PRs remain.
>
> **What's LEFT (all blocked on external inputs — do NOT stub as "done"):** Messages GIF picker (needs
> GIPHY/Tenor key); #6 ETA travel buffer (needs maps/routing key); Wallet real-time card balances (Stripe
> Issuing). Also open: onboarding could defer/infer more; onboarding telemetry needs an anonymous/pre-family
> path (journey_events is family-scoped, no family_id until completion).
>
> **Migration counter:** last is `0124`. **Validate migrations locally** with the PG16 recipe (see the
> "🛒🎙️ 2026-07-04 — MARKETPLACE" block far below: throwaway `initdb` as `postgres`, bootstrap the
> Supabase `auth`/`storage`/roles/`supabase_realtime` objects, apply `supabase/migrations/*.sql` in order).
>
> ---

Living context doc so another agent can continue without re-deriving everything.
Last updated: 2026-07-05 — **Family API hub shipped (North Star pillar #9)**: the orchestration hub for external services. Pure `lib/connections/providers.ts` (8 tests) = provider registry (11 providers × calendar/email/banking/grocery/smart_home) + `mergeConnections`/`groupByCategory`/`connectedCount`. Migration `0128_family_connections.sql` = durable family-scoped RLS connection records (provider/category/status/account_label/last_synced_at; `unique(family_id,provider,external_account_id)`; **no tokens stored** — secret store only). `/dashboard/connections` (`components/modules/connections-module.tsx`): grouped hub, Connect (upsert) / Disconnect, live status, realtime. Nav: Suggested → Connections (`Network`, free). Seed `supabase/seed_pillar9_connections.sql` (500 records, idempotent — note: cycle providers/statuses with `g.i % len`, NOT lateral `random()`, which Postgres constant-folds). **GATED for later (needs provider keys):** real OAuth/token exchange + live two-way sync per provider — the hub/model/CRUD are done; enabling a provider = wiring its OAuth against these rows. **North-Star status: pillars #1–#6, #8, #9 all shipped; #7 = the FOI (shipped as slice 1).** Also flagged: PR #232 (playbook "Family Intelligence") is a stale DRAFT duplicating the already-merged pillar-#3 playbook (`lib/playbook/learn.ts`) — do NOT merge without reconciling. Verified tsc/eslint/**1738 vitest**/build. Earlier: 2026-07-05 — **Design for Calm shipped (North Star pillar #8)**: one prioritized inbox + daily digest + gentle escalation. Pure `lib/calm/inbox.ts` (7 tests) `buildCalmInbox(items)` folds every source (agent_activity · autopilot_suggestions · FOI snapshot suggestions · approval_requests · reminders) into ONE de-duped, ranked list → `needsYou` (action, capped 5), `today` (attention, capped 8), `quieted` count for the rest; `calmDigest`/`noiseReduced`. `/dashboard/calm` (`components/modules/calm-module.tsx`): server maps the real rows → CalmItems → the inbox; digest header + "Needs you"/"For today" + collapsed quieter items. **No migration** (pure aggregation of already-shipped tables). Nav: Suggested → Calm (`Leaf`, free). Also verified #6 (Specialized Agents) fully on main. Verified tsc/eslint/**1730 vitest**/build. Earlier: 2026-07-05 — **Specialized Agents shipped (North Star pillar #6)**: one interface, ten domain agents. Pure `lib/agents/roster.ts` (11 tests) — `runAgent`/`chiefOfStaff`/`runAllAgents` turn a real `AgentContext` (18 family signals) into per-agent briefings; Chief of Staff synthesizes the top cross-agent items. Migration `0127_agent_activity.sql` = durable family-scoped RLS log (agent/kind/severity/status, done/dismissible). `/dashboard/agents` (`components/modules/agents-module.tsx`): server builds the snapshot from real tables (calendar/meals/grocery/bills/subs/chores/docs/maintenance/vacations/approvals/photos/birthdays) → live briefings; roster grid + per-agent items + recent activity write-back. Nav: Suggested → Family Assistant (`Bot`, free). Seed `supabase/seed_pillar6_agents.sql` (500 activity records, all 10 agents, idempotent). To add an agent: extend the roster + `runSpecialist` + a context field. Verified tsc/eslint/**1723 vitest**/build. Earlier: 2026-07-05 — **Playbook (pillar #3) extended — learns Travel Style**: picked up the other bot's Family Playbook (slice 6). Added a `travel` `PlaybookSignal` to `lib/playbook/learn.ts` (MIN 3 trips, 3 new tests → 13) and wired `refreshPlaybookAction` to mine the `vacations` table (`TRAVEL_KIND_STYLE` per `VacationKind` + `travelSeason` from `start_date`) → e.g. "Road trips", "Summer trips" become confirmable playbook facts. Generic review UI needed no change. Verified tsc/eslint/**1712 vitest**/build. Earlier: 2026-07-05 — **Outcomes launcher shipped (North Star pillar #4 — "Outcomes, not features")**: pure `lib/outcomes/launcher.ts` (10 tests) defines the eight family goals (Run Today · Feed the Family · Plan a Trip · Prepare for School · Manage Money · Keep Everyone Healthy · Celebrate Together · Prepare for the Unexpected); `buildOutcomePlan(id, ctx)` assembles each goal's deep-linked capability steps and **auto-badges the urgent ones** from a real `OutcomeContext` (events today · overdue tasks · birthdays soon · open grocery items). Route `/dashboard/outcomes` (server reads the real snapshot via count queries + birthday compute → `components/modules/outcomes-launcher.tsx` goal-first picker). Nav: Suggested → Outcomes, `Wand2`, minLevel 0 (free + in Navigation Choices catalog). **No migration** (reads existing tables). Extend it by adding fields to `OutcomeContext` + badges in `buildOutcomePlan`. Verified tsc/eslint/**1709 vitest**/build. Earlier: 2026-07-04 — **Autopilot self-completes moment prep (Friction #4)**: NEW pure `momentPrepSuggestions(snapshot)` in `lib/autopilot/engine.ts` (folded into `buildSuggestions`) reuses the shared `buildMomentPrep` engine to emit, for imminent (≤2-day) moments, the two SAFE + REVERSIBLE prep steps at auto-tier confidence: a **leave-by reminder** (conf 92 → reuses the existing `create_reminder` executor) and a **snacks/supplies grocery list** (conf 91 → NEW `add_groceries` executor in `scan.ts` = get-or-create active grocery list + insert grocery_items). `EventSignal` gained optional `allDay`/`location`; `runAutopilotScan` now selects `all_day, location` and maps `related_type='event'` for calendar-event reminders. Weather/packing/photo prep deliberately stay on-screen (not auto-run). 5 new engine tests (36 total). No schema change. Earlier same day: **Nav cohesion (user-approved tiering change)**: `NAV_CATALOG` (Navigation Choices allowlist) only includes `minLevel 0` APP_NAV_GROUPS items. Marketplace + Voice Control were `minLevel 1` (paid-only) despite their pages not being plan-gated; **per explicit user approval, lowered both to `minLevel 0`** so all four of this session's new features (Marketplace, Voice, Knowledge Base, Next Best Actions) are free-tier + selectable in Settings → Navigation Choices. (This is a sanctioned exception to the standing "don't change global nav" rule — the user chose it.) Earlier same day: **Onboarding time-to-first-value win**: the PIN step in `onboarding-wizard.tsx` was **mandatory** (Continue disabled until a 4-digit PIN matched) — a real friction tax, since `completeProfileOnboardingAction` already treats an absent PIN as valid (only writes App Lock `if (input.pin)`). Added **"Skip for now"** (`finish(false)`); onboarding is now 1 required field (name) → skip → done, PIN deferred to Settings → App Lock. NOTE: onboarding telemetry does NOT fit `journey_events` (no `family_id` until completion) — a future measure needs an anonymous/pre-family analytics path. Earlier same day: **Journey telemetry now instruments 4 flows**: added `useJourney` emit points to **add_memory** (`create-memory.tsx`: start on mount, complete on save), **voice_command** (`voice-module.tsx`: start/complete/abandon around each `run`), and **next_actions** (`next-actions-module.tsx`: start on mount, complete on first task cleared) — joining **capture**. `/dashboard/journeys` now fills out across all four. Pattern to add more: `const j = useJourney('key'); j.start(); … j.complete()` + a label in `JOURNEY_LABELS`. Earlier same day: **Journey telemetry pipeline shipped** (makes the Experience Scorecard real): migration `0124_journey_events.sql` (append-only, family-scoped RLS — insert+select only), pure `lib/analytics/journey.ts` (`summarizeJourneys`/`median`/formatters, 7 tests), client hook `lib/analytics/use-journey.ts` (`useJourney(key)` → fire-and-forget started/step/completed with duration; **errors always swallowed — telemetry must never break UX**), first instrumented flow = **Capture** (`quick-capture.tsx`: start on open, complete on save), and a super-admin read page `/dashboard/journeys` (per-journey starts / completion rate / median time / median steps). To instrument another flow: `const j = useJourney('key'); j.start(); … j.complete();` and add a label in `JOURNEY_LABELS`. Cross-family aggregate medians would need a service-role read (page is currently per-family via RLS). Earlier same day: **Upload size-guard consistency + honest copy**: exported `DOCUMENT_MAX_BYTES`/`DOCUMENT_MAX_MB` from `lib/storage/documents.ts` (the 25 MB guard already existed there); added fail-fast pre-checks at file-pick time in `documents-module.tsx` + `trip-memories-module.tsx`, and fixed the Documents dropzone's misleading "up to 50 MB" → real **25 MB**. (Both are single-file uploads, so no progress bar — Supabase JS `upload()` exposes no progress event.) Earlier same day: **Voice messages shipped** (finished a "coming soon" stub in `messages-module.tsx`): `MediaRecorder` records a clip → uploads via the existing `sendFile` path (new kind `audio`, reuses the 25 MB cap + orphan-rollback) → renders an inline `<audio controls>` bubble. Composer shows a live timer + cancel(discard)/stop-and-send; graceful fallback when MediaRecorder is unsupported. 100% Supabase (family-media storage + `family_messages` row, `kind='audio'`). Earlier same day: **Knowledge Graph now reads `family_facts`**: `/dashboard/family-knowledge-graph` (already a real data-backed graph of members/teams/routines/classes/goals/events) now also renders member-tagged Knowledge Base facts as cyan "fact" nodes (relation `knows`, capped 3/member pinned-first) + a "Facts known" stat tile. Missing-table safe (pre-0123 → null → no fact nodes). Completes the persistent-knowledge-graph loop (store + visualization). Earlier same day: **Family Knowledge Base seed across all profiles** shipped to `main`: `lib/memory/facts-seed-sql.ts` (`FAMILY_FACTS_SEED_SQL` — realistic starter facts for EVERY family; **non-destructive**, seeds only families with zero facts so real data is never clobbered; idempotent) + admin screen `/dashboard/knowledge/seed` (Copy-SQL, super-admin-gated, iPad-friendly) + a "Seed test data" link on `/dashboard/knowledge` for super-admins. Validated on PG16 (skips populated families, seeds empty ones, re-run = no-op). Earlier same day: **Family Knowledge Base** shipped to `main` (closes the Family Memory persistence gap / roadmap #5): migration `0123_family_facts.sql` (family-scoped RLS store for durable facts — sizes/allergies/contacts/preferences/accounts, member-tagged or family-level, pinnable), pure `lib/memory/facts.ts` (8 tests), `components/modules/knowledge-base-module.tsx` at `/dashboard/knowledge` (nav: Family AI OS, `Brain` icon; search + member/category filters, add/edit/pin/copy/delete). Validated on PG16. The existing `/dashboard/family-knowledge-graph` visualization can now read `family_facts`. Earlier same day: **Next Best Actions** page shipped to `main` (Predictive Planning / roadmap #9 stretch): pure `lib/opportunities/next-actions.ts` (7 tests — bucket overdue→someday, priority tie-break, human reasons) + `components/modules/next-actions-module.tsx` at `/dashboard/next-best-actions` (nav: Suggested, `Target` icon), merging real events + open tasks + open opportunities into one ranked worklist with inline task-complete. **No migration** (reads existing tables). NOTE for coordinators: this session also built a ⌘K command bar but **discarded it** because a parallel session (`claude/command-bar`) shipped an equivalent first — the abandoned dup lives only on local branch `feature/voice-control-cont`, never pushed; use the `lib/command-bar/route.ts` version on main. Earlier same day: **Recurring-routine templates (Friction #10)** shipped to `main` (merge of `claude/routine-templates`): migration `0122_routine_templates.sql`, pure `lib/routines/detect.ts` (13 tests), and a **Routines panel in the calendar right rail** that detects repeating events and lets families save + apply routines. See the "🔁 2026-07-04 SESSION — Routines" block directly below. Earlier same day: **Marketplace** (migration `0120`, `/dashboard/marketplace` + 500-record seed) AND **Voice Control** (migration `0121_voice_commands`, `lib/voice/command-router.ts`, `/dashboard/voice`) both shipped to `main` — completing all 12 roadmap features. Repo-root **`todo.md`** is the build guide. See the "🛒🎙️ 2026-07-04" block below. Earlier: 2026-07-03 — Trust Engine wired into ALL wallet money movement (Trust TODO #1 closed; see top session block). Earlier same day: Mobile polish merged (#210: scrollable wide tables, safe-area overlays, a11y labels), public-route overflow e2e guard (`tests/e2e/overflow.spec.ts`), and the FINAL describeDbError sweep (zero raw `err.message` toast paths remain). Earlier same day: Mobile-readiness **foundation** pass (see the "2026-07-03 SESSION — Mobile foundation" section immediately below). Also recently shipped to `main`: customizable sidebar (Settings → **Navigation Choices**, top-level + per-group sub-pages, `user_preferences.notification_prefs.sidebarNav`/`.sidebarNavChildren`); Capture grid shows only chosen shortcuts with Add gated behind **Customize** (cap 30 = 10 rows, multi-add picker); **in-app camera** on Create Memory (`components/ui/camera-capture.tsx`); removed Planning/Food Hub nav links; new custom **All Services** icon (`components/app/icons/all-services-icon.tsx`).
...
> ## 📱 2026-07-04 SESSION — Admin-mobile table overflow (Friction #9) (branch `claude/admin-table-overflow`)
> The `admin` lane's remaining item: ~24 internal `/admin/**` tables still clipped on phones (family-facing
> ones were fixed in #210). Audited every `<table>` under `app/(app)/admin`: **14 already had an
> `overflow-x-auto` wrapper, 13 did not** (admins ×2, users ×4, audit-logs, audit, content, security,
> subscriptions, support-tickets, sync). Wrapped those 13 in `<div className="overflow-x-auto">` + added
> `min-w-[720px]` to the table so columns keep their shape and scroll instead of clipping. Class-only JSX
> change — no logic, no tests needed. Verified: tsc clean, eslint clean, `next build` exit 0.
> (Chose the `admin` lane deliberately to avoid colliding with the parallel session's `ui`/`engine` work.)
