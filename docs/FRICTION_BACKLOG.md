...
| 4 | Moment prep steps are all manual checkoffs; high-confidence ones (e.g. "add team snacks") could self-complete or pre-stage. | Fold moment prep into the **Autopilot confidence engine** so ≥90% steps auto-execute reversibly (it already does this for reminders). | High | M | ★★ | engine | `lib/autopilot/*` | ✅ **DONE** — `momentPrepSuggestions` in `lib/autopilot/engine.ts` |
| 5 | Nothing reaches the family when they're **not** in the app — an imminent moment/birthday should push. | Push-notify on imminent moment/leave-by/birthday via the existing web-push + `PushNotifications` plugin + a scheduled scan (autopilot cron already runs per-family). | High | M | ★★ | platform | web-push, cron | ✅ **DONE** — web-push dispatch (`lib/server/push.ts`), `imminentMomentNotices` wired into the notification engine, delivered by the daily cron **and a new 2-hourly `/api/cron/push-scan`** so same-day moments/reminders aren't missed. *(Live delivery needs VAPID/FCM keys — degrades honestly like Stripe.)* |
...
| 7 | Home is static across the day; morning vs. night should show different "what matters now". | **Time-of-day Mission Control** — reorder/condense Home sections by hour (morning: schedule/weather/school; night: tomorrow/reflection). | Med | M | ★★ | ui | home data | ✅ **DONE** — `lib/home/time-of-day.ts` + `components/home/time-of-day-focus.tsx` |
...
| 10 | Recurring routines (school mornings, weekly practice) aren't recognized as reusable templates. | Detect recurring event clusters → offer a saved "routine" with its prep bundle. | Med | L | ★ | engine | autopilot twin | ✅ **DONE** — `0122_routine_templates.sql` + `lib/routines/detect.ts` + calendar Routines panel |
...
| 15 | No at-a-glance read of "how much still needs me" — you had to scan every card to know if you're on top of things. | Summary strip: **N upcoming · N need prep · N ready · N conflict** (`summarizeMoments`). | Med | S | ★★★ | engine | — | ✅ **DONE** (bdd3f2c+) |
...
- **2026-07-03** — Shipped **#11** (newly surfaced): smart reminder timing.
  ...
  sessions).
- **2026-07-05** — Backlog reconciled with reality: **#2, #4, #7, #10 were
  already shipped** but still read "Todo" (the same staleness that caused #10 to
  be built twice). Marked them ✅ with their landing files.
- **2026-07-05** — Shipped **#5 push notifications**. The delivery path already
  existed (`lib/server/push.ts` web-push + FCM, `push_devices`, service worker,
  subscribe/unsubscribe, settings toggle) and `imminentMomentNotices` was wired
  into `generateFamilyNotifications` — but it was only dispatched by the
  **once-daily** `/api/cron/notifications` (11:00 UTC). That entangles two
  opposite cadences: time-sensitive **pushes** (imminent moment / leave-by /
  birthday / due reminder) want to be frequent, while the **email digest** wants
  to stay daily. Symptom: a moment or reminder added after the daily run for
  later the same day was **missed until the next day** — after it mattered.
  Fix (platform lane, zero engine/ui touch): a new **`/api/cron/push-scan`**
  running **every 2h** that regenerates notifications + dispatches pending pushes
  only — no email. Composes safely with the daily cron (generation dedups by
  `related_id`, push gated on `pushed_at`, email on `sent_at`), so nothing
  double-sends. Live delivery still needs VAPID/FCM keys; degrades honestly
  (skips, never fakes success) until they're set. Backlog is now **fully cleared**
  of actionable items — remaining are human-owned (maps/ETA key for #6, GIF key,
  Stripe Issuing, prod migrations) or the contended `ui` #8 role-tailoring.
