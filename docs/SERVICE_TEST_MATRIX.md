...
| --- | --- | --- | --- | --- | --- |
| Auth and tenant isolation | `/login`, `/signup`, middleware, family switch | anonymous, member, child, parent, admin | unit, API, RLS, E2E, security | In progress | Live Auth Admin health and cross-family probes |
| Onboarding and invitations | `/onboarding`, `/join`, family settings | owner, parent, guardian, member, invited user | integration, API, E2E, email | In progress | Invite expiry, role change and tier gates |
