...
| --- | --- | --- | --- | --- | --- |
| Auth and tenant isolation | `/login`, `/signup`, middleware, family switch | anonymous, member, child, parent, admin | unit, API, RLS, E2E, security | In progress | `tests/admin-auth-boundary.test.ts`, `tests/tenant-isolation-rls.test.ts`, and `tests/ensure-family-concurrency.test.ts`; migrations 0211/0212 repair membership-update drift and first-family provisioning races, while live Auth Admin health and cross-family probes remain |
| Onboarding and invitations | `/onboarding`, `/join`, family settings | owner, parent, guardian, member, invited user | integration, API, E2E, email | In progress | Keyed replay and first-family concurrency regressions pass; live invite expiry, role change, tier gates and authenticated browser flows remain |
| Home and dashboards | `/home`, `/dashboard/*`, including `/dashboard`, `/dashboard/activity`, `/dashboard/briefing`, `/dashboard/command-center`, `/dashboard/family-operating-index`, `/dashboard/family-signals`, `/dashboard/agents`, `/dashboard/autonomous-family-management`, `/dashboard/family-automation`, `/dashboard/family-stress`, `/dashboard/family-operations`, `/family/reports`, and `/dashboard/contacts/[id]` | parent, child, admin | component, E2E, responsive, accessibility | In progress | Dashboard Home, Dashboard Activity, Dashboard Briefing, Command Center, Family Operating Index, Family Assistant primary/reasoning, and the other covered dashboard read boundaries pass; continue optional reasoning-consumer and action-by-action dashboard traversal |
...
| Marketplace | `/marketplace/*`, including `/marketplace/auctions`, `/marketplace/selling`, `/marketplace/deals`, seller flows | buyer, seller, admin | RLS, concurrency, payments, media, E2E | In progress | Live Auctions, Selling, and Deals read boundaries pass; complete buyer/seller listing to handoff, dispute, media, payment, and responsive/browser evidence |
...
| Notifications and cron | notification center, `/api/cron/*`, Admin digest | system, parent, admin | auth, retry, dedupe, delivery, observability | In progress | Admin digest, provider-sync, batch-cron, notification-delivery, and email-helper failure-summary tests pass; live replay evidence remains |
| Admin and marketing | `/admin/*`, including `/admin/admins`, `/admin/settings`, `/admin/marketing/campaigns/new`, `/admin/marketing/referrals`, `/admin/onboarding`, `/admin/marketing/intelligence`, `/admin/marketing/competitive`, `/admin/marketing/visitor-intelligence`, `/admin/marketing/lead-scores`, `/admin/marketing/personalization`, `/admin/marketing/exit-intent`, `/admin/social/audit`, `/admin/social/providers`, and `/admin/social/usage` | Super Admin, admin, support | authorization, CRUD, audit, responsive, E2E | In progress | Admin Management, Admin Settings, New Campaign, Referrals, Social Providers, notification, document-deletion, family-creation, allowlist-read, content-read, report-read, audit-read, operational-read, billing-read, notification-read, feedback-read, integration-read, subscription-read, Stripe-read, marketing-read, marketing-dashboard, Onboarding Audit, Customer Intelligence, Competitive Intelligence, Visitor Intelligence, Lead Scores, Personalization, Exit-Intent, Social Audit, Social Usage, content/campaign, asset/messaging, CRM, control-plane, publishing, rewards/survey failure boundaries pass; complete permission, storage/family rollback, live-save, responsive, and browser matrix |
