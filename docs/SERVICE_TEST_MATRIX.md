...
| Notifications and cron | notification center, `/api/cron/*`, Admin digest | system, parent, admin | auth, retry, dedupe, delivery, observability | In progress | Admin digest and provider-sync failure-summary tests pass; every scheduled job still needs replay/failure evidence |
