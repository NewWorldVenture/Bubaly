...
### S-01 · M2 + M20 — one household inbox
...
**Files this section owns** (touch nothing outside without saying so in the PR): `lib/inbox/unify.ts`, `app/(app)/dashboard/inbox/page.tsx`, `app/(app)/dashboard/inbox/actions.ts`, `components/modules/inbox-module.tsx`, `lib/contact-center/server.ts`, `lib/paperwork/triage.ts`, `app/(app)/capture/page.tsx`, `public/manifest.webmanifest`, `components/modules/front-desk-module.tsx`, `app/(app)/dashboard/front-desk/page.tsx`
...
**Files this section owns** (touch nothing outside without saying so in the PR): `lib/twin/project.ts`, `lib/twin/project-server.ts`, `lib/family/actions.ts`, `lib/reasoning/context.ts`
...
### S-03 · M10 — meal → pantry → grocery loop
...
**Files this section owns** (touch nothing outside without saying so in the PR): `app/api/ai/meals/plan/route.ts`, `lib/meals/planner.ts`, `lib/services/groceries/index.ts`, `components/modules/shopping-module.tsx`, `components/modules/meals-module.tsx`, `app/(app)/dashboard/grocery/actions.ts`, `lib/grocery/retailers.ts`
...
### S-04 · M12 — home twin asset detail
...
**Files this section owns** (touch nothing outside without saying so in the PR): `components/modules/home-module.tsx`, `app/(app)/dashboard/home/assets/[id]/page.tsx`, `lib/services/home/index.ts`, `lib/home/asset-detail.ts`, `app/(app)/dashboard/home/actions.ts`
...
### S-05 · M15 — travel disruption + pet prep
...
**Files this section owns** (touch nothing outside without saying so in the PR): `lib/vacations/disruption.ts`, `lib/vacations/conflicts.ts`, `lib/ai/planner/templates/prepare-vacation.ts`, `lib/services/trips/index.ts`, `lib/ai/tools/*`, `app/(app)/dashboard/vacations/[id]/travel/*`, `app/(app)/dashboard/vacations/[id]/actions.ts`
...
### S-06 · M21 — actionable notifications

**M21 (no-migration half) — every notification becomes actionable; low-priority types fold into the brief**

**Spec**

Read audit item M21. Verified: components/modules/notifications-module.tsx:114-138 a row click only marks read; related_type/related_id never become a link or an action; notification-bell.tsx links only to /dashboard/notifications; the brief never reads the notifications table. BUILD: (1) NEW pure lib/notifications/actions.ts: notificationAction(row) maps type + related_type/related_id → { href, inlineAction? } using the existing chore sign-off / approval decide / reminder complete actions (every notification_type in the catalogue must map to an href; inline actions only for chore_due and approval kinds); (2) render it in notifications-module.tsx and notification-bell.tsx (44px targets; accessible names); (3) WITHOUT a priority column: add a pure lib/notifications/priority.ts classifying type → 'now' | 'digest' (documented list; conservative), use it so the bell/list show 'now' types prominently and lib/briefing/build.ts folds unread 'digest'-class rows into an 'Also today' section (listUnread through lib/services/notifications; mark read on render only when the brief actually rendered them); keep notify() contract unchanged; leave a TODO naming the priority-column migration. Quiet hours must still be respected by anything that pushes. Tests per the audit: tests/notification-actions.test.ts (every type maps; related ids become deep links; inline actions only for chore_due/approval), extend tests/service-notifications.test.ts, extend tests/briefing-build.test.ts ('Also today' folds digest rows, counted once, not double-listed).

**Files this section owns** (touch nothing outside without saying so in the PR): `lib/services/notifications/index.ts`, `components/modules/notifications-module.tsx`, `components/app/notification-bell.tsx`, `lib/briefing/build.ts`, `lib/briefing/response-schema.ts`

**Tests to add**: `tests/notification-actions.test.ts`, `tests/service-notifications.test.ts (extend)`, `tests/briefing-build.test.ts (extend)`, `tests/notification-digest.test.ts (existing stays green)`

**Done when**: Every notification row has a working deep link; chore/approval rows act inline through existing actions; digest-class rows appear once in the brief's 'Also today' and are not double-listed; no notification type is unmapped (test enumerates the catalogue); existing notification tests stay green.


### S-07 · M24 — Trust Center Activity tab

**M24 (no-migration half) — Trust Center Activity tab: tool-call ledger, dials beside policies, what Bubaly read/withheld, 'based on' on approval cards**

**Spec**

Read audit item M24. Verified: the Trust page is a permissions console; evidence for a decision is a reason string; ai_request_context rows carry snapshot slice names + sensitive_omitted; ai_tool_calls exist per run; autonomy dials live in family_ai_settings (Settings) apart from trust_policies. BUILD: (1) NEW lib/trust/activity.ts (server; read boundary fail closed; managers only for sensitive rows) that unifies: recent ai_tool_calls (via lib/ai/runs/store) with links to runs, the family's autonomy dials (family_ai_settings) rendered beside trust_policies, and a per-request 'What Bubaly read / withheld' list from ai_request_context (slice names + sensitive_omitted — names of slices only, never raw payload); (2) an 'Activity' tab in components/modules/trust-module.tsx + app/(app)/dashboard/trust/page.tsx; (3) a 'Based on' expander on components/approvals/approval-card.tsx (lib/approvals/card-data.ts) listing the slice names from the request's context snapshot; (4) NO ai_context_access_log table (migration) — leave a TODO naming it. Tests: tests/trust-activity-read-boundary.test.ts (reads fail loudly; non-managers see no sensitive rows), extend tests/approval-card.test.ts ('based on' lists slice names, never raw payload).

**Files this section owns** (touch nothing outside without saying so in the PR): `components/modules/trust-module.tsx`, `app/(app)/dashboard/trust/page.tsx`, `lib/approvals/card-data.ts`, `components/approvals/approval-card.tsx`, `lib/ai/runs/store.ts`

**Tests to add**: `tests/trust-activity-read-boundary.test.ts`, `tests/approval-card.test.ts (extend)`, `tests/trust-read-boundary.test.ts (existing stays green)`

**Done when**: The Activity tab shows persisted tool calls with run links, dials beside policies, and per-request read/withheld slice names; approval cards expose 'based on' slice names only; sensitive rows are hidden from non-managers; reads fail closed; existing trust tests green.


### S-08 · M23 + M28 — access, sharing, grandparents
...
**Files this section owns** (touch nothing outside without saying so in the PR): `components/modules/trust-module.tsx`, `app/(app)/dashboard/trust/actions.ts`, `app/(app)/dashboard/grandparent-portal/page.tsx`, `lib/grandparent/digest.ts`, `lib/supabase/auth.ts`, `components/family/invite-form.tsx`
...
### S-09 · M9 — school and sports front desk
...
**Files this section owns** (touch nothing outside without saying so in the PR): `lib/front-desk/school-sports.ts`, `lib/contact-center/routing.ts`, `app/api/ai/import/route.ts`, `components/modules/school-module.tsx`, `app/(app)/dashboard/school/actions.ts`, `lib/services/school/index.ts`, `lib/services/sports/index.ts`
...
### S-10 · M18 + M37 + W6 — display and hardware
...
**Files this section owns** (touch nothing outside without saying so in the PR): `components/display/*`, `lib/display/tiles.ts`, `lib/display/wake-lock.ts`, `app/(app)/display/setup/page.tsx`, `app/(app)/display/page.tsx`, `lib/marketing/certified-devices.ts`, `lib/marketing/display-compare.ts`, `app/(marketing)/display/page.tsx`, `docs/PHYSICAL_DEVICE_TEST_PLAN.md`
...
**Files this section owns** (touch nothing outside without saying so in the PR): `lib/services/search/index.ts`, `lib/search/rank.ts`, `lib/ai/tools/*`, `lib/command-bar/route.ts`, `components/app/command-bar.tsx`, `app/(app)/dashboard/search/page.tsx`, `app/(app)/dashboard/search/actions.ts`
...
### S-12 · M36 second half — deletion requests

**M36 Security as product surface (no-migration half) — Settings › Security with TOTP MFA via Supabase Auth, Privacy Center export, access history**
...
BUILD (no migration; the deletion-request table is OUT — show a 'Request deletion' path that opens the existing support/feedback channel and says a person will confirm; the marketing /security page is owned by the site tranche — do not edit it; keep /dashboard/security (home alarm) untouched):
1. Settings › Security panel (components/settings/security-panel.tsx, mounted in the settings module next to app-lock): TOTP enrol (supabase.auth.mfa.enroll → QR + secret), verify (challengeAndVerify), list/unenrol factors; AAL display. If the project has MFA disabled, the Supabase error is shown verbatim-ish as a retryable error state — never a fake 'enabled'. Add lib/auth/mfa.ts with pure helpers (factor state → UI state) and tests.
2. Enforcement (server): lib/auth/require-aal2.ts — for managers with an enrolled TOTP factor, money/documents/trust routes call it from requireUserContext-adjacent guards (find the existing per-route gate pattern) and redirect to a step-up page app/(app)/auth/step-up/page.tsx when aal1; families with no factor are unaffected. Test the decision function.
3. Privacy Center (components/settings/privacy-center.tsx): 'Export my family's data' → route handler app/api/privacy/export/route.ts that builds family-scoped JSON through the existing services under the caller's ServiceScope (only tables the role may read; RLS applies) and streams it as an attachment — no bucket needed; log the export in trust_audit_logs if the existing audit helper supports a generic action (verify), and show 'Recent exports' from that log. 'Who accessed what' list reusing trust_audit_logs (last 50, fails closed). Session list: show the current session only (provider limitation) — say so.
--- AUDIT M36 — Security as Product Surface: MFA/passkeys, privacy center, encryption, RLS, signed files, child controls and sensitive-access audit, customer-facing — status partial ---
...
**Files this section owns** (touch nothing outside without saying so in the PR): `components/settings/security-panel.tsx`, `components/settings/privacy-center.tsx`, `components/modules/settings-module.tsx`, `lib/auth/mfa.ts`, `lib/auth/require-aal2.ts`, `app/(app)/auth/step-up/page.tsx`, `app/api/privacy/export/route.ts`, `lib/privacy/export.ts`

**Tests to add**: `tests/mfa-state.test.ts`, `tests/require-aal2.test.ts`, `tests/privacy-export.test.ts`

**Done when**: A manager can enrol/verify/unenrol TOTP through Supabase Auth with honest error states; AAL2 is required on money/documents/trust routes only for users with an enrolled factor; the export route streams role-scoped JSON and is logged; i18n gate clean.


### S-13 · M25 + M34 — outcome templates and life events
...
**Files this section owns** (touch nothing outside without saying so in the PR): `lib/ai/planner/templates/index.ts`, `lib/ai/context/intents.ts`, `lib/ai/planner/prompts.ts`, `lib/outcomes/launcher.ts`, `components/modules/outcomes-launcher.tsx`, `lib/life-events/templates.ts`, `app/(app)/dashboard/life-event-actions.ts`, `components/modules/life-events-module.tsx`, `lib/intent/detect.ts`, `lib/ai/context/slices/proactive.ts`, `lib/planning/prep-server.ts`
...
**Files this section owns** (touch nothing outside without saying so in the PR): `components/modules/assistant-module.tsx`, `components/home/ask-bar.tsx`, `components/app/command-bar.tsx`, `components/concierge/ask-bubaly.tsx`, `components/display/display-grid.tsx`, `app/api/ai/voice/transcribe/route.ts`, `app/api/ai/voice/speak/route.ts`, `mobile/app/(tabs)/assistant.tsx`, `mobile/src/lib/api.ts`
...
**Files this section owns** (touch nothing outside without saying so in the PR): `components/modules/wishlists-module.tsx`, `lib/ai/insights.ts`, `lib/ai/context/intents.ts`, `lib/twin/simulate.ts`
...
### S-16 · M22 — household memory controls
...
**Files this section owns** (touch nothing outside without saying so in the PR): `components/settings/ai-memory.tsx`, `app/(app)/dashboard/settings/ai-actions.ts`, `components/modules/knowledge-base-module.tsx`, `lib/services/memory/index.ts`
...
### S-17 · M29 + M30 — contacts import and onboarding
...
**Files this section owns** (touch nothing outside without saying so in the PR): `lib/migrate/parse.ts`, `components/migrate/migrate-wizard.tsx`, `app/(app)/dashboard/migrate/actions.ts`, `app/(app)/dashboard/migrate/page.tsx`, `app/onboarding/actions.ts`, `components/onboarding/onboarding-wizard.tsx`, `app/(app)/home/page.tsx`, `lib/outcomes/launcher.ts`
...
### S-18 · metrics core — X3, X5, X7, X10, X12
...
**Files this section owns** (touch nothing outside without saying so in the PR): `lib/metric/time-saved-server.ts`, `app/(app)/dashboard/intelligence/page.tsx`, `components/concierge/autopilot-panel.tsx`, `lib/briefing/build.ts`, `lib/ai/activity.ts`, `lib/reasoning/engine.ts`, `lib/twin/project.ts`, `lib/referrals/core.ts`, `app/(app)/admin/reports/page.tsx`, `app/(app)/dashboard/graph/page.tsx`, `components/modules/graph-module.tsx`
...
### S-19 · M38 — privacy-safe benchmarks
...
**Files this section owns** (touch nothing outside without saying so in the PR): `lib/network/aggregate-server.ts`, `lib/network/aggregate.ts`, `lib/network/contribution.ts`, `lib/network/insights.ts`, `lib/network/compare-line.ts`, `app/(app)/admin/benchmarks/page.tsx`, `app/(marketing)/resources/benchmarks/page.tsx`, `components/modules/intelligence-module.tsx`
...
**Files this section owns** (touch nothing outside without saying so in the PR): `app/(auth)/signup/page.tsx`, `app/(auth)/signup/*.tsx`, `app/(app)/onboarding/actions.ts`, `lib/referrals/server.ts`, `lib/referrals/core.ts`, `app/api/webhooks/stripe/route.ts`, `lib/stripe/*.ts`, `components/referrals/referral-panel.tsx`, `app/(app)/referrals/actions.ts`, `components/modules/settings-module.tsx`, `app/(app)/home/page.tsx`
...
### S-21 · M26 — service layer compare (rest)
...
**Files this section owns** (touch nothing outside without saying so in the PR): `components/modules/concierge-calls-module.tsx`, `lib/concierge-calls/*`, `lib/ai/planner/templates/find-vendor.ts`, `lib/services/providers/compare.ts`, `lib/ai/tools/*`, `components/modules/home-projects-module.tsx`
