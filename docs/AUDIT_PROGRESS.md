...
| --- | --- | ---: | --- | --- | --- |
| A-01 | Build, lint, typecheck, dependency and CI gates | 5 | Verified | 455 Vitest files, 3,193 tests; 250-route production build; lint/typecheck/audit | Keep green after every repair |
...
| A-08 | Wallet, goals, allowances, transfers and treasury | 7 | In progress | Atomic wallet RPCs, goal funding repair, allowance recovery tests, remaining-wallet read-boundary coverage, and family-scoped wallet hub deletion | Verify every wallet page, RPC deployment, concurrency and reconciliation |
| A-09 | Billing, subscriptions, Stripe checkout and public pay flows | 6 | In progress | Checkout, change-plan, cancel, portal, and Stripe subscription webhook state reads now fail closed; customer/tracking/sync/webhook automation failures are logged and surfaced; focused boundary tests pass | Live webhook/idempotency, tier, failure and refund smoke tests |
