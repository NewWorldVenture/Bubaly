...
| --- | --- | ---: | --- | --- | --- |
| A-01 | Build, lint, typecheck, dependency and CI gates | 5 | Verified | 463 Vitest files, 3,209 tests; 250-route production build; lint/typecheck/audit | Keep green after every repair |
...
| A-18 | Google, email, push, Stripe and other third-party integrations | 5 | In progress | Connections hub no longer reports label-only providers as live; Admin Sync and family Sync now surface provider, dead-letter, webhook-signature, connection, conflict, run, history, connected-account, and provider-detail read failures; implemented Google/Microsoft/Apple routes remain separate from unsupported provider directory entries | Live sandbox callbacks, secret rotation, retry and outage behavior; implement remaining providers |
