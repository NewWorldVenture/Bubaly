...
| LB-009 | P1 | `family-media` storage bucket is undefined in migrations and served public (PLA-0461) | No migration creates the bucket (grep clean); all four consumers use `getPublicUrl`; contrast `documents` bucket (0007, private+RLS) | Supabase owner (prod storage migration is human-owned) + cross-module security/UX decision | Add idempotent bucket-creation migration with family-folder write RLS; decide public vs. signed-URL reads and switch consumers accordingly; add bucket to harness/seed | Open |
