...
| --- | --- | --- | --- | --- | --- | --- |
| LB-001 | P0 | Supabase Auth Admin users health check returns HTTP 500 | `npm.cmd run db:audit:auth` latest request `019f6612-b504-764d-ba45-1f0f81e7d090` | Supabase project/operator access | Admin users endpoint returns healthy response and auth smoke passes | Open |
| LB-002 | P0 | Remote migration ledger is not verified against checkout | Current audit scripts are not linked to a confirmed remote migration history; local checkout now has 228 migrations through 0212 | Supabase project access | Reconcile all 228 migrations with remote ledger and safely apply 0208, 0210, 0211, and 0212 | Open |
