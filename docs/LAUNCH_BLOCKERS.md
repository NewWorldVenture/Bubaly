...
| --- | --- | --- | --- | --- | --- | --- |
| LB-001 | P0 | Supabase Auth Admin users health check returns HTTP 500 | `npm.cmd run db:audit:auth` historical audit evidence | Supabase project/operator access | Admin users endpoint returns healthy response and auth smoke passes | Open |
| LB-002 | P0 | Remote migration ledger is not verified against checkout | Current audit scripts are not linked to a confirmed remote migration history | Supabase project access | Reconcile all 224 migrations with remote ledger and safely apply 0208 | Open |
