...
| **F-G08** | A child can write `allowance_rules`, and the service-role allowance cron mints the money | **OPEN** — evidenced by Pass G's Claude-3 on a live replay |
...
|---|---|---|---|
| **F-G08** | CRITICAL | **A child can write `allowance_rules`, and the service-role cron mints the money.** 0217 narrowed writes to `can_manage_family` on five wallet tables; `allowance_rules` was in the same 0088 loop and is in none of 0217/0254/0267/0275, so it still carries `FOR ALL … is_family_member`. The direct mint 0217 closed is reopened one level up: a child inserts a *rule*, and the nightly cron — service role, RLS bypassed — credits `rule.amount_cents` with no check on its author. Proven on the replay: the child's `wallet_transactions` insert is refused and the `allowance_rules` insert of 999,999 succeeds. | `audit/claude-3.md` |
| **F-G09** | HIGH | Health and medication records are readable **and writable** by a child — `medications`, `medication_schedules`, `medication_doses`, `immunizations`, `health_metrics` and seven more resolve to membership-only policies. The client computes `canEdit = isManager(role)`, but that is a React boolean over a browser anon client. Proven: the child session read a parent's prescription, edited the dosage, then deleted the row. | `audit/claude-3.md` |
| **F-G10** | MEDIUM | A child can rewrite their own `grades` and raise their own `screen_time_limits`. The two parental-control surfaces a child has the most motive to edit are editable by that child from any HTTP client. | `audit/claude-3.md` |
| **F-G11** | MEDIUM | `child_logins`' write policy is **named** "Managers manage" and predicated on membership. Not an escalation — sign-in derives both email and password from `row.username` — but a child can delete a sibling's login row, which is a denial of service against an access-control record. | `audit/claude-3.md` |
