...
import type { CalendarContext } from '@/lib/database.types';
    ...
    try {
      const res = await fetch('/api/ai/schedule', {
        ...
        body: JSON.stringify({
          ...
          contexts: context === 'family' ? undefined : [context],
    ...
    <Modal open title="Find a time" onClose={onClose}>
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            <label className="mb-1.5 block text-xs font-semibold text-muted">Lens</label>
            <div className="flex flex-wrap gap-1.5">
              {CONTEXTS.map((c) => (
                <button key={c} type="button" onClick={() => { setContext(c); setSearched(false); }}
                  className={cn('flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-xs font-medium transition',
                    context === c ? cn('border-transparent', CONTEXT_META[c].chip) : 'border-border bg-surface/40 text-muted hover:text-fg')}>
