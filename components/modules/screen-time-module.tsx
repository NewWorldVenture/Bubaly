...
export function ScreenTimeModule() {
  ...
  async function remove(id: string) {
    ...
    const { data: removed, error } = await createClient().from('screen_time_entries').delete().eq('id', id).select('id');
    if (error) toastError(describeDbError(error)); else if (wroteNoRows(removed)) toastError(t('errors.thatChangeWasNotSaved')); else success(t('screenTimeModule.deleted'));
  }
