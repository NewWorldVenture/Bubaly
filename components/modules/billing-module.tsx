    ...
    <div className="module-with-sidebar">
      <div className="module-main module-page">
        <PageHeader
          ...
          action={<button className="btn-cta"><Plus className="h-4 w-4" /> Add Transaction</button>}
        />
        ...
        <div className="rounded-2xl border border-border bg-surface/40 p-5">
          <div className="mb-5 flex items-center justify-between">
            ...
            <div className="flex items-center gap-4 text-xs">
              ...
              <button className="text-muted hover:text-fg/80">6 months</button>
            </div>
          </div>
        ...
        <div className="grid gap-5 lg:grid-cols-2">
          <div className="rounded-2xl border border-border bg-surface/40 p-5">
            <div className="mb-4 flex items-center justify-between"><h2 className="font-semibold">Recent Transactions</h2><button className="text-xs font-semibold text-brand">View all &rarr;</button></div>
            <div className="space-y-1">
              {TRANSACTIONS_MOCK.slice(0, 6).map((tx, i) => (
                <div key={i} className="flex items-center gap-3 rounded-xl px-2 py-2.5 hover:bg-surface/40">
                  <div className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-surface/40 text-xl">{tx.icon}</div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold truncate">{tx.name}</p>
                    <p className="text-xs text-muted">{tx.cat} · {tx.date}</p>
                  </div>
            ...
            <button className="mt-3 flex items-center gap-1 text-xs font-semibold text-brand">View all transactions <ChevronRight className="h-3.5 w-3.5" /></button>
          </div>
          <div className="rounded-2xl border border-border bg-surface/40 p-5">
            <div className="mb-4 flex items-center justify-between"><h2 className="font-semibold">Budget Overview</h2><button className="text-xs font-semibold text-brand">Manage budgets &rarr;</button></div>
                ...
                return (
                  <div key={b.cat}>
                    <div className="mb-1.5 flex items-center justify-between text-sm">
                      ...
                      <span className={cn('text-xs font-semibold', over ? 'text-red-400' : 'text-muted')}>{fmtCurrency(b.spent)} / {fmtCurrency(b.budget)}</span>
                    </div>
        ...
        {/* Savings Goals */}
        <div className="rounded-2xl border border-border bg-surface/40 p-5">
          <div className="mb-4 flex items-center justify-between"><h2 className="font-semibold">Savings Goals</h2><button className="text-xs font-semibold text-brand">View all goals &rarr;</button></div>
            ...
            {SAVINGS_GOALS.map((g) => {
              ...
              return (
                <div key={g.name} className="rounded-xl border border-border p-4">
                  ...
                  <div className="mb-2 h-2 rounded-full bg-border"><div className="h-full rounded-full" style={{ width: `${pct}%`, background: g.color }} /></div>
      ...
      <aside className="module-sidebar hidden lg:flex lg:flex-col gap-5">
        <div className="rounded-2xl border border-border bg-surface/40 p-5">
          <div className="mb-4 flex items-center justify-between"><h2 className="font-semibold">Accounts</h2><button className="text-xs font-semibold text-brand">Manage &rarr;</button></div>
          ...
          <button className="mt-3 flex w-full items-center justify-center gap-1.5 rounded-xl border border-border py-2.5 text-xs font-semibold text-muted hover:text-fg"><Plus className="h-3.5 w-3.5" /> Add Account</button>
        </div>
        ...
        <div className="rounded-2xl border border-border bg-surface/40 p-5">
          ...
          <div className="flex items-center gap-4">
            <div className="relative h-24 w-24 shrink-0">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center"><span className="text-lg font-black">May</span><span className="text-[9px] text-muted">2024</span></div>
            </div>
            <div className="space-y-1.5">
              {SPEND_BREAKDOWN.map(({ label, pct, color }) => (
                <div key={label} className="flex items-center gap-2 text-xs">
                  ...
                  <span className="flex-1 text-muted">{label}</span><span className="font-bold">{pct}%</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="rounded-2xl border border-border bg-surface/40 p-5">
          <div className="mb-4 flex items-center justify-between"><h2 className="font-semibold">Upcoming Bills</h2><button className="text-xs font-semibold text-brand">View all &rarr;</button></div>
          <div className="space-y-3">
            {BILLS_MOCK.map((b) => (
              <div key={b.name} className="flex items-center gap-3">
                <span className="text-xl">{b.icon}</span>
                <div className="flex-1"><p className="text-sm font-semibold">{b.name}</p><p className="text-xs text-muted">Due {b.date}</p></div>
