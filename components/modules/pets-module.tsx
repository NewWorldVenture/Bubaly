...
import type { PetsInsights, PetsAIResponse } from '@/lib/pets/pets-ai';
import {
...
export function PetsModule() {
  ...
  const [aiAnalysis, setAiAnalysis] = useState<PetsInsights | null>(null);
  const [aiInsights, setAiInsights] = useState<PetsAIResponse | null>(null);

  async function runAiAssist() {
    setAiLoading(true);
    try {
      const res = await fetch('/api/ai/pets', { method: 'POST' });
      ...
      <PageHeader
        ...
        action={
          <div className="flex items-center gap-2">
            ...
            <Button onClick={() => setAddPetOpen(true)}><Plus className="h-4 w-4" /> Add pet</Button>
          </div>
        }
      />
        ...
        <div className="rounded-xl border border-brand/30 bg-brand/5 p-4">
          <div className="mb-2 flex items-center justify-between">
            <div className="flex items-center gap-2 text-sm font-semibold text-brand">
              <Sparkles className="h-4 w-4" /> AI Pet Insights
            </div>
          ...
          {aiInsights?.careTips && aiInsights.careTips.length > 0 && (
            <div className="mb-2">
              <p className="text-xs font-medium mb-1">Care Tips</p>
              <ul className="space-y-1">{aiInsights.careTips.map((s, i) => <li key={i} className="text-xs text-muted">{'•'} {s}</li>)}</ul>
            </div>
          )}
          {aiInsights?.healthReminder && <p className="text-xs text-muted italic">{aiInsights.healthReminder}</p>}
        </div>
      )}
