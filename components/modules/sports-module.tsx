    ...
    <div className="module-with-sidebar">
      <div className="module-main module-page">
        <PageHeader
          ...
          action={<button onClick={() => setOpen(true)} className="btn-cta"><Plus className="h-4 w-4" /> Add Event</button>}
        />
        ...
        <div className="rounded-2xl border border-border bg-surface/40">
          <div className="flex items-center justify-between p-5">
            ...
            <button className="flex items-center gap-1 text-xs font-semibold text-brand">View full schedule <ChevronRight className="h-3.5 w-3.5" /></button>
          </div>
                  ...
                  return (
                    <tr key={e.id} className="hover:bg-surface/20">
                      ...
                      <td className="px-4 py-3.5">
                        {member ? <div className="flex items-center gap-2"><Avatar name={member.display_name} color={member.color} size={28} /><span>{member.display_name.split(' ')[0]}</span></div> : <span className="text-muted/60">—</span>}
                      </td>
                      <td className="px-4 py-3.5">
                        <div className="flex items-center gap-2">
                          <div className={cn('h-2.5 w-2.5 rounded-full', SPORT_COLORS[i % SPORT_COLORS.length])} />
                          <span className="text-fg">{e.sport || 'Sports'}</span>
                        </div>
                      </td>
                      <td className="px-4 py-3.5">
          ...
          <div className="border-t border-border p-4 text-center">
            <button className="mx-auto flex items-center gap-1 text-xs font-semibold text-brand">View full schedule <ChevronRight className="h-3.5 w-3.5" /></button>
          </div>
        </div>
        <div className="grid gap-5 lg:grid-cols-2">
          <div className="rounded-2xl border border-border bg-surface/40 p-5">
            <div className="mb-4 flex items-center justify-between"><h2 className="font-semibold">My Teams</h2><button className="text-xs font-semibold text-brand">View all →</button></div>
            <div className="space-y-3">
              {TEAMS_MOCK.map((team, i) => (
                <div key={team.team} className="flex items-center gap-3 rounded-xl border border-border p-3">
                  <div className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-surface/40 text-xl">{SPORT_EMOJIS[team.sport] ?? '🏆'}</div>
                  <div className="flex-1 min-w-0">
                    <p className="font-semibold">{team.team}</p>
                    <p className="text-xs text-muted">{team.sport} · {members[i % Math.max(members.length, 1)]?.display_name.split(' ')[0] ?? 'Family'}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-sm font-bold">{team.record}</p>
                    <p className="text-xs text-muted">{team.position} Place</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="rounded-2xl border border-border bg-surface/40 p-5">
            <div className="mb-4 flex items-center justify-between"><h2 className="font-semibold">Recent Results</h2><button className="text-xs font-semibold text-brand">View all →</button></div>
            <div className="space-y-3">
              {RESULTS_MOCK.map((r, i) => (
                <div key={i} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0"><p className="text-sm font-semibold">{r.team} vs {r.vs}</p><p className="text-xs text-muted">{r.sport} · {r.date}</p></div>
      ...
      <aside className="module-sidebar hidden lg:flex lg:flex-col gap-5">
        <div className="rounded-2xl border border-border bg-surface/40 p-5">
          <div className="mb-4 flex items-center justify-between"><h2 className="font-semibold">Upcoming Games</h2><button className="text-xs font-semibold text-brand">View schedule →</button></div>
          <div className="space-y-3">
        ...
        <div className="rounded-2xl border border-border bg-surface/40 p-5">
          <div className="mb-4 flex items-center justify-between"><h2 className="font-semibold">Team Standings</h2><button className="text-xs font-semibold text-brand">View all →</button></div>
          <table className="w-full text-xs">
            <thead><tr className="text-muted"><th className="pb-2 text-left">#</th><th className="pb-2 text-left">Team</th><th className="pb-2 text-right">W</th><th className="pb-2 text-right">L</th><th className="pb-2 text-right">PCT</th></tr></thead>
            <tbody className="divide-y divide-white/5">
              {STANDINGS_MOCK.map((s) => (
                <tr key={s.team} className="font-medium">
                  <td className="py-2 text-muted">{s.pos}</td>
                  ...
                  <td className="py-2 text-right text-muted">{s.pct}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="rounded-2xl border border-border bg-surface/40 p-5">
          ...
          <div className="space-y-2">
            {['Add a game or event', 'View full schedule', 'Message a coach', 'Track team expenses'].map((action) => (
              <button key={action} onClick={action === 'Add a game or event' ? () => setOpen(true) : undefined} className="flex w-full items-center justify-between rounded-xl border border-border px-4 py-2.5 text-sm hover:border-border">
                {action} <ChevronRight className="h-3.5 w-3.5 text-muted/60" />
              </button>
            ))}
          </div>
        </div>
          ...
          <div className="space-y-3">
            {NEWS_MOCK.map((n) => (
              <div key={n.title} className="flex gap-3">
                ...
                <div><p className="text-sm font-semibold leading-snug">{n.title}</p><p className="mt-0.5 text-xs text-muted/60">{n.time}</p></div>
              </div>
            ))}
          </div>
        </div>
