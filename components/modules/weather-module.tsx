...
export function WeatherModule() {
  ...
  async function addCity(r: GeoResult) {
    ...
    success(t('weather.addedCity', { name: r.name }));
      ...
      {adding && (
        <div className="rounded-2xl border border-border bg-surface/40 p-4">
          <form onSubmit={runSearch} className="flex gap-2">
            ...
            <button aria-label={t('iconAction.cancel')} type="button" onClick={() => { setAdding(false); setQuery(''); setResults([]); }} className="grid h-10 w-10 place-items-center rounded-xl border border-border text-muted hover:bg-elevated"><X className="h-4 w-4" /></button>
          </form>
          {results.length > 0 && (
