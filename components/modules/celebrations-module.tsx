            ...
            return (
              <li key={c.id} className={`flex items-center gap-3 rounded-2xl border p-4 ${soon ? 'border-brand/40 bg-brand/5' : 'border-border bg-surface/40'}`}>
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-muted">{new Date(c.nextDate + 'T00:00:00').toLocaleDateString(locale, { weekday: 'long', month: 'long', day: 'numeric' })}</p>
                </div>
                ...
                {admin && c.id.startsWith('d-') && (
                  <button aria-label={t('iconAction.delete')} onClick={() => remove(c.id)} className="rounded-lg p-1.5 text-muted hover:bg-elevated hover:text-danger"><Trash2 className="h-4 w-4" /></button>
                )}
              </li>
            );
          })}
