...
export function ReportModeration({ id, status }: { id: string; status: string }) {
  ...
  if (resolved) return <span className="text-xs text-muted">Resolved ({status})</span>;
  ...
  return (
    <div className="mt-2 space-y-2">
      ...
      <label className="flex items-center gap-1.5 text-xs text-muted">
        <input type="checkbox" checked={withdraw} onChange={(e) => setWithdraw(e.target.checked)} /> Withdraw the listing when actioning
      </label>
      <div className="flex flex-wrap gap-2">
        <button onClick={() => run(() => resolveReportAction({ id, status: 'actioned', resolution, withdrawListing: withdraw }))} disabled={pending}
          ...
          {pending ? <Loader2 className="h-3 w-3 animate-spin" /> : <ShieldX className="h-3 w-3" />} Action
        </button>
