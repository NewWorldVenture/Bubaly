...
export function TripItinerary({ vacationId }: { vacationId: string }) {
  ...
  async function generateDays() {
    ...
    setBusy(false);
    if (error) toastError(error.message); else success(t('tripItinerary.addedDaysCount', { count: toAdd.length }));
  }
                  ...
                  return (
                    <div key={part.value} className="rounded-xl border border-border/60 bg-elevated/30 p-2">
                      <div className="mb-1.5 flex items-center justify-between">
                        ...
                        <button aria-label={t('iconAction.add')} onClick={() => setForm(blankItem(day.id, part.value))} className="rounded p-0.5 text-muted hover:text-brand-text"><Plus className="h-3.5 w-3.5" /></button>
                      </div>
                          ...
                          <li key={it.id} className="group rounded-lg bg-surface/60 p-2 text-sm">
                            <div className="flex items-start justify-between gap-1">
                              ...
                              <span className="hidden shrink-0 gap-0.5 group-hover:flex">
                                <button aria-label={t('iconAction.edit')} onClick={() => editItem(it)} className="text-muted hover:text-fg"><Pencil className="h-3 w-3" /></button>
                                <button aria-label={t('iconAction.delete')} onClick={() => removeItem(it.id)} className="text-muted hover:text-danger"><Trash2 className="h-3 w-3" /></button>
                              </span>
                            </div>
