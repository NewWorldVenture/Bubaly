            ...
            <li key={row.id} className="rounded-2xl border border-border bg-surface/40 p-4">
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="flex shrink-0 gap-1">
                  <button aria-label={tr('iconAction.edit')} onClick={() => setForm(fromRow(row, fields))} className="rounded-lg p-1.5 text-muted hover:bg-elevated hover:text-fg"><Pencil className="h-4 w-4" /></button>
                  <button aria-label={tr('iconAction.delete')} onClick={() => remove(row.id)} className="rounded-lg p-1.5 text-muted hover:bg-elevated hover:text-danger"><Trash2 className="h-4 w-4" /></button>
                </div>
              </div>
            </li>
          ))}
