...
export function TripPacking({ vacationId }: { vacationId: string }) {
  ...
  async function generate() {
    ...
    setBusy(false);
    if (error) toastError(error.message); else success(tr('tripPacking.addedSuggestedItems', { count: toAdd.length }));
  }
              ...
              <ul className="space-y-1">
                {byCategory.get(cat.value)!.map((it) => (
                  <li key={it.id} className="group flex items-center gap-2 text-sm">
                    ...
                    <button aria-label={tr('iconAction.delete')} onClick={() => remove(it.id)} className="hidden text-muted hover:text-danger group-hover:block"><Trash2 className="h-3.5 w-3.5" /></button>
                  </li>
                ))}
              </ul>
            </div>
