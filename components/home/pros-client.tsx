          ...
          {contractors.map((c) => (
            <Card key={c.id}>
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                ...
                <button aria-label={tr('iconAction.delete')} onClick={() => start(async () => { await deleteContractorAction(c.id); })} className="ml-auto inline-flex h-8 items-center gap-1 rounded-lg px-2 text-xs text-muted hover:text-danger"><Trash2 className="h-3.5 w-3.5" /></button>
              </div>
            </Card>
          ))}
        </div>
