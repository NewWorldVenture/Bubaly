...
export async function FrontDoorHero({ frontDoor, canDecide = false }: { frontDoor: FrontDoor; canDecide?: boolean }) {
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
            ...
            {doneCount > 0 && (
              <div className="rounded-xl border border-border bg-surface/50 p-3">
                <div className="mb-1.5 flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wide text-emerald-500">
                  <Check className="h-3.5 w-3.5" />{' '}{t('frontDoorHero.doneForYou')}</div>
            ...
            {pendingCount > 0 && (
              <div className="rounded-xl border border-border bg-surface/50 p-3">
                <div className="mb-1.5 flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wide text-amber-500">
                  <BellRing className="h-3.5 w-3.5" />{' '}{t('frontDoorHero.waitingOnYou')}</div>
          ...
          <Link href="/dashboard/calm"
            className="mt-3 inline-flex items-center gap-1 text-xs font-semibold text-muted transition hover:text-brand-text">{t('frontDoorHero.seeEverythingInCalm')}{' '}<ArrowRight className="h-3 w-3" />
          </Link>
        </div>
      </div>
    </section>
