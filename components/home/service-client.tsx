    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted">{t('serviceClient.aCompleteRepairAmpMaintenanceHistory')}</p>
        </div>
            ...
            <tbody>
              {records.map((r) => (
                <tr key={r.id} className="border-t border-border">
                  ...
                  <td className="px-3 py-2 text-right"><button aria-label={t('iconAction.delete')} onClick={() => start(async () => { await deleteServiceRecordAction(r.id); })} className="text-muted hover:text-danger"><Trash2 className="h-4 w-4" /></button></td>
                </tr>
              ))}
            </tbody>
          </table>
