...
export function HomeApprovalActions({ approvalId, kind }: { approvalId: string; kind: string }) {
  ...
  if (done) return <span className="shrink-0 text-xs font-semibold text-muted">{t('homeApprovalActions.done')}</span>;

  return (
    <div className="flex shrink-0 items-center gap-1.5">
      <button onClick={() => decide('approved')} disabled={busy !== null} aria-label={t('homeApprovalActions.approve')}
      ...
      <button onClick={() => decide('rejected')} disabled={busy !== null} aria-label={t('homeApprovalActions.decline')}
