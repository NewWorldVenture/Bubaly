...
export function CalendarSyncPanel() {
  ...
  async function remove(feed: CalendarFeed) {
    if (!confirm(t('confirmPrompt.removeFeedWithEvents', { name: feed.name }))) return;
            ...
            return (
              <div key={feed.id} className="flex items-center gap-3 rounded-xl border border-border bg-surface/40 px-4 py-3">
                ...
                <div className="flex items-center gap-1 shrink-0">
                  ...
                  <button aria-label={t('iconAction.delete')} onClick={() => remove(feed)} className="rounded p-1.5 text-muted hover:text-danger transition">
