...
export async function AiHomeDashboard({ ctx }: { ctx: UserContext }) {
  ...
  // Quick Access reflects the family's real plan tier so the "Unlock more" upsell
  // (locked tiles → /pricing) stays visible and demonstrable for everyone,
  // including super-admins. Super-admins still get full ACCESS to every feature
  // via the sidebar nav (featureAccessByTier treats them as having access); this
  // section is the discovery/upgrade surface, intentionally tier-gated.
