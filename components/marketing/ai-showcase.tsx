    ...
    <div className="grid gap-6 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)] lg:items-stretch">
        ...
        {DEMOS.map((d, i) => {
          ...
          return (
            ...
            >
              ...
              <span className={on ? 'font-medium text-fg' : 'text-muted group-hover:text-fg'}>
                {tr(d.prompt)}
              </span>
            </button>
          );
      ...
      >
        ...
        <div className="relative flex items-center gap-2.5 border-b border-white/10 pb-4">
          ...
          <span className="ml-auto inline-flex items-center gap-1.5 rounded-full bg-emerald-500/10 px-2.5 py-1 text-[11px] font-medium text-emerald-400">
        ...
        <div key={active} className="assistant-message-enter relative mt-4 space-y-3">
          <div className="ml-auto w-fit max-w-[92%] rounded-2xl rounded-br-md border border-white/10 bg-white/[0.08] px-4 py-2.5 text-sm font-medium text-fg shadow-lg backdrop-blur-xl">
            {tr(demo.prompt)}
          </div>
