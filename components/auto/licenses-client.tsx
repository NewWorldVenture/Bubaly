  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-sm font-semibold">Driver&apos;s licenses</h2>
        <Button onClick={() => { setEditing(null); setOpen(true); }}><Plus className="h-4 w-4" /> Add license</Button>
      </div>

      {licenses.length === 0 ? (
        <EmptyState icon={IdCard} title="No licenses yet" description="Store each driver's license with its renewal date so nobody drives on an expired one." action={<Button onClick={() => { setEditing(null); setOpen(true); }}><Plus className="h-4 w-4" /> Add license</Button>} />
      ) : (
      ...
      <Modal open={open} onClose={() => setOpen(false)} title={editing ? 'Edit license' : 'Add license'}>
        <form action={(fd) => start(async () => { await saveLicenseAction(fd); setOpen(false); })} className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field label="Holder name"><Input name="holder_name" required defaultValue={editing?.holder_name ?? ''} /></Field>
            <Field label="Family member"><Select name="member_id" defaultValue={editing?.member_id ?? ''}><option value="">—</option>{members.map((m) => <option key={m.id} value={m.id}>{m.display_name ?? 'Member'}</option>)}</Select></Field>
          </div>
          <div className="grid grid-cols-3 gap-3">
            <Field label="License #"><Input name="license_number" defaultValue={editing?.license_number ?? ''} /></Field>
            <Field label="State"><Input name="state" maxLength={2} defaultValue={editing?.state ?? ''} placeholder="CA" /></Field>
            <Field label="Class"><Input name="license_class" defaultValue={editing?.license_class ?? ''} placeholder="C" /></Field>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <Field label="Issued"><Input type="date" name="issued_on" defaultValue={editing?.issued_on ?? ''} /></Field>
            <Field label="Expires"><Input type="date" name="expires_on" defaultValue={editing?.expires_on ?? ''} /></Field>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <Field label="Endorsements"><Input name="endorsements" defaultValue={editing?.endorsements ?? ''} /></Field>
            <Field label="Restrictions"><Input name="restrictions" defaultValue={editing?.restrictions ?? ''} placeholder="Corrective lenses" /></Field>
          </div>
