              ...
              <tbody>
                {records.map((r) => (
                  <tr key={r.id} className="border-t border-border">
                    ...
                    <td className="px-3 py-2 text-right"><button aria-label={t('a11y.delete')} onClick={() => start(async () => { await deleteAutoServiceAction(r.id); })} className="text-muted hover:text-danger"><Trash2 className="h-4 w-4" /></button></td>
                  </tr>
                ))}
              </tbody>
            </table>
