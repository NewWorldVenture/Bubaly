...
describe('autopilot resolve reminder insert fails visibly', () => {
  ...
  it('captures the family_reminders insert error', () => {
    expect(b).toContain("const { error: remErr } = await supabase.from('family_reminders').insert");
    expect(b).toContain('if (remErr) return toastError(describeDbError(remErr))');
  });

  it("only marks the suggestion executed / claims success after the insert guard", () => {
    const guard = b.indexOf('if (remErr) return toastError');
