...
vi.mock('@/lib/auth/recovery-cookies', () => ({
  createRecoveryCookieExchange: async () => ({ client: await seam.factory(), applyTo: vi.fn(), dispose: vi.fn() }),
  createPkceCookieExchange: async () => ({ client: await seam.factory(), applyTo: vi.fn(), dispose: vi.fn() }),
}));
