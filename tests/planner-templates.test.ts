...
describe('workflow templates are runnable skeletons', () => {
  ...
  it('covers the seven signature workflows, the two proactive asks and the move', () => {
    expect(allTemplates().map((t) => t.intent).sort()).toEqual([
      'daily_brief', 'find_vendor', 'organize_weekend', 'plan_meals', 'plan_move', 'plan_week', 'prepare_vacation', 'remind_everyone', 'spending_review', 'what_am_i_forgetting',
    ]);
