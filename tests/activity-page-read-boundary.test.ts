...
describe('activity page read boundary', () => {
  it('fails visibly when a source or enrichment read fails', () => {
    ...
    expect(page).toContain('Could not load family activity from Supabase. Refresh and try again.');
    expect(page).toContain('return <ErrorState message="Could not load family activity from Supabase. Refresh and try again." />;');
  });
});
