 ...
 * `0300_social_tokens_service_role_only.sql` drops them again.
 *
...
describe('the social OAuth token store is reachable only by the service role', () => {
  it('no migration after 0300 creates a policy on it', () => {
    ...
    expect(offenders, 'this table is service-role only — see migration 0034 and 0300').toEqual([]);
  });

  it('0300 drops all four policies 0297 added', () => {
    const sql = readFileSync(`${MIGRATIONS}/0300_social_tokens_service_role_only.sql`, 'utf8');
