...
describe('Calm dashboard read boundary', () => {
  it('preserves inbox and reasoning failures before building the inbox', () => {
    ...
    expect(page).toContain('Could not load your Calm inbox from Supabase. Refresh and try again.');
    expect(page).toContain('return <ErrorState message="Could not load your Calm inbox from Supabase. Refresh and try again." />;');
  });
});
