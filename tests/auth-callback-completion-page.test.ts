...
describe('direct completion page admission', () => {
  it('awaits Next 15 searchParams and passes one admitted code and internal destination', async () => {
    ...
    expect(tree.props).toEqual({ code: 'synthetic-code', next: '/dashboard/meals?week=next' });
  ...
  it.each<[string, Query]>([
    ...
    expect((await page(query)).props).toEqual({ code: null, next: '/home' });
  });
  ...
  it.each<[string, Query, string]>([
    ...
    expect((await page({ code: 'synthetic-code', ...query })).props).toEqual({ code: 'synthetic-code', next: expected });
  });
  ...
  it.each(['access_token', 'refresh_token', 'id_token', 'token_hash'])('rejects a direct code mixed with %s query credentials', async name => {
    expect((await page({ code: 'synthetic-code', [name]: 'synthetic-secret' })).props).toEqual({ code: null, next: '/home' });
  });
