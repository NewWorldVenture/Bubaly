...
/**
 * Every catalogue key the file names with a given prefix — whether inline as
 * t('…') or held in a module-level data array and rendered through t(x.title)
 * later. The page keeps only keys at module level (an English label beside a
 * key would be a literal on the gated marketing surface), so a quoted string
 * under the prefix IS a key the page renders.
 */
