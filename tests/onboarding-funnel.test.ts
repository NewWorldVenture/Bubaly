...
// 3 sessions through the 7-step flow (profile → family → value → about → members
// → pin → done): s1 completes, s2 drops at pin, s3 drops at profile.
const events: OnboardingEventLike[] = [
  ...
  { session_id: 's1', step: 'value', phase: 'step', duration_ms: 3000, created_at: '2026-07-01T00:00:03Z' },
  ...
  { session_id: 's2', step: 'value', phase: 'step', duration_ms: 3000, created_at: '2026-07-01T00:01:03Z' },
...
describe('summarizeOnboardingFunnel', () => {
  it('counts distinct sessions reaching each step', () => {
    ...
    expect(byKey).toEqual({ profile: 3, family: 2, value: 2, about: 2, members: 2, pin: 2, done: 1 });
  });
  ...
  it('reports the biggest drop-off step', () => {
    ...
    for (const s of sessions) {
      for (const step of ['profile', 'family', 'value', 'about', 'members']) {
