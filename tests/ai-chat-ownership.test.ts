...
describe('AI chat conversation ownership', () => {
  it('rechecks client-supplied conversation IDs against family and user ownership', () => {
    ...
    expect(source).toContain("return NextResponse.json({ error: t('chat.conversationNotFound') }, { status: 404 });");
  });
  ...
  it('does not expose history or persist messages when the ownership check fails', () => {
    ...
    expect(ownershipBlock).toContain("return NextResponse.json({ error: t('chat.couldNotOpenThisConversation') }, { status: 503 });");
    expect(ownershipBlock).toContain("return NextResponse.json({ error: t('chat.conversationNotFound') }, { status: 404 });");
