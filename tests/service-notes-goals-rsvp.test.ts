// The three writers that let a gated approval actually execute.
//
// Before these, `notes`, `goals` and `event_rsvps` were written only by the
// hand-written chat tools, so `executeTool` — which the approval replay goes
// through — had no tool to resolve and a parent who approved "save a note" read
// back "Bubaly has no tool called add_note".
//
// The headline assertions are the two keys and the one lookup:
//   * notes.created_by / goals.created_by reference auth.users (0002), so the
//     AUTH id belongs there.
//   * event_rsvps.member_id references family_members (0047), so the MEMBER id
//     belongs there — and it comes from the scope, never from arguments,
//     because 0047's policy has no member predicate at all.
//   * the event lookup takes the NEXT occurrence, not the furthest-out one.
