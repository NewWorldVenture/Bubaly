...
/**
 * Names read as `process.env.NAME` — the spelling a scanner can be SURE about.
 *
 * Used for the "everything read is documented" direction, where a false positive
 * is an accusation: it demands a template line for a name that may be nothing
 * more than a string in a table.
 */
...
/**
 * The same, plus the names a file could plausibly reach INDIRECTLY.
 *
 * Two files index `process.env` by a variable rather than naming it inline:
 * `lib/server/native-push.ts` does it in `privateKeyEnv(name)` so one function
 * can undo the literal `\n` a PEM picks up when pasted into a dashboard field,
 * and `lib/social/capabilities.ts` does it over a `credentialEnv` table so each
 * provider declares its own credentials once.
 *
 * This is used for the OPPOSITE direction — "the template offers nothing that is
 * never read" — and the asymmetry is deliberate. There, a false positive deletes
 * a working template line: the dotted-only scanner reported FCM_PRIVATE_KEY and
 * APNS_PRIVATE_KEY as orphans when both are read on every native push. So each
 * direction is generous in the way that avoids making a wrong accusation, rather
 * than one scanner being tuned to split the difference and being wrong twice.
 */
function envNamesPossiblyReadBy(paths: string[]): Set<string> {
...
describe('.env.example covers the runtime configuration surface', () => {
  ...
  const sources = [...ROOTS.flatMap((r) => walk(r)), ...FILES];
  const read = envNamesReadBy(sources);
  const maybeRead = envNamesPossiblyReadBy(sources);
  ...
  it('does not offer a variable no code reads', () => {
    ...
    const orphaned = [...assigned]
      .filter((name) => !maybeRead.has(name) && !scriptsAndTooling.has(name))
      .sort();
