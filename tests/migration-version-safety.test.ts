...
describe('Supabase migration filename safety', () => {
  ...
  it('points new migrations at the next unused version', () => {
    // Bumped whenever a migration lands — 0310 gives a Guardian number to one
    // family, which is the only scope that can see the clash. Stating it rather
    ...
    expect(audit.nextVersion).toBe('0311');
  });
