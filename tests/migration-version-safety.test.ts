...
describe('Supabase migration filename safety', () => {
  ...
  it('points new migrations at the next unused version', () => {
    ...
    //
    // This reads 0339: main holds 0001-0317, this branch 0318-0327, 0328-0332,
    ...
    // migration on main names), 0337 (an email event is counted once — a
    // column on resend_webhook_events and a service-only function) and 0338
    // (a child's milestones are a parent's to mark — policies on
    // independence_milestones, which no migration on main touches). 0333 touches only guardian_contacts and
