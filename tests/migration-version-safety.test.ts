...
describe('Supabase migration filename safety', () => {
  ...
  it('points new migrations at the next unused version', () => {
    // Bumped whenever a migration lands — 0340 applies a campaign counter once
    // per webhook event (EMAIL-002); 0339 adds per-recipient push
    // delivery receipts (PUSH-003); 0338 makes family media private
    // (SEC-001, deploy-coupled); 0337 lets only a parent make,
    ...
    // server can actually resolve, after 0322 held a single-choice poll to one
    // vote per member, 0321 let an investment order be approved, 0320 stopped a
    // restricted member deleting their way back to a broader social role, 0319
    // scoped a location row to the member it names and 0318 gave
