...
const tr = (key: string, params?: Record<string, string | number>) => translate(getMessages('en-US'), key, params);
...
describe('touchCadenceDays', () => {
  it('returns the median gap with ≥3 touches, null otherwise', () => {
    ...
    const t2 = buildContactTimeline({ t: tr, interactions: [li('1', '2026-05-01')], communications: [], now: NOW });
...
describe('contactHealth', () => {
  it('reports no_history with a starter suggestion', () => {
    const h = contactHealth([], 'Grandma June', tr, NOW);
  ...
  it('is fresh right after a touch', () => {
    ...
    expect(contactHealth(t, 'Aunt Meg', tr, NOW).status).toBe('fresh');
  });
  ...
  it('flags overdue when the gap dwarfs the cadence', () => {
    ...
    const h = contactHealth(t, 'Coach Dana', tr, NOW);
  ...
  it('uses gentle defaults without a cadence baseline (due at 30d)', () => {
    const t = buildContactTimeline({ t: tr, interactions: [li('1', '2026-05-10')], communications: [], now: NOW });
    expect(contactHealth(t, 'Dr. Patel', tr, NOW).status).toBe('due'); // 36 days
  });
});
