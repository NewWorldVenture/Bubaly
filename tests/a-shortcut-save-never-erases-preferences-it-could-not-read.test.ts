// saveCaptureShortcutsAction does a read-merge-write into the shared
...
// collapses to `{}`. One tap on a shortcut's X during a statement timeout would
// then write `{ captureShortcuts: [...] }` over the member's App Lock config,
// their Google Calendar token and everything else in the blob — and report ok.
//
// The guard is the one the siblings already have: app/(app)/settings/app-lock-actions.ts
// (`if (prefsError) return actionFailure(...)`), app/(app)/referrals/actions.ts
// and app/api/google/calendar/callback/route.ts. Read the error, FAIL CLOSED,
// write nothing. The client already renders that as a "couldn't save" toast
// (components/capture/capture-shortcuts.tsx:143), so the failure is visible and
// the member's settings survive it.
...
// What the member already has in the shared blob. None of it belongs to capture.
const OTHER_PREFS = {
...
describe('saving capture shortcuts onto a preferences read that failed', () => {
  it('writes NOTHING when the preferences read is refused', async () => {
    ...
    const result = await saveCaptureShortcutsAction({ keys: ['tasks', 'grocery'] });

    // The whole point: no upsert at all. An upsert built on a read that did not
    // happen replaces the column with just the capture key.
  ...
  it('reports the failure to the caller instead of a silent ok', async () => {
    ...
    await expect(saveCaptureShortcutsAction({ keys: ['tasks'] })).resolves.toMatchObject({ ok: false });
  });
  ...
  it('still merges — and preserves every other key — when the read succeeds', async () => {
    ...
    expect(upserts[0].row.notification_prefs).toEqual({ ...OTHER_PREFS, captureShortcuts: ['tasks', 'grocery'] });
  });

  it('treats a member with no preferences row as empty, not as a failure', async () => {
    read = { data: null, error: null };

    const result = await saveCaptureShortcutsAction({ keys: ['notes'] });

    expect(result).toEqual({ ok: true });
    expect(upserts).toHaveLength(1);
    expect(upserts[0].row.notification_prefs).toEqual({ captureShortcuts: ['notes'] });
  });
});
