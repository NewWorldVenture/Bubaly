...
for (const operation of ['edit', 'delete'] as const) test(`a policy-refused catalog ${operation} reports failure and preserves the reward`, async ({ page }) => {
  const state = await fixture(page); await mount(page, 'parent'); await ready(page);
  state.denyMutation = true;
  ...
  await expect.poll(() => page.evaluate(() => window.__rewardsLedger.toasts.filter(toast => toast.kind === 'error').length)).toBe(1);
  expect(await page.evaluate(() => window.__rewardsLedger.toasts.filter(toast => toast.kind === 'success'))).toEqual([]);
  expect(state.rows.rewards).toEqual([{ ...reward }]);
  expect(state.writes).toHaveLength(1);
  if (operation === 'edit') await expect(page.getByRole('dialog')).toBeVisible();
