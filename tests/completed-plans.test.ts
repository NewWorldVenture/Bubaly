...
import { loadCompletedPlans } from '@/lib/metric/completed-plans';
...
describe('recorded completed-plan counts', () => {
  it('uses durable completion time, including older requests completed this week', async () => {
    ...
    await expect(loadCompletedPlans(db, FAMILY, NOW)).resolves.toEqual({
  ...
  it('counts each complete run once and excludes explicit partial and unsuccessful states', async () => {
    ...
    const result = await loadCompletedPlans(db, FAMILY, NOW);
  ...
  it('reports undated completed history separately without substituting creation or approval time', async () => {
    ...
    expect(await loadCompletedPlans(db, FAMILY, NOW)).toMatchObject({ available: true, completedRuns: 0, undatedCompletedRuns: 3 });
  });

  it('keeps actual zero available and never adds activity, reminders or autopilot to completed plans', async () => {
    ...
    expect(await loadCompletedPlans(db, FAMILY, NOW)).toMatchObject({ available: true, completedRuns: 0, undatedCompletedRuns: 0 });
  ...
  it('uses exact server counts instead of a capped row list', async () => {
    ...
    expect(await loadCompletedPlans(db, FAMILY, NOW)).toMatchObject({ available: true, completedRuns: 1501 });
  });
  ...
  it.each([0, 1])('makes either missing exact count unavailable (read %s)', async (failedRead) => {
    ...
    expect(await loadCompletedPlans(db, FAMILY, NOW)).toEqual({ available: false });
  ...
  it('rejects an invalid window or missing household before making a query', async () => {
    const from = vi.spyOn(db, 'from');
    expect(await loadCompletedPlans(db, '', NOW)).toEqual({ available: false });
    expect(await loadCompletedPlans(db, FAMILY, new Date('invalid'))).toEqual({ available: false });
