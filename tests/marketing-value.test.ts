...
describe('pricing-content.tsx mounts the block and keeps the toggle intact', () => {
  ...
  it('renders the per-day line UNDER the price, on paid tiers only', () => {
    expect(pricing).toContain('import {\n  VALUE_TIERS,\n  formatPerDay,\n  perDayCents,');
    ...
    // Only the two paid cards pass perDay; the trial card does not.
    const trialCard = pricing.slice(pricing.indexOf('name="5-Day Free Trial"'), pricing.indexOf('name="Family Basic"'));
  ...
  it('puts cancel-anytime under the Family+ price', () => {
    const plusCard = pricing.slice(pricing.indexOf('name="Family+"'));
  ...
  it('takes the highlight rail from the six hero outcomes plus Kitchen Mode', () => {
    ...
    expect(pricing).toContain("titleKey: 'kitchenMode.eyebrow'");
