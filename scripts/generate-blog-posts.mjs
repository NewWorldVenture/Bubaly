...
// category tabs, free-license Unsplash hero photos, #hashtags (always #bubaly),
...
// Batch 1 (0226) pins a LoremFlickr URL per post (kept for its committed file).
// Batch 2 stores NO hero URL: the codebase moved off LoremFlickr (0231 drops it,
// lib/blog/posts.ts strips it) and renders a bespoke, unique, on-brand
// <BlogCover> for image-less posts — so batch-2 photos are professional,
// unique, and free with zero third-party/unverified-license URLs in the DB.
const LOCK_BASE = 1000;
const valueRows = posts.map((p, idx) => {
  const hero = BATCH === 2
    ? `null, null, null`
    : `${q(`https://loremflickr.com/1600/900/${p.heroKeywords}?lock=${LOCK_BASE + idx}`)}, ${q(p.heroImageAlt)}, ${q('LoremFlickr (CC)')}`;
