...
import { createPkceCookieExchange, createRecoveryCookieExchange } from '@/lib/auth/recovery-cookies';
...
export async function GET(request: Request) {
  ...
  if (code) {
    // Exchange against an isolated cookie view: a constructor refresh of the
    // visitor's old session must not consume the pending verifier first.
    // Failed exchanges never publish the SDK's cleanup of that verifier.
    const exchange = await createPkceCookieExchange();
    try {
      const supabase = exchange.client;
      const { error } = await supabase.auth.exchangeCodeForSession(code);
      if (error) console.error('[auth-callback] code exchange failed', error);
      if (!error) {
        const finish = (response: NextResponse) => { exchange.applyTo(response); return response; };
        ...
        // The provider exchange succeeded and its session is staged. A transient
        // user lookup failure should still let that sign-in reach its ordinary
        // destination. A definitive rejection discards the staged cookie batch.
        //
        ...
        if (!user) {
          if (userError && isRetryableAuthError(userError)) {
            return finish(redirect(next));
          }
          return redirect(retryHref);
        }
        ...
        const res = redirect(destination);
