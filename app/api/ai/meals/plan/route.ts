...
import { normalizeOptions } from '@/lib/meals/voting';
 ...
 * meals + recipes, weighted by their votes, honoring dietary constraints and
...
export async function POST(req: Request) {
  ...
  // Candidate dishes ------------------------------------------------------
  const [{ data: meals }, { data: recipes }, { data: polls }] = await Promise.all([
    ...
    supabase.from('meal_polls').select('options').eq('family_id', familyId),
  ]);

  // Vote scores: how many votes each meal/recipe has accumulated across polls.
  const voteScore: Record<string, number> = {};
  for (const p of polls ?? []) {
    for (const o of normalizeOptions((p as { options: unknown }).options)) {
      const ref = o.meal_id ? `meal:${o.meal_id}` : o.recipe_id ? `recipe:${o.recipe_id}` : null;
      if (ref) voteScore[ref] = (voteScore[ref] ?? 0) + o.voter_ids.length;
    }
  }

  const candidates = buildCandidates(meals ?? [], recipes ?? [], voteScore);
