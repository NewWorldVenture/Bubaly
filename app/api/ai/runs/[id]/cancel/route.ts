// POST /api/ai/runs/[id]/cancel — one of the human levers over a run (§17).
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!id) return NextResponse.json({ error: t('cancel.runNotFound'), code: 'not_found' }, { status: 404 });

    const scope = scopeFromUserContext(authed.ctx, authed.supabase);
    const result = await applyRunControl(scope, id, 'cancel', {}, { startedAtMs: startedAt });
    ...
    console.error('[api/ai/runs/cancel] control failed', error);
    return NextResponse.json({ error: t('cancel.bubalyCouldNotUpdateThat'), code: 'unknown' }, { status: 500 });
  }
}
