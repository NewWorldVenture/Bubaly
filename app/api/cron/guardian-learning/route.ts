...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    await gFrom('guardian_suggestions')
    ...
    const { data: recent } = await gFrom('guardian_communications')
      .select('family_id')
    ...
    return NextResponse.json({ ok: true, families: families.length, suggestionsCreated: totalCreated });
  } catch (err) {
