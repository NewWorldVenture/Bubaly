...
import {
  buildSuggestions, confidenceTier, type FamilySnapshot,
} from '@/lib/autopilot/engine';

// POST /api/autopilot/scan — the Family Autopilot pass.
// Reads the family's real data, runs the prediction engine, and reconciles the
// `autopilot_suggestions` table: new high-confidence ("auto") reminders are
// executed automatically (reversibly), everything else is surfaced for approval
// or awareness. Resolved suggestions are respected (never re-nagged); stale open
// ones whose signal disappeared are cleared.
export async function POST() {
  try {
    ...
    const [
      ...
      { data: members }, { data: groceries }, { data: apptReminders }, { data: existing },
    ] = await Promise.all([
    ...
    for (const d of drafts) {
      ...
      if (prior && RESOLVED.has(prior.status)) continue; // respect prior resolution
      if (prior) continue; // already open — leave it (avoid churn)

      const isAuto = confidenceTier(d.confidence) === 'auto';
      let status: 'open' | 'auto_executed' = 'open';

      // Reversible auto-execution: create the reminder the autopilot is confident about.
    ...
    return NextResponse.json({ scanned: drafts.length, autoExecuted, cleared: stale.length });
  } catch (err) {
