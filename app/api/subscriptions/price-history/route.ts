...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (!ctx) return json({ code: 'signed_out', error: t('priceHistory.signInToReviewRecorded') }, 401);
    if ('needsFamily' in ctx) return json({ code: 'needs_family', error: t('priceHistory.selectAnExistingFamilyFirst') }, 403);
    if (!isManager(ctx.active.role)) return json({ code: 'role_required', error: t('priceHistory.aParentOrAdultMust') }, 403);
    if (!ctx.active.member?.id || ctx.active.member.is_active !== true) return json({ code: 'membership_required', error: t('priceHistory.aCurrentActiveMembershipIs') }, 403);
    ...
    if (ids.length !== 1 || !/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(ids[0])) {
      return json({ code: 'invalid_subscription', error: t('priceHistory.chooseAnExistingTrackedSubscription') }, 400);
    }
    ...
    if (settingError) return json({ code: 'access_unavailable', error: t('priceHistory.featureAccessCouldNotBe') }, 503);
    ...
    for (const key of ['subscription-tracking', 'finances']) {
      ...
      if (access === 'hidden') return json({ code: 'feature_off', error: t('priceHistory.notFound') }, 404);
      if (access === 'locked') return json({ code: 'plan_required', error: t('priceHistory.yourFamilyPlanDoesNot') }, 403);
    }
    ...
    if (tracked.error) return json({ code: 'source_unavailable', error: t('priceHistory.theTrackedSubscriptionCouldNot') }, 503);
    if (!tracked.data) return json({ code: 'not_found', error: t('priceHistory.thisTrackedSubscriptionIsNot') }, 404);
    if (tracked.data.family_id !== familyId || tracked.data.id !== ids[0]) return json({ code: 'source_unavailable', error: t('priceHistory.theTrackedSubscriptionScopeCould') }, 503);
    ...
    if (accountIds.length) {
      ...
      if (accounts.error || !Array.isArray(accounts.data)
        ...
        return json({ code: 'source_unavailable', error: t('priceHistory.recordedChargeCurrenciesCouldNot') }, 503);
      }
    ...
    return json({ code: 'review_unavailable', error: t('priceHistory.recordedChargeHistoryIsTemporarily') }, 503);
  }
}
