    ...
    <div className="rounded-2xl border border-border bg-surface/40 p-4">
      <div className="flex flex-wrap items-start justify-between gap-2">
        ...
        <div className="flex flex-col items-end gap-1">
          {item.isDisputed && <Badge tone="warning"><MessageCircle className="h-3 w-3" /> {t('missionsReviewCard.disputed')}</Badge>}
          {item.safetyFlags.length > 0 && <Badge tone="danger"><AlertTriangle className="h-3 w-3" /> {t('missionsReviewCard.safety')}</Badge>}
      ...
      {item.note && <p className="mt-2 rounded-lg bg-elevated/50 p-2 text-sm"><span className="font-medium">{t('missionsReviewCard.kidNote')}</span> {item.note}</p>}
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2 rounded-xl border border-border bg-elevated/40 p-2 text-sm">
        ...
        {item.aiIsFallback ? (
          <span className="text-muted">{t('missionsReviewCard.aiReviewUnavailablePleaseCheckManually')}</span>
        ) : (
          <>
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2 border-t border-border/50 pt-3">
        <form action={(fd) => start(async () => {
          setError(null);
          const result = await approveSubmissionAction(fd);
        ...
        <form action={(fd) => start(async () => {
          setError(null);
          const result = await rejectSubmissionAction(fd);
          if (!result.ok) setError(result.error ?? null);
          ...
          <button name="redo" value="0" disabled={pending} className="inline-flex h-9 items-center gap-1 rounded-lg border border-border px-3 text-sm font-medium text-muted hover:text-danger">
            <X className="h-4 w-4" /> {t('missionsReviewCard.reject')}
          </button>
        </form>
      </div>

      {/* Approve and Reject used to fail in complete silence: seven and five
          bare returns, with revalidatePath on the success path only, so a failed
          wallet credit left the card sitting exactly where it was and the parent
          clicked again. The child already got this treatment on their own submit
          form; the parent gets it here. */}
      {error && (
