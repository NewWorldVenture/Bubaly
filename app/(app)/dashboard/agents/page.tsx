...
import { loadFamilyGraph } from '@/lib/reasoning/server';
...
export default async function AgentsPage() {
  ...
  // Knowledge graph → relationship-level reasoning for the Chief of Staff, loaded
  // through the shared reasoning loader (moat R1). Best-effort: a missing table
  // (migration not yet applied) yields an empty graph and no insights, never an error.
  const graph = await loadFamilyGraph(supabase, familyId);
