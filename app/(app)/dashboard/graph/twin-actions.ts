...
// Twin projection — the wired half of the Household Digital Twin's "continuously-
// updated linked model". Reads the family's real cross-domain tables (members,
// pets, vehicles, schools, teams, routines, places, accounts), runs the pure
// projector in lib/twin/project.ts, and upserts the result into the knowledge
// graph (graph_entities + graph_edges). Idempotent: entities upsert on their
// source row (uq_graph_entities_ref), edges on (family,source,target,relation).
// Everything goes through the RLS-scoped server client. No mock data.

import { requireUserContext } from '@/lib/supabase/auth';
import { createServer } from '@/lib/supabase/server';
import { projectTwin, projectionSummary, type TwinSnapshot } from '@/lib/twin/project';
...
export async function projectTwinAction(): Promise<ProjectResult> {
  ...
  // Pull the live cross-domain snapshot (each query is family-scoped by RLS too).
  ...
  const snap: TwinSnapshot = {
    ...
    vehicles: (vehicles.data ?? []).map((v) => ({
      id: v.id,
      label: v.nickname || [v.make, v.model].filter(Boolean).join(' ') || null,
  ...
  if (projection.entities.length === 0) {
    return { ok: true, entities: 0, edges: 0 };
  }

  // 1) Upsert entities keyed by their source row.
  ...
  const { error: entErr } = await sb
    .from('graph_entities')
    .upsert(entityRows, { onConflict: 'family_id,ref_table,ref_id' });
  if (entErr) return { ok: false, error: entErr.message };

  // 2) Resolve entity keys -> ids (read back the mirrored rows).
  const { data: idRows, error: readErr } = await sb
    .from('graph_entities')
    .select('id, ref_table, ref_id')
  ...
  // 3) Upsert edges, resolving keys to ids (skip any that didn't resolve).
  const edgeRows = projection.edges
    .map((e) => {
      ...
      return { family_id: familyId, source_id, target_id, relation: e.relation, weight: e.weight, created_by: ctx.user.id };
    })
  ...
  if (edgeRows.length) {
    const { error: edgeErr } = await sb
      .from('graph_edges')
      .upsert(edgeRows, { onConflict: 'family_id,source_id,target_id,relation' });
    if (edgeErr) return { ok: false, error: edgeErr.message };
  }
