...
async function softDelete(supabase: Client, table: string, id: string, familyId: string, userId: string) {
  await supabase.from(table as 'vehicles').update({ deleted_at: new Date().toISOString(), updated_by: userId }).eq('id', id).eq('family_id', familyId);
}
...
export async function saveVehicleAction(fd: FormData) {
  ...
  if (id) await supabase.from('vehicles').update(row).eq('id', id).eq('family_id', familyId);
  else await supabase.from('vehicles').insert({ ...row, family_id: familyId, created_by: userId });
...
export async function saveLicenseAction(fd: FormData) {
  ...
  if (id) await supabase.from('driver_licenses').update(row).eq('id', id).eq('family_id', familyId);
  else await supabase.from('driver_licenses').insert({ ...row, family_id: familyId, created_by: userId });
...
export async function saveRegistrationAction(fd: FormData) {
  ...
  if (id) await supabase.from('vehicle_registrations').update(row).eq('id', id).eq('family_id', familyId);
  else await supabase.from('vehicle_registrations').insert({ ...row, family_id: familyId, created_by: userId });
...
export async function saveInspectionAction(fd: FormData) {
  ...
  if (id) await supabase.from('vehicle_inspections').update(row).eq('id', id).eq('family_id', familyId);
  else await supabase.from('vehicle_inspections').insert({ ...row, family_id: familyId, created_by: userId });
  revalidate('/dashboard/auto', '/dashboard/auto/registration');
}
...
export async function savePolicyAction(fd: FormData) {
  ...
  if (id) await supabase.from('auto_insurance_policies').update(row).eq('id', id).eq('family_id', familyId);
  else await supabase.from('auto_insurance_policies').insert({ ...row, family_id: familyId, created_by: userId });
...
export async function saveRentalAction(fd: FormData) {
  ...
  if (id) await supabase.from('rental_cars').update(row).eq('id', id).eq('family_id', familyId);
  else await supabase.from('rental_cars').insert({ ...row, family_id: familyId, created_by: userId });
...
export async function saveAutoServiceAction(fd: FormData) {
  ...
  await supabase.from('auto_service_records').insert({
  ...
  // Keep the vehicle odometer fresh.
  if (vehicleId && mileage != null) await supabase.from('vehicles').update({ mileage }).eq('id', vehicleId).eq('family_id', familyId);
