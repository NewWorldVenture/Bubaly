...
// failure (resolved `error` field or throw) so a partial outage is diagnosable
  ...
  return (
    <section className="flex flex-col rounded-2xl border border-border bg-surface/40 p-5">
      <div className="mb-4 flex items-center justify-between">
        ...
        <Link href={href} className="flex items-center gap-0.5 rounded-lg px-2 py-1 text-xs font-semibold text-brand-text transition hover:bg-brand/10" aria-label={t('itemAction.open', { name: title })}>{t('food.open')}{' '}<ChevronRight className="h-3.5 w-3.5" />
        </Link>
      </div>
...
export default async function FoodPage() {
  ...
  const [
    ...
    safe('meal_plans', supabase.from('meal_plans').select('plan_date, meal_id', { count: 'exact' })
      .eq('family_id', familyId).eq('meal_type', 'dinner').gte('plan_date', todayIso).lte('plan_date', weekEnd).order('plan_date')),
    safe('recipes', supabase.from('family_recipes').select('id, name, rating', { count: 'exact' })
      .eq('family_id', familyId).order('created_at', { ascending: false }).limit(4)),
    safe('grocery_items', supabase.from('grocery_items').select('id, name', { count: 'exact' })
      .eq('family_id', familyId).eq('is_checked', false).limit(4)),
    safe('pantry_items', supabase.from('pantry_items').select('id, name, expires_at, quantity, unit', { count: 'exact' })
      .eq('family_id', familyId).lte('expires_at', expSoon).order('expires_at', { ascending: true, nullsFirst: false }).limit(4)),
    safe('favorites', supabase.from('family_recipes').select('id, name, rating', { count: 'exact' })
      .eq('family_id', familyId).eq('is_favorite', true).order('rating', { ascending: false, nullsFirst: false }).limit(4)),
    safe('food_scores', supabase.from('family_food_scores').select('overall, grade, snapshot_date')
      .eq('family_id', familyId).order('snapshot_date', { ascending: false }).limit(1)),
    safe('dining_out', supabase.from('dining_out').select('id, name, cuisine, rating', { count: 'exact' })
  ...
  const { data: meals } = mealIds.length
    ? await safe('meals', supabase.from('meals').select('id, name').in('id', mealIds))
      ...
      <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <FeatureCard index={1} title={t('dashboardFood.mealPlanner')} href="/dashboard/meals" icon={CalendarRange} tint="bg-blue-500/15 text-blue-400" count={planCount ?? 0} countLabel="dinners planned this week">
          {planRows.length === 0 ? <EmptyHint>{t('dashboardFood.noDinnersPlannedYet')}</EmptyHint>
            : planRows.slice(0, 4).map((p) => <ListRow key={p.plan_date} label={p.meal_id ? (mealName.get(p.meal_id) ?? 'Planned meal') : 'Planned meal'} meta={p.plan_date === todayIso ? 'Today' : fmtDay(p.plan_date, locale)} dot="bg-blue-400" />)}
        </FeatureCard>
        ...
        <FeatureCard index={4} title={t('dashboardFood.pantryInventory')} href="/dashboard/pantry" icon={Boxes} tint="bg-orange-500/15 text-orange-400" count={pantryCount ?? 0} countLabel="expiring soon">
          {(pantry ?? []).length === 0 ? <EmptyHint>{t('dashboardFood.nothingExpiringSoon')}</EmptyHint>
            : (pantry as P[]).map((p) => <ListRow key={p.id} label={p.name} meta={p.expires_at ? fmtDay(p.expires_at, locale) : `${p.quantity}${p.unit ? ' ' + p.unit : ''}`} dot="bg-orange-400" />)}
        </FeatureCard>
