  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-border p-3">
      ...
      <div className="flex shrink-0 items-center gap-2 text-xs">
        ...
        <button aria-label={t('a11y.delete')} onClick={() => start(async () => { await deleteRewardAction(reward.id); })} disabled={pending} className="inline-flex items-center gap-1 text-muted hover:text-danger">{pending ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Trash2 className="h-3.5 w-3.5" />}</button>
      </div>
    </div>
  );
}
