        ...
        <div className="space-y-5">
          <Card id="new-page">
            ...
            <form action={createPlatformPage} className="mt-4 space-y-3">
              <select aria-label={t('fieldName.pageType')} name="page_type" className={inputCls} defaultValue="guide">{PAGE_TYPES.map((type) => <option key={type.value} value={type.value}>{type.label} ({type.prefix})</option>)}</select>
