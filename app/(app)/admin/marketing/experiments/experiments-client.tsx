  ...
  return (
    <div>
      {!open ? (
        ...
        >
          <input name="name" required placeholder="Experiment name (e.g. Hero CTA copy)" className="h-10 w-full rounded-lg border border-border bg-bg px-3 text-sm" />
          <input name="hypothesis" placeholder="Hypothesis (optional)" className="h-10 w-full rounded-lg border border-border bg-bg px-3 text-sm" />
          <input name="metric" defaultValue="signup" placeholder="Conversion metric label" className="h-10 w-full rounded-lg border border-border bg-bg px-3 text-sm" />
          <label className="block text-xs text-muted">Variants (one per line; first is the control)
          ...
          <div className="flex justify-end gap-2">
            <button type="button" onClick={() => setOpen(false)} className="rounded-lg border border-border px-3 py-1.5 text-sm">Cancel</button>
...
export function ExperimentControls({ id, status, variants }: {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      {status !== 'running' && <button disabled={pending} onClick={() => go(() => setExperimentStatus(id, 'running'))} className="rounded-lg border border-border px-2.5 py-1 text-xs hover:bg-elevated">Start</button>}
      {status === 'running' && <button disabled={pending} onClick={() => go(() => setExperimentStatus(id, 'paused'))} className="rounded-lg border border-border px-2.5 py-1 text-xs hover:bg-elevated">Pause</button>}
      <select
        disabled={pending}
        defaultValue=""
