...
export default async function CaptureLayout({ children }: { children: React.ReactNode }) {
  ...
  const [{ data: members }, { data: prefs }, { data: sub }, superAdmin, featureTiers] = await Promise.all([
    ...
    supabase.from('subscriptions').select('plan, status').eq('family_id', ctx.active.familyId).in('status', ['active', 'trialing']).maybeSingle(),
    isSuperAdmin(),
